...
- **`composer.space` and every iOS build ship the curated set**; `nightly`, `dev` and `staging` ship
  the full one. So the plugin registry, the dev plugin loader and most plugins are simply absent from
  production — not broken.
- **Both talk to Edge prod**, so an object created on nightly can be opened in production with no
...
| -------------- | ------------------------ | ------------ | --------------------------------- | --------------------- | ------------------------------------------------------------------------------------------- |
| **dev**        | `composer-dev…`          | EDGE nightly | manual → `dev`                    | composer              | desktop + iOS → TestFlight; iOS ships the curated plugin set                                |
