...
| K2 | Tables | Contacts → Organizations as a table; add a column with a *type*; sort/filter. | The column has a schema, not a format. | D P | M | |
...
| K7 | One dataset, four views | Table → kanban → map → chart on the *same* objects, in four seconds. | This is the single best 10 seconds in the product. | C D P I | M | |
...
| L2 | **Offline merge** | Window B goes offline (devtools), both edit the same paragraph, B reconnects, texts merge. | No conflict dialog. No "last write wins". *Partner beat: Automerge.* | C D I | M | |
...
| L5 | Presence across types | Two people in the same kanban / sheet / drawing, not just text. | CRDT collaboration on a *spreadsheet*. | D I | M | |
...
| A4 | Generate a table | "Build a table of wholesale accounts by reorder cadence" → a real, typed table. | It made a *schema*, then filled it. | C D I | M | |
...
| ID | Clip | What you see | The "wait, what?" | Cuts | Cost | Mark |
| --- | --- | --- | --- | --- | --- | --- |
| G1 | **Trace panel** | Assistant answers; trace panel shows tool calls, arguments, typed results, timings. | *Partner beat: Effect.* | C D | M | |
...
*Gmail is mocked locally (synthetic mailbox in the demo seed). The real OAuth connect flow is deliberately not shown.*
...
| ID | Clip | What you see | The "wait, what?" | Cuts | Cost | Mark |
| --- | --- | --- | --- | --- | --- | --- |
| T1 | **Native desktop** | Tauri build beside the browser, mutating the same live object. | *Partner beat: Tauri.* | C D I | L | |
...
*Depends on the sibling repo at `/Users/burdon/Code/dxos/edge` — not this worktree.*
...
| W2 | **A space is a Durable Object** | Follow one space's sync into the DO that hosts it. | The sync engine *is* a Durable Object. | C D | L | |
