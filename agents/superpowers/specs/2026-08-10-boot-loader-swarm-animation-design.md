...
| B — Orbital        | Anticlockwise circulation at a personal radius (uniform in [74, 128]), slight radial wobble; docked ring keeps rotating rigidly (`ringRotationSpeed` 1.5, normalized units of 0.0001 rad/ms, range [-10, 10], negative = clockwise).                                                                                                           | 32  | 2.0  |
...
| E — Halo           | Tiny dots fade in on three tight concentric rings (radii 66/78/90, slot index mod 3) with alternating directions (anticlockwise/clockwise/anticlockwise), each dot at its own fast rate (base `ringRotationSpeed` 8 normalized units + per-dot variation); close cross-ring passes (≤18) randomly link (p=0.5, held ~0.8 s) — no wander phase. | 24  | 0.9  |
