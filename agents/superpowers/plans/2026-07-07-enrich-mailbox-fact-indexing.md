...
- **Create** `packages/core/compute/pipeline-email/src/stages/extract-facts-commit.ts` — a facts-_returning_ stage variant + a `FactUnit` type (kept separate so the side-effecting `extractFactsStage` stays untouched for batch use).
...
- **Create** `packages/ui/react-ui-rdf/*` — relocate `FactViewer` + its presentational helpers into a plugin-consumable package.
...
The batch `EmailPipeline.run` keeps the side-effecting `extractFactsStage`. The cursored operation needs a variant that _returns_ facts so `factsCommit` can write them page-atomically.
...
export const makeFactStoreRegistry = (): FactStoreRegistry => {
  ...
  const forSpace = (spaceId: string): FactStore.FactStoreApi => {
    ...
    const service = Effect.runSync(
      Layer.build(FactStore.layerMemory).pipe(
        Effect.scoped,
        Effect.map((ctx) => ctx.get(FactStore.FactStore as any)),
      ),
    ) as FactStore.FactStoreApi;
        ...
        Effect.flatMap((source) =>
          source.pipe(
            SyncBinding.dedupStage(
              'facts-dedup',
              (m: Message.Message) => messageSource(m),
              (m) => Date.parse(m.created),
            ),
            extractFactsUnitStage(extract),
            Stage.map('tally', (unit) =>
              Effect.sync(() => {
                processed += 1;
                factCount += unit.facts.length;
                return unit;
              }),
            ),
...
## Task 8: Relocate `FactViewer` into `@dxos/react-ui-rdf`
...
- Create: `packages/ui/react-ui-rdf/{package.json,moon.yml,tsconfig.json,src/index.ts,src/FactViewer.tsx,src/types.ts}`
...
Create `packages/ui/react-ui-rdf/package.json` with `"private": true` (mandatory for new packages), name `@dxos/react-ui-rdf`, and deps: `@dxos/pipeline-rdf` (`workspace:*`), `@dxos/react-ui` (`workspace:*`), `@dxos/react-ui-graph` (`workspace:*`), `@dxos/react-ui-list` (`workspace:*`), `@dxos/ui-theme` (`workspace:*`), plus `react` from the catalog. Add `moon.yml` (copy a sibling react-ui package's) and `tsconfig.json` with refs to those deps.
...
In stories-brain, replace the local `FactViewer` export/usages with imports from `@dxos/react-ui-rdf`; add the dep (`pnpm add --filter @dxos/stories-brain @dxos/react-ui-rdf` — but as a workspace package add it as `workspace:*` in package.json, per repo rule) and a tsconfig ref. Delete the old component files. Move `FactViewer.stories.tsx` into the new package (or keep it in stories-brain importing from the new package — pick one; prefer colocating the story with the component in the new package).

- [ ] **Step 4: Install + build**

Run: `pnpm install --no-frozen-lockfile && moon run react-ui-rdf:build && moon run stories-brain:build`
...
```bash
git add packages/ui/react-ui-rdf packages/stories/stories-brain pnpm-lock.yaml
git commit -m "refactor(react-ui-rdf): extract FactViewer into a reusable package"
```
...
- Modify: plugin-inbox `package.json` (add `@dxos/react-ui-rdf` as `workspace:*`) + tsconfig ref
...
describe('MailboxFactsCompanion', () => {
  test('renders FactViewer with the mailbox facts', ({ expect }) => {
    render(
      /* <Providers facts=[…]> */ <MailboxFactsCompanion mailbox={/* mailbox */ undefined as any} /> /* </Providers> */,
    );
...
```bash
moon run plugin-connector:test pipeline-email:test plugin-inbox:test react-ui-rdf:build stories-brain:build
```
...
Expected: no _new_ casts except justified type-system-boundary ones. The registry's `Layer.build` materialization (Task 5) may need one boundary cast — if so, add a concise comment saying why no typed alternative exists, or refactor to the LayerSpec-direct approach noted in Task 5. Remove all others.
