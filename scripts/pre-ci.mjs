...
async function main() {
  ...
  try {
    await $`moon exec :lint --on-failure continue --quiet -- --fix`;
  ...
  try {
    await $`moon exec :build --on-failure continue --quiet`;
    await $({
      env: {
        ...process.env,
        CI: 1,
      },
    })`moon exec :test --on-failure continue --quiet -- --no-file-parallelism --project=node`;
