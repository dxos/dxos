...
- **It is faster**, and its runners sit closest to the moon remote cache — measured, and the reason Depot stayed when Blacksmith was evaluated as an alternative (CI project [`REPORT.md`](../.agents/projects/ci/REPORT.md), "Runners").
- **The workflow syntax is GitHub's.** Migrating a workflow is moving the file; `uses:`, `needs:`, matrices, `if:` expressions and the `github` context all behave as before. Depot then adds things Actions has no syntax for — `parallel:` blocks are one, below.
- **A workflow can be run from the CLI**, against your working tree, without pushing. That is the capability this repo leans on hardest, and it is the reason the affected scoping below is written the way it is.

Compatibility ends where a third party checks GitHub's *own* run identity rather than the workflow's behaviour — artifact handoff across `workflow_run`, npm OIDC, pkg.pr.new. Four workflows stayed behind for exactly that; each names its symptom in the GitHub README's table.
...
`act` (the previous local runner) reads `.github/workflows` and no longer sees **Check**.

## Affected scoping

Every `moon` step in **Check** is unconditional. What it runs is decided once per job by [`actions/affected`](./actions/affected), which exports `MOON_AFFECTED` / `MOON_BASE` — both read by `moon run` and `moon exec` (2.5.2), including through the Trunk uploader's `run:` input. `moon` steps therefore carry no `--affected` flag and no `if:`; the previous shape was an Affected and an All variant of every step, gated on `branch != 'main'`.
...
That second-to-last row is what makes `depot ci run` useful: with no `GITHUB_*` in the environment the resolver falls back to the merge-base, so a local run scopes itself the way the real trigger would instead of rebuilding the world.
...
Depot CI runs [steps concurrently](https://depot.dev/docs/ci/how-to-guides/parallel-steps) inside one job via a `parallel:` block (with `sequential:` for ordered groups inside it) — one of the things it adds over GitHub's syntax. Used where steps are independent and the job is otherwise paying their sum:
...
Only step outputs, env and `$GITHUB_PATH` are snapshotted per unit and merged back on join. The filesystem is shared across the whole block.
...
| `e2e` | Playwright e2e via uploader, 6 matrix cells: browser (`chromium` / `firefox` / `webkit` via `PLAYWRIGHT_BROWSER`) x shard (`composer` runs `composer-app:e2e` in a dedicated cell; `rest` runs an explicit target list computed by `moon query tasks "task=e2e && project!=composer-app"`, so a new suite joins the pool with no hand-maintained list — see DESIGN.md for why a glob or `moon exec --query` cannot express this). Job runs only for `main` / `changeset-release/*` refs, or `workflow_dispatch` with `e2e` (see [`check.yml`](./workflows/check.yml)). | `test-results/playwright/report/*.xml` |

**unit/browser/storybook/workerd** go through Trunk on typical PRs; **e2e** only when the `e2e` job runs (not on ordinary topic-branch PRs). Exact `moon` commands and `env` are in [`check.yml`](./workflows/check.yml).
