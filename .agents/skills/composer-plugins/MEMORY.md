...
- Icon-only buttons (drag handle, sigil) are BLANK in a Mosaic native drag image because `@dxos` `Icon` renders `<svg><use href={useIconHref(icon)}/></svg>` and `useIconHref` returns an EXTERNAL file ref `/icons.svg#<id>` — Chrome's native drag-image rasterization does not resolve external `<use href>` sprite references. Fix: `Mosaic.Tile` now takes an optional `preview?: ReactNode` slot rendered into the `setCustomNativeDragPreview` portal instead of the cloned tile; supply INLINE svg/text only (no sprite `Icon`). `StackSection` passes a chip = inline grip glyph (`<svg><circle/>…`) + title. When `preview` is omitted the tile is still cloned at full size (legacy behavior). NotebookSection/ObjectCardStack could adopt the same slot.
