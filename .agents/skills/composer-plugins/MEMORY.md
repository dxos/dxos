...
- Conductor GPT execution tests (`conductor/src/tests/gpt.test.ts`) are `describe.runIf(DX_RUN_SLOW_TESTS==='1')` and use memoized conversations. Regenerate fixtures with `ALLOW_LLM_GENERATION=1 DX_RUN_SLOW_TESTS=1` + creds (write `pnpm -ws 1p-credentials` to a temp file and `source` it — the `eval "$(…)"` form trips the Bash hook); commit `gpt.conversations.json`. On replay, exact-string assertions on LLM text are brittle (responses vary) — assert `text.length > 0`. The `stream output` test HANGS on memoized replay: the gpt node's `tokenStream` is `Stream.fromPubSub` fed by the `request.run` observer, and memoized replay returns the final message without re-publishing/closing the PubSub → `Stream.runCollect` never ends (pre-existing, gated). Prefer a CI-green BUILD/topology test (`react-ui-canvas-compute/src/graph/createComputeGraph.test.ts`). The gpt node uses `@dxos/assistant` `AiRequest.run` → `@effect/ai` `LanguageModel.streamText` (NOT generateObject), so the extractor's `mockAiService` (generateObject) does not stub it.
