...
## 2026-06-19 — plugin-automation / plugin-assistant (move Routine/Skill/Template UI down)

- Moving UI DOWN the dep graph: plugin-assistant already deps plugin-automation, so Routine/Skill/Template containers+components moved INTO automation; assistant re-imports nothing (surfaces moved too). Check the existing edge direction first (`node -e` on package.json) before deciding which way code can move.
