...
python3 .agents/skills/composer-forensics/scripts/locate-origin.py \
  --origin https://nightly.composer.space
```
...
| ----------- | -------------------------------- | ------------------------ |
| `--origin`  | `https://nightly.composer.space` | Target origin URL        |
...
OPFS_DIR="$(python3 .agents/skills/composer-forensics/scripts/locate-origin.py \
  --origin https://nightly.composer.space --json | python3 -c 'import json,sys; print(json.load(sys.stdin)["opfs_pool_dir"])')"

python3 .agents/skills/composer-forensics/scripts/extract-opfs-sqlite.py \
  --opfs-dir "$OPFS_DIR" \
  --out /tmp/composer-forensics/nightly.composer.space
```

Output:

```
/tmp/composer-forensics/nightly.composer.space/
...
node profile-wrap.js /tmp/composer-forensics/nightly.composer.space/DXOS.sqlite
...
bash .agents/skills/composer-forensics/scripts/validate-extract.sh \
  /tmp/composer-forensics/nightly.composer.space/DXOS.sqlite
```
...
node automerge-list.js /tmp/composer-forensics/nightly.composer.space/DXOS.sqlite
# or via probe:
node probe.js /tmp/composer-forensics/nightly.composer.space/DXOS.sqlite automerge list
```
...
```bash
sqlite3 /tmp/composer-forensics/nightly.composer.space/DXOS.sqlite \
