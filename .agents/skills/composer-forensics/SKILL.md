...
- Debug data loss, corruption, or unexpected state on `composer.space`, `nightly.composer.space`, retired origins (`main.composer.space`, `labs.composer.space`), or preview deploys.
...
```bash
node automerge-bench-load.js /tmp/composer-forensics/nightly.composer.space/DXOS.sqlite --largest
