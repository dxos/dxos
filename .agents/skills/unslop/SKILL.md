...
Applies to prose **you write or substantially rewrite**: PR descriptions,
changeset bodies, commit message bodies, `README.md` / `DESIGN.md` / `TASKS.md`,
skill and instruction files, and design docs. It does not license a churn pass
over documents you were not otherwise editing. Rewording an unchanged paragraph
costs review attention and buries the real diff.

Two neighbours own their own ground, and this skill defers to both:

- **Chat replies.** `AGENTS.md` → "Responding to the user" is canonical for the
  shape of a reply (lead with the answer, numbered options, verbosity mode). The
  patterns below still improve the words inside that shape.
- **Code comments.** The `code-style` skill owns the comment rule (say why,
  once, one load-bearing clause, end with a period). Don't apply prose voice
  rules to comments.
