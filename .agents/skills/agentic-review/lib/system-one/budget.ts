...
export const charsForTokens = (tokens: number): number => Math.max(0, Math.floor(tokens * CHARS_PER_TOKEN));

/** A question paired with the id it is asked under, the minimal shape `packQuestions` needs. */
export type PackableQuestion = { id: string; question: unknown };

export type PackedQuestions<T extends PackableQuestion> = {
  batches: T[][];
  oversized: string[];
};
 ...
 */
export const packQuestions = <T extends PackableQuestion>(
  stateTokens: number,
