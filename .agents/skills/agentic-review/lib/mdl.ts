 ...
 */
export const UNITS = ['file', 'pr'] as const;
...
export type ContextKind = keyof typeof CONTEXT_KINDS;

/** A `rule` block as loaded from an `.mdl` document. */
export type Rule = {
  id: string;
  title: string;
  files: string[];
  ...
  systemOne: boolean;
  /** Directory of the `.mdl` file, the glob base for `scope: dir`. */
  dir: string;
  instructions: string;
  path: string;
};
...
export const parseMdlBlocks = (text: string): string[][] => {
...
/** A block header's parsed parts: `<type> [<id>][: <title>]`. */
type ParsedHeader = { type: string; id: string | null; title: string | null };

/** Parse a block's header line `<type> [<id>][: <title>]`. */
const parseHeader = (line: string): ParsedHeader => {
...
/** A `rule` block body before field validation — raw strings, not yet checked against the enums. */
type ParsedRuleBody = {
  files: string[];
  context: string[];
  grep: string | null;
 ...
 */
const parseRuleBody = (bodyLines: string[]): ParsedRuleBody => {
  const lines = dedent(bodyLines);
  const fields: Record<string, string> = {};
  const lists: { files: string[]; context: string[] } = { files: [], context: [] };
  const prose: string[] = [];
  ...
  for (let index = 0; index < lines.length; index++) {
    ...
    if (keyMatch && RULE_KEYS.has(keyMatch[1])) {
      ...
      if (LIST_KEYS.has(key)) {
        ...
        lists[key as 'files' | 'context'] = items.filter(Boolean);
      } else {
        fields[key] = value;
      }
    } else {
...
const isSeverity = (value: string): value is 'warn' | 'error' => VALID_SEVERITIES.has(value);
const isScope = (value: string): value is 'dir' | 'repo' => VALID_SCOPES.has(value);
const isUnit = (value: string): value is Unit => (UNITS as readonly string[]).includes(value);
const isSystemOneToggle = (value: string): value is 'on' | 'off' => VALID_SYSTEM_ONE.has(value);
 ...
 *
 * @param path Absolute path to the `.mdl` file.
 */
export const loadRules = (path: string): Rule[] => {
  ...
  for (const blockLines of parseMdlBlocks(text)) {
    ...
    for (const kind of context) {
      ...
      resolvedContext.push(kind);
    }
    const resolvedSystemOne = systemOne ?? 'on';
    if (!isSystemOneToggle(resolvedSystemOne)) {
    ...
    rules.push({
      ...
      scope: resolvedScope,
      unit: resolvedUnit,
      context: [...new Set(resolvedContext)],
      question,
