...
//
// Usage:
//   bun system-one.ts [--slug=<slug> | --dir=<store>]   fill a prepared review's fragments
//   bun system-one.ts --file=<path> [--file=…] [--rule=<id> …]   probe files, print verdicts
//
...
import {
  ...
  type Verdict,
  type VerdictStatus,
} from '../lib/system-one/checker.ts';
import { makeClient, DEFAULT_MODEL } from '../lib/system-one/client.ts';
...
const settings: Settings = {
  ...
  rounds: Number.parseInt(values.rounds ?? '2', 10),
};
const chunkSize = Math.max(1, Number.parseInt(values.chunk ?? '15', 10) || 15);
const log = (message: string): void => console.error(`system-one: ${message}`);
const client = values['dry-run']
  ? null
  : makeClient({
      apiKey: process.env.TYPESAFE_API_KEY,
      model: values.model,
      concurrency: Number.parseInt(values.concurrency ?? '16', 10),
    });
...
const resolveStore = (): string => {
  ...
  const pending = existsSync(reviews)
    ...
    : [];
  if (pending.length === 0) {
    throw new Error('no prepared review to fill: run prepare.ts first, or pass --file to probe');
  }
  return pending[0];
};

/** Split rule/file pairs into per-file targets and per-rule change-set targets. */
const buildTargets = (pairs: { rule: Rule; file: string }[]): { fileTargets: FileTarget[]; prTargets: PrTarget[] } => {
  const fileTargets = new Map<string, FileTarget>();
  const prTargets = new Map<string, PrTarget>();
...
const summarizeStats = (stats: ReviewStats): string =>
  [
...
const reviewBase = (declared: string | null | undefined): string | null =>
  values.base ?? (declared && declared !== FULL_BASE ? declared : mainMergeBase());
...
if (values.file?.length) {
  ...
  const selected: Rule[] = values.rule?.length
    ? values.rule.map((id) => {
    ...
    : [...rulesById.values()];
  const pairs: { rule: Rule; file: string }[] = [];
  ...
  const manifest = JSON.parse(readFileSync(join(store, GROUPS_MANIFEST), 'utf8')) as GroupsManifest;
  const base = reviewBase(review?.data?.base ? String(review.data.base) : null);
  const pairs: { rule: Rule; file: string; nn: string }[] = [];
  const skipped: { nn: string; ruleId: string; files: string[]; reason: string }[] = [];
  for (const [nn, group] of Object.entries(manifest)) {
    const rule = rulesById.get(group.ruleId);
    if (!group.files) {
      throw new Error(`${GROUPS_MANIFEST} has no file lists: re-run prepare.ts with this version of the harness`);
    }
    if (!rule?.systemOne) {
      skipped.push({
        nn,
  ...
  const groupOf = (ruleId: string, file: string): string | undefined =>
    Object.entries(manifest).find(([, group]) => group.ruleId === ruleId && group.files.includes(file))?.[0];
  const counts: Record<VerdictStatus, number> = { violation: 0, uncertain: 0, clean: 0, unanswered: 0 };
  const uncertain = new Map<string, FollowUpEntry>();
  const bounds = uncertainBounds(result.verdicts, settings);
  if (client) {
    for (const verdict of result.verdicts) {
      ...
      if (status === 'violation' && isReportableVerdict(verdict)) {
    ...
    writeFileSync(
      ...
      `${JSON.stringify(
        result.verdicts.map(({ rule, files: _files, ...verdict }: Verdict) => ({
          rule: rule.id,
          status: classify({ rule, probability: verdict.probability }, settings, bounds),
          ...verdict,
        })),
        null,
        2,
  ...
  const skippedByRule = new Map<string, { reason: string; groups: string[] }>();
