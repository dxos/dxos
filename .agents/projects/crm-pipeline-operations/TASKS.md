...
- [x] **Fixture acquisition + secure storage (goal #1)** — `scripts/fixtures.mjs` (`pnpm fixtures push|pull|list`): age-encrypted client-side, private `test-fixtures` R2 bucket under `mailbox/<user>/`, wrangler auth, writes only into the git-ignored `fixtures/local/`. Decisions: age **per-user private** key (recipient derived from the identity via `age-keygen -y`, so push/pull cannot drift; sharing is per-object via `--recipient`), user provisions the bucket, scripts+docs only, on this PR. Usage documented in `stories-inbox/README.md`. DESIGN.md §8.2 rewritten as implemented.
