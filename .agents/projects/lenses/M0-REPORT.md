...
| `validation.test.ts` (7)      | §10.7 q5 under strict schemas: `ObjectCore.setType` (internal) is the only type switch; an `@2` write while still `@1` is rejected; whole-set validation + data + type switch in one `Obj.update` works; a write to a retained source property is rejected (`Unknown property`) unless the target re-declares it; an invalid late value folds to a report, source untouched |
