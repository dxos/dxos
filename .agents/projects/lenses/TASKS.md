...
- [x] **Post-M0 (Josiah, 2026-08-02) · history-native conflicts via `changeAt`. — VERIFIED,
    ...
    A.merge(...))` → counters tie, fold deterministically loses (user wins — recommended
      default). Same-key fan-in conflicts history-native for free. FORCES §10.6-D: epochs must
      preserve history or drain live conflicts (materialize-to-POJO erases them). Writeups:
