...
- [~] E2E smoke script #2 (OAuth-stub path): .agents/projects/mcp/scripts/e2e-oauth-smoke.mjs written; MCP legs verified live (text+document created, root-collection attach worked — collection now 5 members); playwright leg CONFIRMED FLAW: fresh playwright profile auto-creates its OWN identity so it cannot see the target space; MCP legs pass, assert times out. RESTRUCTURE NEEDED: browser-first self-contained flow — (1) playwright boots composer (fresh context, identity auto-created, agent registers), (2) harvest that identity's key hex + space id from edge-dev.log (newest identityKey auth event / busiest new B… space), (3) OAuth+MCP create doc + root-collection attach into THAT space, (4) assert title visible in the SAME persistent browser context (launchPersistentContext or keep page open)
