...
- [~] USER DIRECTIVE 2026-08-01 (active, overnight): #12431 MERGED; main merged here; edge branch `mcp-task-tools` carries task-tools.ts (5 MCP verbs, 52/52 tests w/ stub) + registry TasksOperationHandlerSet/type registration + uniform catalog bump to 37c17cca78 (blocked on pkg.pr.new publishing that SHA — poller armed). OVERNIGHT RUN (user offline, caffeinate active): (a) Path A — e2e-task-smoke via OAuth stub into the user's space (leave a visible TaskSet); (b) Path B — device-invitation pairing with the 0.32.3 transport (#12432 in queue), then MCP task verbs as the paired identity; debug admission under LOG_FILTER=debug if it still fails
