...
1. **Admission criterion — "impossible demos."** A clip earns its place only if it
   produces a _wait, what?_: an offline edit that reconverges, an agent running at
...
2. **Hero-cut spine — narrative.** Follow Bramble Coffee Roasters through a working
   ...
   EDGE are revealed as the _explanation_ of why it works, never as a feature tour.
...
Credits earn a _moment_, not a logo card. Each gets one clip where the technology is
the point:
...
Chosen so the five _are_ the Cloudflare cut in miniature and simultaneously the
...
is deliberately _not_ shown). Grounded in `about-bramble.md`, which stays canonical for
every world-fact.
