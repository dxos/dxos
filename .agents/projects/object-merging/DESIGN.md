...
- **2026-08-31 (§4.11 ratified — Josiah: "that's the filter.everything
  replacement … Yes build it" — and implemented)** — The worker rewrites
  references, index-driven; the client merge surface is gone:
  1. **`ConvergenceKeyMerger` gains `queryReferrers`** (reverse-ref index joined
     to the object metadata) and repoints each indexed `EncodedReference` at the
     chain-resolved winner — after a merge's tombstones, and on every
     `#foldRedirected` service pass, which is what re-covers late referrers.
     Best-effort by design: failures log and skip, no flush is ordered, and a ref
     left behind still resolves through the redirect.
  2. **`db.mergeDuplicates()` and the client executor are deleted** — the last
     `Filter.everything()` with them. `merge-executor.ts`'s other exports
     (`resolveMerged`, `getMergedFrom`, `foldLateEdits`, `isMergedAway`) had no
     production consumers; tests observe the same state via `getObjectCore`.
  3. **The merge core moved host-side** (`echo-host/src/db-host/merge-core.ts`,
     unit suite alongside); `@dxos/echo/internal` keeps only
     `resolveMergeRedirect` — settling the reviewer's layering question.
  4. **The e2e suites re-targeted the worker path**: rewrites observed via
     `waitForCondition` on ref URIs; partial-view merges staged through cores;
     the freshness residual pinned by a late-referrer test. The
     merged-away-referrer skip rule is asserted at the unit level (worker test),
     where servicing order is controllable.
     See §4.11's implementation notes for the mechanics.
...
5. **Rewrite inbound references opportunistically** — _partially implemented_:
   data-ref rewriting (including refs nested in arrays and records) runs in the
   worker off the reverse-ref index (§4.11), and never writes to a
...
- Peer A sees duplicates `{X, Y}` (winner X); peer B sees `{X, Y, Z}` with
  ...
  every service pass over a redirected loser also re-rewrites its inbound refs to
  the chain-resolved live end (§4.11), so refs follow the chain's collapse too.
...
### 4.11 Index-driven reference rewriting — replace the `Filter.everything()` pass (RATIFIED & IMPLEMENTED 2026-08-31)
...
**Implementation notes (2026-08-31, hardened same day after a three-model
adversarial review):** landed as designed, with these specifics:

- `queryReferrers` is `IndexEngine.queryReferrers` → `ReverseRefIndex`'s SQL join
  of `reverseRef` to `objectMeta` (discharging that module's standing join TODO),
  returning rows grouped per referrer with unescaped prop paths. Only same-space
  rows with a `documentId` survive the `WHERE` (queue entities have no document to
  rewrite; a qualified ref from another space is not this merge's to repoint).
  `lookupByRecordIds` is chunked while at it — a widely-referenced object could
  otherwise blow `SQLITE_LIMIT_VARIABLE_NUMBER` and silently disable rewriting for
  exactly the hottest targets.
- **Only losers whose tombstone actually applied get their referrers repointed.**
  A loser re-keyed, deleted, or concurrently redirected during the winner flush is
  declined by the tombstone callback and carries no redirect of ours — rewriting
  its referrers would sever links to a live entity with no redirect to resolve
  back through. The callback records what it wrote; the rewrite runs off that set.
- The whole per-referrer rewrite body — the write included — is log-and-swallow: a
  throw escaping it would mark the key un-serviced and wedge the durable intent on
  permanent retry, which a non-load-bearing optimization must never do. Referrer
  loads pass `fetchFromNetwork: false` plus a timeout, so a stale index row can
  never stall the indexing loop, and a referrer named by several losers of one
  group is loaded and written once (batched per document).
- The rewrite preserves the reference's own spelling: a local `echo:///<id>` stays
  local, a space-qualified `echo://<space>/<id>` stays qualified.
- `#foldRedirected` rewrites on **every** service pass, including one with nothing
  to fold — a loser re-indexing is the only signal that re-presents the key, so
  that is where late referrers (point 4's residual) catch up. Steady state is
  cheap: a rewritten referrer re-indexes with rows naming the winner, so the
  loser's lookup comes back empty and no documents are loaded.
- The merge core now lives in `echo-host/src/db-host/merge-core.ts` with its unit
  suite — trimmed to what production calls (`MergeCandidate`, `MergeResult`,
  `mergeCandidates`; the client-object helpers `toMergeCandidate` /
  `findMergeDuplicates` had test-only consumers and were deleted rather than
  re-published on the worker package). `@dxos/echo/internal`'s `merge.ts` keeps
  only `resolveMergeRedirect` (consumed by the client's resolution and
  transitive-deletion walks), with its unit tests beside it.
- The e2e suites observe the worker's rewrites (`waitForCondition` on the ref URI)
  and stage partial-view merges through `getObjectCore` exactly as a peer's worker
  would write them; the residual is pinned by a test that adds a referrer _after_
  convergence and asserts resolution through the redirect.
