...
Run against the whole `:build`/`:bundle` graph rather than one package. Paths
inside the shell scripts are absolute to a session scratchpad — edit `SP` before
reuse.

- `parse-report.mjs` — one `runReport.json` → hydration / hash / upload /
  `task-execution` totals, with `--per-task` and `--dump=<file>`. Counts
  `task-execution`, which F2's taxonomy missed (F8).
- `reps.sh <tag> <reps> [MOON_REMOTE_HOST] [target]` — N fully-hydrated reps
  against one arm, cold local cache each. Omit the host to use `workspace.yml`
  (the Depot arm).
- `sweep.sh` — drives `delay-proxy.mjs` across `RTTS`, N reps each.
- `analyze-ab.mjs <dir>` — per-rep table plus median / p90 / range / CV per arm.
- `analyze-sweep.mjs <dir>` — medians per RTT and the least-squares slope.
- `latency-sweep-324.txt`, `loopback-reps-324.txt` — raw output behind F9/F11.
