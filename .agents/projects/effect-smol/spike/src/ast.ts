...
//
// Effect 4 port of packages/common/effect/src/internal/ast.ts.
//
...
/**
 * v4 marks 129 of `SchemaAST`'s 138 exports `@internal` — including `annotate`,
 * `isMutable` and `replaceContext` — so node-level edits have to be expressed through
 * the `Schema` wrapper, which is public and typed.
 */
export const annotateAst = (ast: SchemaAST.AST, annotations: Schema.Annotations.Annotations): SchemaAST.AST =>
  Schema.make<Schema.Top>(ast).annotate(annotations).ast;

/** Public stand-in for the internal `SchemaAST.isMutable`. */
 ...
 */
export const findProperty = (schema: Schema.Top, path: string): SchemaAST.AST | undefined => {
...
/**
 * Annotating a node that carries checks attaches to the last check, not the node,
 * so every read must go through the resolver or annotations on refined types vanish.
 */
export const resolveAnnotations = (ast: SchemaAST.AST): Schema.Annotations.Annotations | undefined =>
