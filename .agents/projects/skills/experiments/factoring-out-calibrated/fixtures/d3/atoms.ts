...
export const parentAtom = <T extends Unknown>(snapshot: Snapshot<T>) =>
  Atom.make((get) => {
    const db = getDatabase(snapshot as any);
    return db ? (getSnapshot(db.getObjectById((snapshot as any).id)!) as unknown as Snapshot<T>) : undefined;
  });
