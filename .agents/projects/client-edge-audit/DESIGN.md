...
| ------------------------------------------------------------------------------------------ | ----------------------------------------------- | --------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [ip-data.ts:59](packages/sdk/observability/src/providers/ip-data.ts)                       | `https://api.ipdata.co`                         | `DX_IPDATA_API_KEY` present             | **Runs before/independent of the `disabled` check** (still open). Pre-fix it also `log.warn`ed at every boot when the key was absent — downgraded to debug in this PR |
