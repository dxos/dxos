...
**The teardown is a chain, not a set of slices.** `@dxos/protocols/proto/*` is generated by
`protobuf-compiler` and its output imports `@dxos/codec-protobuf` directly (40 files, for `Schema`,
`decompressSchema` and `RequestOptions`). So removing protobuf.js means, strictly in order: move the
263 consumer files off the bindings, delete the bindings, delete the two packages, drop the catalog
pin. Group 2 (`codec-protobuf`'s own 14 direct consumers) is the one thing that runs in parallel.

A revision of this plan claimed the 263-file barrel sweep was separable from killing protobuf.js.
It is not — it is the critical path, and treating it as optional understates the work by an order of
magnitude. Full chain and per-group scope: "The teardown: one dependency chain" in the audit.
