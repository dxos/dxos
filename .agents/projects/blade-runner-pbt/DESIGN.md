...
```
┌──────────────────────── orchestrator (node) ─────────────────────────┐
│ EdgePbt implements TestPlan<EdgePbtSpec, EdgePbtResult>              │
│   fc.assert(fc.asyncProperty(fc.commands([...]), run), { seed, ... })│
...
- **New spec** `src/spec/edge-pbt.ts`, registered in `main.ts` as `edgePbt`.
...
- fast-check runs **in the orchestrator**; commands are `fc.AsyncCommand<Model, Real>` whose
  `check(model)` is pure (fast-check requires purity there) and whose `run(model, real)` mutates
  the model and drives replicants over RPC.
...
- **Seed** comes from one CLI flag (`--seed`; today `GlobalOptions.randomSeed` is always
  ...
  behavior) and `fc.assert(..., { seed })`. The seed fixes the **generated command sequence**; it
...
- **Preview (G2/G3):** `https://preview.dxos.network`. Every run stamps what it creates:
  space labels and identity display names carry `pbt-<runId>`, and the SDK's
...
CI secret required at all), **path B as a periodic sweeper** keyed on the `pbt-` tag for runs that
...
New `.github/workflows/nightly-pbt.yml` — this repo has no nightly test workflow today (only
...
- Steps: build blade-runner → run `edgePbt` with the nightly spec and a random seed →
