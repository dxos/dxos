# blade-runner-pbt — Phase 1/2 results (local EDGE)
...
- **Reproducibility is proven** (§2) — the point of the exercise.
...
- **`fc.assert` cannot drive this.** With `numRuns: 1` it biases to the smallest input and drew
  **2 commands** against a `maxCommands` of 25 (measured), so nothing was exercised. The plan now
  draws `sampleDraws` sequences with `fc.sample` seeded from `--seed` and executes the longest.
