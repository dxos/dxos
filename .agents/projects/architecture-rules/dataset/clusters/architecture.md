...
- one-mechanism-per-concern: 99 comments tagged with this seed across 60 PRs in this slice; the four clusters it names hold 140 comments between them, since they also absorb comments the classifier left untagged. Keep — by far the best-supported seed, but it's carrying at least four distinguishable sub-rules in practice (no-parallel-mechanism, delete-dead-code-after-migration, canonical-api-surface, business-logic-out-of-ui); consider shipping those as separate rules with sharper flag/do-not-flag boundaries rather than one broad rule, since "reuse X" reads very differently for a service than for an import path.
