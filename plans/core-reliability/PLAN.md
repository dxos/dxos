...
Tests go in [packages/core/ai/src/resolvers/ChatCompletionsAdapter.test.ts](../../packages/core/ai/src/resolvers/ChatCompletionsAdapter.test.ts). Existing tests hit real Ollama/LM Studio; they carry vitest's native `tags: ['manual']` option so they only run when `--tagsFilter=llm` is passed. We'll add tests that use an in-process mock `HttpClient` layer so they run in CI.
