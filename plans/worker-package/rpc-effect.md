...
**Status:** Phase A implemented (uncommitted); tests and downstream build not green yet. Phases B–D not started.
...
- [ ] **Fix suite 1 in `effect-rpc.test.ts`** — 8/10 tests fail with `RpcClosedError` / wrong error types after `MessageChannel` switch. Likely causes to investigate:
  - `MessagePort.start()` not called on one or both ports (Node/vitest).
  - Runner `ManagedRuntime` lifecycle vs test `onTestFinished` ordering.
  - Native protocol has no Ping/Pong handshake — connect timing may differ from old `createLinkedPorts` RpcPort path.
- [ ] Add streaming round-trip test over `ClientRpcServer` + `makeClientServicesRpc` (QueryService/DataService-style).
- [ ] Re-bridge devtools extension (Worker protocol over `window.postMessage`) or document as follow-up.
- [ ] Commit Phase A once tests green.
