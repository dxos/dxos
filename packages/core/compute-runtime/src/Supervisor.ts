 ...
 *
 * Non-blocking: returns the child's process id immediately without awaiting its output. This is
 * the supervisor primitive — unlike `Operation.invoke` (which awaits the child and blocks the
...
/**
 * Reads the result of a finished child process. Intended for use inside `onChildEvent`, where
 * `Process.ChildEvent` carries only the exit signal, not the child's output value. A child
 * failure is returned as a failed `Exit` (the caller decides how to handle it); a missing
 * process is an invariant violation (the child just reported its exit) and dies.
 */
