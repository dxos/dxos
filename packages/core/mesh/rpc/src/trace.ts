...
export class PortTracer {
  ...
  constructor(private readonly _wrappedPort: RpcPort) {
    this._port = {
      send: (msg: Uint8Array) => {
        this.message.emit(create(MessageTraceSchema, { direction: MessageTrace_Direction.OUTGOING, data: msg }));

        return this._wrappedPort.send(msg);
      },
      subscribe: (cb: (msg: Uint8Array) => void) => {
        return this._wrappedPort.subscribe((msg) => {
          this.message.emit(create(MessageTraceSchema, { direction: MessageTrace_Direction.INCOMING, data: msg }));
          cb(msg);
        });
      },
    };
