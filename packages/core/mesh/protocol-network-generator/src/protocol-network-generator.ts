...
export class ProtocolNetworkGenerator extends EventEmitter {
  ...
  constructor(createPeer: CreatePeerCallback) {
    ...
    assert(
      ...
      'createPeer is required and must be a function'
    );
    this._createPeer = (...args) => createPeer(...args);
    TOPOLOGIES.forEach((topology) => {
      this[topology] = async (options: GenerateOptions) =>
        this._generate(topology, options);
    });
  }
  ...
  async _generate(
    ...
    const generator = new NetworkGenerator({
      createPeer: async (id) => {
        ...
        assert(
          typeof peer.createStream === 'function',
          'peer.createStream is required and must be a function'
        );
        return peer;
      },
