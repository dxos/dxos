...
const genericTest = async (network: Network) => {
  ...
  expect(conn1.stream).to.be.instanceOf(PassThrough);
  expect(conn1.fromPeer).to.have.property('name');
  expect(conn1.toPeer).to.have.property('name');

  let connectionLength = network.connections.length;
  await network.deleteConnection(conn1.fromPeer.id, conn1.toPeer.id);
  await waitForExpect(async () => {
    expect(network.connections.length).to.be.lessThan(connectionLength);
  });
