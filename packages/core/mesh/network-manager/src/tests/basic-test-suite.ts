...
export const basicTestSuite = (testBuilder: TestBuilder, runTests = true) => {
  ...
  test('going offline and back online', { timeout: 2_000 }, async () => {
    ...
    const peerLeft = peer2._signalManager.swarmEvent.waitFor(
      (event) => event.event.case === 'peerLeft' && peer1.peerId.equals(event.event.value.peer!.peerKey),
    );
    ...
    await expect
      .poll(() => !!peer2._networkManager.getSwarm(topic)!._peers.get(create(PeerSchema, { peerKey: peer1.peerId.toHex() }))?.advertizing, {
        timeout: 1_000,
      })
      .toBe(false);
    ...
    await expect
      .poll(() => peer1._networkManager.getSwarm(topic)?._peers.get(create(PeerSchema, { peerKey: peer2.peerId.toHex() }))?.advertizing, {
        timeout: 2_000,
      })
      .toBe(true);
    await expect
