...
describe('Memory transport', () => {
  ...
  test.skip('load', async () => {
    ...
    const pairs = await Promise.all(range(NUM_PAIRS).map(async () => {
    ...
    for (let i = 0; i < NUM_ROUNDS; i++) {
      ...
      await Promise.all(pairs.map(async ({ swarm1, swarm2 }) => {
        await exchangeMessages(swarm1, swarm2, message);
      }));
    }

    await Promise.all(pairs.map(async ({ peer1, peer2, topic }) => {
