...
export const bindDataChannel = (
  ...
  let disposed = false;
  // Frames are converted one after another, so a blob's asynchronous read cannot let a later frame land first.
  ...
  // Bytes arrive as ArrayBuffers, which convert synchronously, rather than as the browser's default blobs.
