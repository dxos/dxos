...
import AlarmBlueprint from './blueprint';
...
const TestLayer = AssistantTestLayer({
  ...
  blueprints: [AlarmBlueprint.make()],
  disableLlmMemoization: true,
});

describe('Alarm blueprint', () => {
  ...
  describe('set-alarm operation (Tier B)', () => {
    it.scoped(
      ...
      Effect.fnUntraced(
        function* ({ expect }) {
          const agent = yield* AgentService.createSession({ blueprints: [AlarmBlueprint.make()] });
