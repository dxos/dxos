...
`Routine` and `Agent` are two ECHO types that today live in different packages, model overlapping concerns, and were introduced for different reasons. They share the notion of a _named, instructed unit of LLM-driven work_ with `skills` and `context`, but `Agent` adds runtime state (chat, plan, queue, scheduling) while `Routine` keeps the static specification of a callable unit (typed I/O, template instructions).
...
Schema.Struct({
  ...
  skills: Schema.Array(Ref.Ref(Skill)),
...
**Conceptual role**: a _declarative specification_ of an LLM task — what it consumes, what it produces, what skills it may invoke, and the natural-language template that drives it. Roughly analogous to a function signature plus body.
...
- `Agent.makeInitialized(props, skill)` — Effect-flavored factory that allocates a `Feed`, a `Chat`, an input `Queue`, an `AiContextBinder` binding `skills + objects`, and a `Chat.CompanionTo` relation back to the Agent.
- `Agent.resetChatHistory(agent)` — rebuilds chat/feed while preserving the existing context binder's skills and objects.
...
- **`Skill`** (compute) — the unit both types reference.
...
| Skills                | `skills: Ref<Skill>[]` (schema field)    | bound at runtime via `AiContextBinder.bind({ skills })`   | Routine declares them statically; Agent stores them in its context binder, not in the schema.                                 |
...
2. **Convergent siblings**. Both are "named, instructed, skill-aware units of LLM work" — Routine is the _one-shot tool-call_ shape, Agent is the _long-running subscriber_ shape. The overlap (`name`, `instructions`, `skills`, `context`) is a shared base; the differences are runtime extensions.
...
### 4.2 Two locations for `skills` + `context`
...
Practical consequences: you cannot statically inspect "what skills does this Agent use?" by reading its ECHO object — you must load the binder.
...
Add `routine: Ref<Routine>` (or `routines: Ref<Routine>[]`) to `Agent`, and drop `instructions`/`skills` accumulation from Agent in favor of resolving them via the Routine.
...
Factor out a base schema (e.g. `Instructable`) containing `name`, `description`, `instructions`, `skills`, `artifacts`/`context`. Make both `Routine` and `Agent` extend it; Agent adds `chat`, `plan`, `queue`, `subscriptions`, `filterEvents`, `cron`, `enabled`; Routine adds `input`, `output`.
...
3. Promote `skills` onto Agent's schema (or remove from Routine).
...
1. **Tighten first** — independently of unification, fix the cross-cutting inconsistencies: `instructions` representation, `context`/`artifacts` shape, skill location.
2. **Then compose** — make `Agent.routine: Ref<Routine>` the canonical way an Agent gets its instructions/skills/I/O contract, with `instructions` on Agent kept transitionally for migration.
...
2. Should `skills` live on the ECHO object or stay in the `AiContextBinder` feed? Inspectability vs. binder-as-source-of-truth.
...
- `core/compute/assistant-toolkit` (skills/agent + agent-wizard functions) — ~8
