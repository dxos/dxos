...
// The project skill's own workflow, end to end: a rough outline becomes a task ledger under a
// milestone, with one item closed. Unlike `projects.eval.ts` (which grades a single artifact-filing
// verb), this drives the `tasks-*` verbs the skill names as plain strings — the ones that reach
// nothing at all unless plugin-projects and plugin-tasks are registered, since `resolveToolkit`
// drops an unresolvable ToolId with only a log line.
...
const REQUIRED_TOOLS = ['tasks-get-outline', 'tasks-create', 'tasks-create-milestone'];
...
const task = createEvalRunner({
  ...
  // Without these the skill's `tasks-*`/`projects-*` ToolIds resolve to nothing and are dropped.
  ...
  // Reading the outline, four creates, a milestone, four files and a completion is a long chain.
  timeout: 240_000,
  seed: ({ instructions }) =>
    Effect.gen(function* () {
  ...
  dbQuery: () =>
    Effect.gen(function* () {
      ...
      if (!taskSet) {
        return { ...trace, matchedTasks: 0, filedUnderMilestone: 0, expected: PLAN.length, closedTheRightOne: false };
      }
...
evalite('Projects — a project chat turns its outline into a task ledger', {
  ...
  scorers: [
    ...
    },
    {
      ...
      description: 'The outline/task/milestone verbs were called, and none returned an error.',
      scorer: ({ output }) =>
