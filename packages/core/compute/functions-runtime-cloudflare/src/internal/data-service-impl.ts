...
export class DataServiceImpl implements DataService.Handlers {
  ...
  // Clients here hold Automerge replicas: the functions runtime reads commit blobs, not tab documents.
  ...
  ['DataService.subscribeProxy'](
    ...
    return EffectStream.fail(new NotImplementedError({ message: 'Tab documents are not served in the EDGE runtime.' }));
  }

  ['DataService.updateProxySubscription'](
    _request: DataService.UpdateProxySubscriptionRequest,
  ): Effect.Effect<void, BaseError> {
    return Effect.fail(new NotImplementedError({ message: 'Tab documents are not served in the EDGE runtime.' }));
  }
