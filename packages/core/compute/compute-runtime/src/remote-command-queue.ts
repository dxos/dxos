...
export class RemoteCommandQueue {
  ...
  /** Records a failed delivery attempt, which the flusher's backoff is a function of. */
  recordAttempt(id: string): Effect.Effect<number> {
    return this.#modify((commands) => {
      const next = commands.map((entry) => (entry.id === id ? { ...entry, attempts: entry.attempts + 1 } : entry));
      return [next, next.find((entry) => entry.id === id)?.attempts ?? 0];
    });
  }
