 ...
 *
 * Space is a parameter of every verb, not of the implementation: processes are per-space on the host
 * (the routes are `/processes/:spaceId/...`), but one manager serves all of a client's spaces, so a
 * stack does not need a manager instance per space. Ids are strings to keep this interface free of
 * `@dxos/keys` value imports.
 *
 * Effects carry no error channel, matching the local manager: transport and host failures surface as
 * defects.
 */
export interface Control {
  /** Spawn one of the host's built-in processes in `space`. */
  spawn(space: string, request: ProcessProtocol.SpawnProcessRequest): Effect.Effect<ProcessProtocol.ProcessInfo>;

  list(
    space: string,
    query?: ProcessProtocol.ListProcessesQuery,
  ): Effect.Effect<readonly ProcessProtocol.ProcessInfo[]>;

  status(space: string, pid: Process.ID): Effect.Effect<ProcessProtocol.ProcessInfo>;

  /** Submit an input already encoded via the process definition's input schema. */
  submitInput(space: string, pid: Process.ID, input: unknown): Effect.Effect<void>;
  ...
  terminate(space: string, pid: Process.ID): Effect.Effect<void>;
   ...
   */
  readEvents(space: string, pid: Process.ID, cursor: number): Effect.Effect<ProcessProtocol.ProcessEventsResponse>;
}
