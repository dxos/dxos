...
describe('ComputeGraph', () => {
  ...
  test('map functions', async () => {
    ...
    expect(id).to.eq(`${Obj.getEchoId(functionObject)}()`);
  ...
  test('cross-node references', async () => {
...

    {
      ...
      const value1 = node1.graph.hf.getCellValue({
        sheet: node1.sheetId,
        col: 3,
        row: 0,
      });
      const value2 = node2.graph.hf.getCellValue({
        sheet: node2.sheetId,
        col: 0,
        row: 0,
      });
    ...
    node2.update.on(({ change }) => {
      const value = node2.graph.hf.getCellValue({
        sheet: node2.sheetId,
        col: 0,
        row: 0,
      });
