...
const Trigger_ = Schema.Struct({
	...
	function: Schema.optional(
		Type.Ref(Type.Expando).annotations({ title: "Function" }),
	),
	 ...
	 */
	inputNodeId: Schema.optional(
		Schema.String.annotations({ title: "Input Node ID" }),
	),
	...
	input: Schema.optional(
		Schema.mutable(Schema.Record({ key: Schema.String, value: Schema.Any })),
	),
}).pipe(
	Type.Obj({
...
export interface TriggerEncoded
	extends Schema.Schema.Encoded<typeof Trigger_> {}
export const Trigger: Schema.Schema<Trigger, TriggerEncoded> = Trigger_;

export const make = (props: Obj.MakeProps<typeof Trigger>) =>
	Obj.make(Trigger, props);
