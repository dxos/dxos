...
export const callTool: <Tools extends Record<string, Tool.Any>>(
	...
	AiError.AiError,
	Tool.Requirements<Tools>
> = Effect.fn("callTool")(function* (toolkit, toolCall) {
	...
	const toolResult = yield* toolkit
		.handle(toolCall.name as any, input as any)
		.pipe(
			Effect.map(
				({ result }) =>
					({
	...
	log("toolResult", {
		...
		...{
			...
			result:
				"result" in toolResult ? safeParseJson(toolResult.result) : undefined,
		},
	});
