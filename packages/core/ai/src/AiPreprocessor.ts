...
const convertAssistantMessagePart: (
  ...
  function* (block) {
    ...
    switch (block._tag) {
      ...
      case 'summary':
        return Prompt.makePart('text', {
          text: `This is the continuation of a conversation that was compacted to preserve context-window space. Summary of what happend in this conversation previously: <summary>${block.content}</summary>`,
        });
      case 'toolkit':
        return Prompt.makePart('text', {
          text: '<toolkit/>',
