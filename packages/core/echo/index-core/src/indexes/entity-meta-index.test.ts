...
describe('EntityMetaIndex', () => {
  ...
  it.effect('should compute grouped aggregates over the meta index', () =>
    Effect.gen(function* () {
      const index = new EntityMetaIndex();
      yield* index.migrate();

      const spaceId = SpaceId.random();
      const queueIdA1 = EntityId.random();
      const queueIdA2 = EntityId.random();
      const queueIdB = EntityId.random();
      const objectIdA1 = EntityId.random();
      const objectIdA2 = EntityId.random();
      const objectIdB = EntityId.random();

      const earlyTimestamp = 1700000000000;
      const sameHourTimestamp = earlyTimestamp + 60_000;
      const nextHourTimestamp = earlyTimestamp + 3_600_000;
      ...
      yield* index.update([
        {
          ...
          createdAt: null,
          updatedAt: earlyTimestamp,
          data: { id: objectIdA1, [ATTR_TYPE]: TYPE_PERSON, [ATTR_DELETED]: false },
        },
        {
          spaceId,
          queueId: queueIdA2,
          ...
          recordId: null,
          createdAt: null,
          updatedAt: sameHourTimestamp,
          data: { id: objectIdA2, [ATTR_TYPE]: TYPE_PERSON, [ATTR_DELETED]: false },
        },
        {
          spaceId,
          queueId: queueIdB,
          ...
          recordId: null,
          createdAt: null,
          updatedAt: nextHourTimestamp,
          data: { id: objectIdB, [ATTR_TYPE]: TYPE_RELATION, [ATTR_DELETED]: false },
        },
      ]);

      // (a) Grouped by type: two A's, one B, ordered by typeDXN.
      const byType = yield* index.queryAggregate({
        spaceIds: [spaceId],
        includeAllQueues: true,
        groupBy: [{ kind: 'type' }],
      });
      expect(byType.map((row) => ({ key: row.key, count: row.count }))).toEqual([
        { key: [TYPE_PERSON], count: 2 },
        { key: [TYPE_RELATION], count: 1 },
      ]);

      // (b) Grouped by hour bucket: the first two rows share an hour, the third falls in the next.
      const byHour = yield* index.queryAggregate({
        spaceIds: [spaceId],
        includeAllQueues: true,
        groupBy: [{ kind: 'bucket', field: 'updatedAt', unit: 'hour' }],
      });
      expect(byHour.map((row) => ({ key: row.key, count: row.count }))).toEqual([
        { key: [Math.floor(earlyTimestamp / 3_600_000)], count: 2 },
        { key: [Math.floor(nextHourTimestamp / 3_600_000)], count: 1 },
      ]);

      // (c) Deleting the B row: excluded by default, still counted with `deleted: 'include'`.
      yield* index.update([
        {
          spaceId,
          queueId: queueIdB,
          ...
          createdAt: null,
          updatedAt: nextHourTimestamp,
          data: { id: objectIdB, [ATTR_TYPE]: TYPE_RELATION, [ATTR_DELETED]: true },
        },
      ]);

      const excludingDeleted = yield* index.queryAggregate({
        spaceIds: [spaceId],
        includeAllQueues: true,
        groupBy: [{ kind: 'type' }],
      });
      expect(excludingDeleted.map((row) => ({ key: row.key, count: row.count }))).toEqual([
        { key: [TYPE_PERSON], count: 2 },
      ]);

      const includingDeleted = yield* index.queryAggregate({
        spaceIds: [spaceId],
        includeAllQueues: true,
        groupBy: [{ kind: 'type' }],
        deleted: 'include',
      });
      expect(includingDeleted.map((row) => ({ key: row.key, count: row.count }))).toEqual([
        { key: [TYPE_PERSON], count: 2 },
        { key: [TYPE_RELATION], count: 1 },
      ]);
    }).pipe(Effect.provide(TestLayer)),
