...
import * as EchoError from './Error';
...
export interface Database extends Queryable {
   ...
   * Rejects with `EchoError.BlobTooLargeError` (over inline storage's fixed cap, or the backend's own
   * `maxSize`), `EchoError.BlobWriteError` (backend upload failure), or `EchoError.BlobNotAvailableError`
  ...
  /**
   * Loads a blob's bytes. Rejects with `EchoError.BlobNotAvailableError` if the backend for the blob's
...
export const resolve: {
  ...
  <S extends Type.AnyEntity>(
    ref: URI.URI | Ref<any>,
    schema: S,
  ): Effect.Effect<Type.InstanceType<S>, EchoError.EntityNotFoundError, Service>;
} = (<S extends Type.AnyEntity>(
  ref: URI.URI | Ref<any>,
  schema?: S,
): Effect.Effect<Type.InstanceType<S>, EchoError.EntityNotFoundError, Service> =>
  Effect.gen(function* () {
    ...
    if (!object) {
      return yield* Effect.fail(new EchoError.EntityNotFoundError(dxn));
    }
 ...
 *
 */
export const load: <T>(ref: Ref<T>) => Effect.Effect<T, EchoError.EntityNotFoundError, never> = Effect.fn(
  'Database.load',
)(function* (ref) {
  const object = yield* Effect.promise(() => ref.tryLoad());
  if (!object) {
    return yield* Effect.fail(new EchoError.EntityNotFoundError(ref.uri));
  }
  return object;
});
