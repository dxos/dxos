...
export type SubscribeOptions = {
   ...
   * `latestOnly` atom additionally recomputes its value under {@link withLatestRead}.
   */
  latestOnly?: boolean;
};
 ...
 * @param opts - Subscription options; `latestOnly` uses the latest channel (insulated from scrubbing).
...
export const subscribe = (obj: unknown, callback: () => void, opts?: SubscribeOptions): (() => void) => {
  ...
  const channel = opts?.latestOnly ? LatestEventId : EventId;
  if (target && channel in target) {
    return (target as any)[channel].on(callback);
  }
  return () => {};
};
