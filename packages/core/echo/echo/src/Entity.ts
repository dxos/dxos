 ...
 * carrying the same natural key are the same entity and eventually converge to one. It sits
 * alongside the entity's id, which is a surrogate — system-minted, random, and meaningless
 * outside the database.
 */
export const getNaturalKey = (entity: Unknown | Snapshot): string | undefined => getMeta(entity).naturalKey;
 ...
 */
export const setNaturalKey = (entity: Unknown, naturalKey: string | undefined): void => {
  ...
  update(entity, (entity) => {
    getMeta(entity).naturalKey = naturalKey;
  });
};
