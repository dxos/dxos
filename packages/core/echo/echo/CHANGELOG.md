...
- a92ea18: `ActionMenu` takes a `deferUntilOpen` prop, which renders the trigger alone until its first click
...
- f82c78f: Files can now be created from bytes uploaded directly to blob storage, without the bytes passing through the caller. `Blob.fromUpload(uploadId)` adopts an upload that a third party has already written to the store over a signed URL, and reports the size and content type the store observed rather than trusting the caller — nothing in the client process ever sees the bytes. Blob backends opt in by implementing `BlobBackend.adoptUpload`; the hosted edge backend does, via the new `BlobTransport.finalizeUpload`. Inline storage rejects the call, since it keeps bytes on the ECHO object itself and has nowhere for an upload to have gone.
...
- 0cde959: Reading a property on an ECHO object is roughly an order of magnitude cheaper. A record now carries its
...
- 915db6a: An edge-hosted agent session can complete a turn, call a tool, and wake itself.
...
- eeff74c: The pull request review composer now floats at the diff line it addresses instead of rendering as a
...
- b00ee72: Objects now have a **mnemonic**: the last 6 characters of their id, uppercased — short enough for a person to read out or type, and stable for the life of the object.
...
- 17ed864: Remove the `query-lite` DSL mirror and the QuickJS query sandbox.
...
- 9a3f01e: `Hypergraph.Service` is the cross-space counterpart to `Database.Service`: query every space, or
...
- eda8b55: `installWasmMemoryProbe()` (`@dxos/util`) counts a realm's wasm linear memory — the bytes automerge,
...
- 0c6c186: Fix a self-scheduled alarm being dropped before the space index caught up.
...
- c95def4: Refuse an ask-question whose task title two tasks share.
...
- b1dc20c: Update `automerge` to 3.5.0.
- ac71815: Update `automerge-subduction` to 0.17.2, which propagates writes only to peers subscribed to a sedimentree and no longer settles delivery before the write resolves.
...
- 7c87626: Update Automerge to `automerge-repo` 2.6.0-subduction.48, `automerge` 3.3.2 and `automerge-subduction` 0.16.1.
- 63fc847: Complete the protobuf.js → buf migration and remove `@dxos/codec-protobuf` and
...
- b2caee6: Objects filed at the root of a space that had no root collection yet now show up in queries. `ContainerModel.add` created that root collection without persisting it. Filed objects then pointed at a parent that did not exist in the space, and queries dropped them.
...
- d0beedc: Diff chunks in a walkthrough (`diffBlocks` in `@dxos/ui-editor`) are highlighted in context: each
  ...
  `safeFetchBytes` in `@dxos/util` requests `redirect: 'manual'` instead of `'error'`, which Cloudflare
  Workers reject outright; redirects are still refused. This fixes `file.createFromSource` with an
  HTTPS source on the edge.
...
- 9ab38fa: Queries no longer pay an exception per candidate object. `EID.tryParse` and `DXN.tryMake` validate
...
- b3673ee: Pack a credential assertion's `type_url` as the bare type name.
...
- 872f391: `useFeedSyncState` no longer polls feed sync state while the document is hidden, and its default poll interval is now 15s — removing a per-space `peekPull` round-trip to edge every 5s for anyone with the devtools stats panel open.
- 51820a1: A `File`'s card shows the image or video it holds (a chat embed, a link preview), instead of a form of its properties. The assistant is told how to link (`[name](echo://…)`) and embed (`![name](echo://…)`) an object, with its URI in full; a block reference chip reads the object's name.
- 66e9264: Fix object-property atoms dropping a ref's contents. `snapshotForComparison` shallow-copied every object value, but a `Ref` carries `uri` and `target` as prototype getters over private fields, so the copy came back empty — `useObject(obj, refField)` handed consumers a ref with no URI. Change detection for a ref held directly now compares the URI and the inlined target instead of reporting every mutation of the owning object as a change.
- 84362af: `db.flush()` now has the host save only the documents this client wrote since its last flush, instead of every loaded document. The patched `@automerge/automerge-repo` looks documents up by id without re-validating the id, and compares saved heads without encoding them. Together these cut the client worker's allocation by about a quarter while bulk-creating objects.
- 76d6fca: ECHO's object snapshots move out of the full-text index into their own `ObjectSnapshotIndex`, an
...
- 72f7584: The index opens on Cloudflare Durable Object SQLite again. The SQLite version check added with the SQL query executor called `sqlite_version()`, which Durable Objects refuse. That failed every index migration and every query against the index. The check is now skipped where that function is denied.
- e94ed89: `IndexEngine` and the index stores behind it now take their SQL client as a constructor argument
...
- 851791f: `JsonSchema.toJsonSchema` emits a named schema reached through several properties as one `$defs` entry instead of one copy per occurrence (`node`, `node_1`, ...), which shrinks generated operation and tool schemas.
...
- a283607: `AutomergeHost` no longer keeps the 256 most recently released documents loaded regardless of age: a released document is evicted after the 30-second idle delay, and later documents reuse its Automerge memory. Hosts can still set a floor through the `residency` option.
...
- 690dcaa: Releasing a query subscription twice no longer stops the query for every other subscriber: the
...
- 9205bd3: Studio and adjacent plugins.
...
- fce2060: Recover from an edge subduction session loss by re-running the handshake on the existing connection instead of tearing it down, so the peer id and the collection sync state survive and a routine edge restart no longer costs a full re-announce of every document in the space.
- bda45ac: Syncing a large space no longer freezes the tab: the client integrates incoming documents, opens linked documents, and hydrates index results in short slices that yield to the event loop, the worker caps each batch it forwards, and db update events are coalesced to ten per second during a burst of synced documents while a local write still emits at once.
- 5885380: `SyntaxHighlighter` no longer re-tokenizes unchanged source on every parent render and builds its token elements without rescanning the whole stylesheet per token, and a feed re-read skips decoding and digesting objects whose block has already been applied.
- 693d1b4: `TaskList` renders a pull-request artifact as its `#number` pill, with an icon coloured by the PR's
  state, matching how a GitHub PR link looks in a task description.
