  ...
  private async _execSelectStep(
    ...
    switch (step.selector._tag) {
      case 'WildcardSelector': {
        ...
        const metas = await this._queryAllFromSqlIndex(ctx, spaces, allQueuesFromSpaces, queueIds);
        ...
        const items = await Promise.all(
          step.selector.objectIds.map((id) => {
            if (queues.length > 0) {
              ...
              return this._loadFromDXN(ctx, DXN.parse(queues[0]).extend([id]), { sourceSpaceId: spaceId });
            } else if (spaces.length > 0) {
              return this._loadFromDXN(ctx, DXN.fromLocalObjectId(id), { sourceSpaceId: spaces[0] });
            } else {
