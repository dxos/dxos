...
export type BufQueryServiceProps = {
  indexer: Indexer;
...
type ActiveQuery = {
  ...
  sendResults: (results: ProtobufJsQueryResult[]) => void;
...
@trace.resource()
export class BufQueryServiceImpl extends Resource implements BufRpcHandlers<typeof QueryService> {
  ...
  constructor(private readonly _params: BufQueryServiceProps) {
    super();
  ...
  private _createQuery(
    ...
    const queryEntry: ActiveQuery = {
      ...
      sendResults: (results) => {
        ...
        onResults(create(QueryResponseSchema, { queryId: request.queryId, results: bufResults }));
      },
      onError,
      close: async () => {
        onClose();
