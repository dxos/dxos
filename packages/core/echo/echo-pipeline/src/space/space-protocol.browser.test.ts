  ...
  test('two peers discover each other', async () => {
    ...
    await expect
      .poll(() =>
        presence1
          .getPeersOnline()
          .some(({ identityKey }) => identityKey && PublicKey.from(identityKey as any).equals(peer2.identityKey)),
      )
      .toBeTruthy();
    await expect
      .poll(() =>
        presence2
          .getPeersOnline()
          .some(({ identityKey }) => identityKey && PublicKey.from(identityKey as any).equals(peer1.identityKey)),
      )
      .toBeTruthy();
  });
