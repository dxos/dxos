...
describe.skip('loadObjectReferences', () => {
  test('loads a field', async () => {
    ...
    const restartedPeer = await testBuilder.createPeer({ storagePath: tmpPath });
  ...
  test('loads as array of non-nullable items', async () => {
    const Nested = Type.makeObject(DXN.make('com.example.nested', '0.1.0'))(Schema.Struct({ value: Schema.Number }));

    const TestSchema = Type.makeObject(DXN.make('com.example.test', '0.1.0'))(
      Schema.Struct({
    ...
    const db = await testPeer.createDatabase(spaceKey);
    db.graph.registry.add([TestSchema, Nested]);
    db.add(object);
    await db.flush();
    await testPeer.close();
