 ...
 * {@link createDocAccessor} and its label via `Obj.labelAtom` — resolve the value at `heads` until
 * cleared, and the default atom/subscription channels fire so the UI reflects the historical view.
 * `latestOnly` subscribers stay live, writes throw, and the full edit history is unaffected.
 *
