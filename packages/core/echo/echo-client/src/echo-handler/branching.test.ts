...
describe('branching persistence', () => {
  ...
  test('the current branch survives a reload (device-local)', async () => {
    ...
    await db2._entityManager.loadObjectCoreById(rootId);

    expect(db2.getCurrentBranch(rootId)).toBe('b1');
    expect(db2._entityManager.getObjectCoreById(rootId)?.getDecoded(['data', 'title'])).toBe('root-b1');
    expect(db2._entityManager.getObjectCoreById(childId)?.getDecoded(['data', 'content'])).toBe('child-b1');
  });
});
