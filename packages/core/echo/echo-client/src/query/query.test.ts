...
describe('Query', () => {
  ...
  describe('Timestamp queries', () => {
    ...
    test('timestamps survive peer reload', async () => {
      ...
      await initialDb.entityManager.updateIndexes();
      ...
      await db.flush();
      await db.entityManager.updateIndexes();
  ...
  test('query.run() queries everything after restart', async () => {
    ...
    let root: AutomergeUrl;
    {
      ...
      root = db.entityManager.getSpaceRootDocHandle().url!;
      await peer.close();
    }
  ...
  test('objects with incorrect document urls are ignored', async () => {
    ...
    let expectedObjectId: string;
    {
      ...
      const rootDocHandle = db.entityManager.getSpaceRootDocHandle();
