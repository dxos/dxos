...
const getFeedUriOrThrow = (feed: Feed.Feed): EID.EID => {
  const uri = Feed.getFeedUri(feed);
  invariant(uri, 'feed URI missing');
  return uri;
};
...
type ObjectProps = {
  [Obj.Meta]?: { tags?: Ref.Ref<unknown>[]; key?: string; version?: string };
  value?: number;
};
...
describe('Query', () => {
  ...
  describe('aggregate', () => {
    test('groups by a single property, with per-group counts', async () => {
      ...
      expect(byKey.get(100)?.items).to.have.length(2);
      expect(byKey.get(200)?.count).to.equal(1);
      expect(byKey.get(200)?.items).to.have.length(1);
    });

    test('aggregate-everything (no group entries) produces a single row', async () => {
      const { db } = await builder.createDatabase();
      db.add(Obj.make(TestSchema.Expando, { sentAt: 1000 }));
      db.add(Obj.make(TestSchema.Expando, { sentAt: 3000 }));
      db.add(Obj.make(TestSchema.Expando, { sentAt: 2000 }));
      await db.flush();

      const rows = await db
        .query(
          Query.select(Filter.everything()).aggregate({
            latest: Aggregate.max('sentAt'),
      ...
      expect(rows[0].latest).to.equal(3000);
      expect(rows[0].total).to.equal(3);
    });

    test('items limit caps the exposed members while count reflects the full total', async () => {
      const { db } = await builder.createDatabase();
      for (let i = 0; i < 5; i++) {
        db.add(Obj.make(TestSchema.Expando, { category: 'a', rank: i }));
      }
      db.add(Obj.make(TestSchema.Expando, { category: 'b', rank: 99 }));
      await db.flush();
      ...
      const byKey = new Map(groups.map((group) => [group.category, group]));
      // The 'a' group has 5 members but only the first 2 (by rank) are exposed; count stays 5.
      expect(byKey.get('a')?.count).to.equal(5);
      expect(byKey.get('a')?.items.map((obj) => obj.rank)).to.deep.equal([0, 1]);
    ...
    test('groups by multiple properties (composite key)', async () => {
      ...
      const aOneGroup = groups.find((group) => group.category === 'a' && group.value === 1);
    ...
    test('missing or non-scalar property values group together under the null key', async () => {
      ...
      expect(groups).to.have.length(2);
      const nullGroup = groups.find((group) => group.value === null);
    ...
    test('empty result set produces no groups', async () => {
      ...
      const groups = await db
        .query(Query.select(Filter.nothing()).aggregate({ value: Aggregate.group('value') }))
        .run();
      expect(groups).to.deep.equal([]);
    });

    test('orderBy before aggregate controls within-group order', async () => {
      const { db } = await builder.createDatabase();
      ...
      expect(groups).to.have.length(1);
      expect(groups[0].items.map((obj) => obj.rank)).to.deep.equal([1, 2, 3]);
    });

    test('orderBy the key property before aggregate yields key-ascending group order', async () => {
      const { db } = await builder.createDatabase();
      ...
      expect(groups.map((group) => group.category)).to.deep.equal(['a', 'b', 'c']);
    });

    test('a group field named differently from its property is orderable by that name', async () => {
      const { db } = await builder.createDatabase();
      db.add(Obj.make(TestSchema.Expando, { category: 'c' }));
      db.add(Obj.make(TestSchema.Expando, { category: 'a' }));
      db.add(Obj.make(TestSchema.Expando, { category: 'b' }));
      ...
      const groups = await db
        .query(
          Query.select(Filter.everything())
            .aggregate({ bucket: Aggregate.group('category') })
            .orderBy(Order.property('bucket', 'asc')),
        )
        .run();

      // The group field is named `bucket` (not `category`); ordering by that name reorders groups.
      expect(groups.map((group) => group.bucket)).to.deep.equal(['a', 'b', 'c']);
    });
      ...
      const groups = await db
        .query(
          Query.select(Filter.everything())
            ...
            .aggregate({ threadId: Aggregate.group('threadId'), items: Aggregate.items() }),
        )
        .run();

      // Groups ordered by each thread's most recent message: thread-b (4000), thread-a (2000), thread-c (1500).
      expect(groups.map((group) => group.threadId)).to.deep.equal(['thread-b', 'thread-a', 'thread-c']);
      // Messages within each group are newest-first.
      expect(groups[0].items.map((obj) => obj.sentAt)).to.deep.equal([4000, 3000]);
      expect(groups[1].items.map((obj) => obj.sentAt)).to.deep.equal([2000, 1000]);
      expect(groups[2].items.map((obj) => obj.sentAt)).to.deep.equal([1500]);
    });
    ...
    test('orders threads by a max aggregate, in both directions, independent of within-group order', async () => {
      ...
      expect(desc.map((group) => group.threadId)).to.deep.equal(['thread-b', 'thread-a', 'thread-c']);
      ...
      expect(asc.map((group) => group.threadId)).to.deep.equal(['thread-c', 'thread-a', 'thread-b']);
    ...
    test('a min aggregate exposes the earliest member and can order groups', async () => {
      ...
      const groups = await db
        .query(
          Query.select(Filter.everything())
            .orderBy(Order.property('sentAt', 'desc'))
            .aggregate({ threadId: Aggregate.group('threadId'), earliest: Aggregate.min('sentAt') })
            .orderBy(Order.property('earliest', 'asc')),
        )
        .run();

      // Ordered by each thread's earliest message: thread-a (1000) then thread-b (2000).
      expect(groups.map((group) => group.threadId)).to.deep.equal(['thread-a', 'thread-b']);
      expect(groups.map((group) => group.earliest)).to.deep.equal([1000, 2000]);
    });
    ...
    test('aggregate ordering pages over whole groups (limit + skip)', async () => {
      ...
      const page = await db
        .query(
          Query.select(Filter.everything())
            .orderBy(Order.property('sentAt', 'desc'))
            .aggregate({ threadId: Aggregate.group('threadId'), latest: Aggregate.max('sentAt') })
      ...
      expect(page.map((group) => group.threadId)).to.deep.equal(['c', 'd']);
    });

    test('limit applies to the flat stream before aggregating', async () => {
      const { db } = await builder.createDatabase();
      db.add(Obj.make(TestSchema.Expando, { category: 'a', rank: 1 }));
      db.add(Obj.make(TestSchema.Expando, { category: 'a', rank: 2 }));
      ...
      const groups = await db
        .query(
          Query.select(Filter.everything())
            .orderBy(Order.property('rank', 'asc'))
            .limit(2)
            .aggregate({ category: Aggregate.group('category'), count: Aggregate.count() }),
        )
        .run();

      // Only the first 2 (by rank) objects reach grouping: both are category 'a'.
      expect(groups).to.have.length(1);
      expect(groups[0].category).to.equal('a');
      expect(groups[0].count).to.equal(2);
    });

    test('limit + skip after aggregate pages over whole groups', async () => {
      const { db } = await builder.createDatabase();
      ...
      expect(page.map((group) => group.category)).to.deep.equal(['b', 'c']);
      expect(page.every((group) => group.count === 2 && group.items.length === 2)).to.be.true;
    });

    describe('reactivity', () => {
      // Aggregate queries are index-backed (like order/limit queries), so `.runSync()`/`.results`
      ...
      test('adding an object creates a new group / updates counts', async () => {
        ...
        expect(lastResult.find((group) => group.category === 'b')?.count).to.equal(1);
      ...
      test('property edit moves an object between groups, including the same-flat-index boundary case', async () => {
        ...
        expect(lastResult).to.have.length(1);
        expect(lastResult[0].category).to.equal('b');
      ...
      test('deleting the last object in a group removes the group', async () => {
        ...
        const query = db.query(Query.select(Filter.everything()).aggregate({ category: Aggregate.group('category') }));
        const initialResult = await subscribeAndWaitForFirstResult(query);
        expect(initialResult).to.have.length(2);
    ...
    test('run() matches subscribed results', async () => {
      ...
      expect(ran.map((group) => group.category).sort()).to.deep.equal(subscribed.map((group) => group.category).sort());
      expect(ran.map((group) => group.count).sort()).to.deep.equal(subscribed.map((group) => group.count).sort());
    });

    test('aggregate queries participate in the result/atom cache like ordinary queries', async () => {
      const { db } = await builder.createDatabase();

      const first = db.query(Query.select(Filter.everything()).aggregate({ category: Aggregate.group('category') }));
  ...
  describe('Filter.in subquery (semi-join)', () => {
    test('pulls in every object sharing a threadId with the subquery results, across feed + space scopes', async () => {
      ...
      const matches = Query.select(Filter.type(TestSchema.Task, { completed: true })).from(
        Scope.feed(getFeedUriOrThrow(feed)),
      );
    ...
    test('composes with aggregate: groups whole threads with accurate per-thread counts', async () => {
      ...
      const source = Query.select(Filter.type(TestSchema.Task, { title: Filter.in(matches.project('title')) }))
        .from(Scope.feed(getFeedUriOrThrow(feed)))
  ...
  describe('Timestamp queries', () => {
    ...
    test('Obj.getMeta(obj).updatedAt increases after Obj.update', async () => {
      ...
      invariant(before !== undefined, 'updatedAt missing');
    ...
    test('timestamps survive peer reload', async () => {
      ...
      invariant(reloaded, 'object missing after reload');
    ...
    test('Order.created ordering is consistent with Obj.getMeta(obj).createdAt', async ({ expect }) => {
      ...
      invariant(t1 !== undefined, 'createdAt missing');
      invariant(t2 !== undefined, 'createdAt missing');
      invariant(t3 !== undefined, 'createdAt missing');
    ...
    test('Order.updated ordering is consistent with Obj.getMeta(obj).updatedAt', async ({ expect }) => {
      ...
      invariant(upd1 !== undefined, 'updatedAt missing');
      invariant(upd2 !== undefined, 'updatedAt missing');
      invariant(upd3 !== undefined, 'updatedAt missing');
  ...
  describe('Queue queries', () => {
    test('typeURI: versionless matches any version', async () => {
      ...
      const both = await db
        .query(Query.select(Filter.type(DXN.make('com.example.type.person'))).from(Scope.feed(getFeedUriOrThrow(feed))))
        .run();
        ...
        const results = await graph
          .query(
            Query.select(Filter.type(TestSchema.Task)).from([
              ...
              Scope.feed(getFeedUriOrThrow(feed1)),
            ]),
          )
          .run();
        const titles = results.map((r: TestSchema.Task) => r.title).sort();
...

      {
        ...
        const results = await graph
          .query(
            Query.select(Filter.type(TestSchema.Task)).from([
              ...bothSpaces.map((spaceId) => Scope.space({ id: spaceId })),
              Scope.feed(getFeedUriOrThrow(feed2)),
            ]),
          )
          .run();
        const titles = results.map((r: TestSchema.Task) => r.title).sort();
  ...
  describe('from() clause', () => {
    ...
    test('Filter.type with includeFeeds includes trace subspace queue results', async () => {
      ...
      const feed = db.add(Feed.make({}));
      expect(getFeedUriOrThrow(feed)).toMatch(/^echo:\/\//);
      ...
      invariant(traceResult, 'trace result missing');
    ...
    test('from(feed) scopes query to feed items', async () => {
      ...
      expect(feedResults.map((obj) => obj.title).sort()).toEqual(['Feed Task 1', 'Feed Task 2']);
    });
    ...
    test('from(feed) with Filter.id scopes to feed', async () => {
      ...
      expect((feedResult[0] as TestSchema.Task).title).toBe('Feed Task');
  ...
  test('objects url changes, the latest document is loaded', async () => {
    ...
    let assertion: { objectId: string; documentUrl: string };
    {
      ...
      invariant(obj1DocHandle, 'docHandle missing');
      invariant(anotherDocHandle, 'docHandle missing');
      ...
      const obj1Entity = obj1DocHandle.doc().objects?.[obj1.id];
      invariant(obj1Entity, 'source entity missing');
      const anotherDocUrl = anotherDocHandle.url;
      invariant(anotherDocUrl, 'docHandle url missing');
      anotherDocHandle.change((doc: DatabaseDirectory) => {
        doc.objects ??= {};
        doc.objects[obj1.id] = obj1Entity;
      });
      rootDocHandle.change((doc: DatabaseDirectory) => {
        doc.links ??= {};
        doc.links[obj1.id] = new A.RawString(anotherDocUrl);
      });
      await db.flush();

      assertion = { objectId: obj2.id, documentUrl: anotherDocUrl };
    }
  ...
  test('query fails if one of the results fails to load', async () => {
    ...
    invariant(obj2Core.docHandle, 'docHandle missing');
  ...
  test.skip('map over refs in query result', async () => {
    const { db } = await builder.createDatabase();
    const folder = db.add(Obj.make(TestSchema.Expando, { name: 'folder', objects: [] as Ref.Ref<Entity.Any>[] }));
  ...
  describe('Traversal', () => {
    ...
    test('traverse to children of a feed returns feed queue items', async () => {
      ...
      expect(objects).toHaveLength(2);
      const titles = objects.map((obj) => obj.title).sort();
  ...
  describe('indexer2 text search', () => {
    ...
    test('full-text search in queues via indexer2', async () => {
      ...
      // Search in specific queue.
      {
        const objects = await db
          .query(
            Query.select(Filter.text('TypeScript', { type: 'full-text' })).from([Scope.feed(getFeedUriOrThrow(feed))]),
          )
          .run();
        expect(objects).toHaveLength(1);
      ...
      {
        const objects = await db
          .query(Query.select(Filter.text('React', { type: 'full-text' })).from([Scope.feed(getFeedUriOrThrow(feed))]))
          .run();
        expect(objects).toHaveLength(1);
      ...
      // Non-matching query.
      {
        const objects = await db
          .query(
            Query.select(Filter.text('JavaScript', { type: 'full-text' })).from([Scope.feed(getFeedUriOrThrow(feed))]),
          )
          .run();
        expect(objects).toHaveLength(0);
      }
    ...
    test('full-text search returns rank in entries', async () => {
      ...
      for (const entry of entries) {
        const { match } = entry;
        invariant(match, 'match metadata missing');
    ...
    test('full-text search order by rank descending (default)', async () => {
      ...
      for (let i = 0; i < entries.length - 1; i++) {
        ...
        const { match: nextMatch } = entries[i + 1];
        invariant(match, 'match metadata missing');
        invariant(nextMatch, 'match metadata missing');
        expect(match.rank).toBeGreaterThanOrEqual(nextMatch.rank);
      }

      // The item with highest relevance (most occurrences) should be first.
      const { result: topResult } = entries[0];
      invariant(topResult, 'result missing');
      expect(topResult.title).toBe('TypeScript TypeScript TypeScript TypeScript');
    });
    ...
    test('full-text search order by rank ascending', async () => {
      ...
      for (let i = 0; i < entries.length - 1; i++) {
        ...
        invariant(match, 'match metadata missing');
        invariant(nextMatch, 'match metadata missing');
        expect(match.rank).toBeLessThanOrEqual(nextMatch.rank);
      }

      // The item with lowest relevance should be first.
      const { result: lastResult } = entries[entries.length - 1];
      invariant(lastResult, 'result missing');
      expect(lastResult.title).toBe('TypeScript TypeScript TypeScript TypeScript');
    });
    ...
    test('order by rank with limit', async () => {
      ...
      const { result: firstResult } = entries[0];
      const { result: secondResult } = entries[1];
      invariant(firstResult, 'result missing');
      invariant(secondResult, 'result missing');
      expect(firstResult.title).toBe('TypeScript TypeScript TypeScript TypeScript');
      expect(secondResult.title).toBe('TypeScript TypeScript TypeScript');
    });
  });

  describe('Reactivity', () => {
    let db: EchoDatabase;
    let objects: Obj.OfShape<TestSchema.Expando>[];
  ...
  describe('RegistryQuerySource', () => {
    ...
    test('registry is opt-in: plain db.query() excludes registry objects', async ({ expect }) => {
      ...
      expect(dbResults[0].value).toBe(99);

      // Opting into the registry scope fans in both.
      const bothResults = await db
        .query(Query.select(Filter.type(TestSchema.Expando)).from(Scope.space(), Scope.registry()))
        .run();
      const values = bothResults.map((r) => r.value).sort((a, b) => (a ?? 0) - (b ?? 0));
  ...
  describe('Filter.childOf', () => {
    ...
    test('A -> Feed -> B: childOf([A]) matches B', async () => {
      ...
      const taskResults = objects.filter((obj) => obj.title === 'Grandchild task');
    ...
    test('childOf with type filter prevents Feed from being returned', async () => {
      ...
      expect(objects.every((obj) => Obj.getTypename(obj) !== 'org.dxos.type.feed')).toBe(true);
    });
    ...
    test('childOf with limit returns feed items (regression: limit must not be pushed past child-of)', async ({
      ...
      const objects = await db
        ...
        .run();
      expect(objects).toHaveLength(2);
      expect(objects.map((obj) => obj.title).sort()).toEqual(['A', 'B']);
    });
  ...
  describe('Filter.hasParent', () => {
    test('selects parented vs unparented objects', async ({ expect }) => {
      ...
      expect(roots.map((obj) => obj.name).sort()).toEqual(['Parent', 'Standalone']);

      const children = await db.query(Query.select(Filter.hasParent())).run();
      expect(children.map((obj) => obj.name)).toEqual(['Child']);
    });
    ...
    test('negation inverts the predicate', async ({ expect }) => {
      ...
      expect(objects.map((obj) => obj.name)).toEqual(['Parent']);
    });
    ...
    test('reacts to Obj.setParent after the fact', async ({ expect }) => {
      ...
      await db.flush({ updates: true });
      await waitForCondition({ condition: () => query.results.length === 1, timeout: 2000 });
      expect(query.results.map((obj) => obj.name)).toEqual(['Parent']);
...
const assertQuery = async (db: EchoDatabase, filter: Filter.Any, expected: Entity.Unknown[]) => {
...
const sortById = <T extends Entity.Unknown>(objects: T[]) => objects.sort((a, b) => a.id.localeCompare(b.id));
