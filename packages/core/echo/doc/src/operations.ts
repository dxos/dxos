...
import { DocAccessor, type KeyPath } from './accessor';
 ...
 */
export const change = <T>(accessor: DocAccessor<T>, mutator: (doc: T) => void): void => {
 ...
 */
export const splice = (accessor: DocAccessor, index: number, deleteCount: number, text: string): void => {
 ...
 */
export const append = (accessor: DocAccessor, text: string): void => {
  const current = DocAccessor.getValue<string>(accessor) ?? '';
 ...
 */
export const onChange = (accessor: DocAccessor, callback: () => void): (() => void) => {
 ...
 */
export const getValueAt = <T>(accessor: DocAccessor, subPath: KeyPath = []): T | undefined =>
  getDeep(accessor.handle.doc(), [...accessor.path, ...subPath]) as T | undefined;

/**
 * Current change-history heads of the underlying document.
 */
export const getHeads = (accessor: DocAccessor): A.Heads => A.getHeads(accessor.handle.doc()!);

/**
 * Patches between two sets of heads. Returns an empty array when the heads are equal.
 */
export const diff = (accessor: DocAccessor, from: A.Heads, to: A.Heads): Patch[] => {
  if (A.equals(from, to)) {
    return [];
  }
