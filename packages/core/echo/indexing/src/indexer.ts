...
import { type IndexConfig, IndexKind_Kind, IndexKindSchema } from '@dxos/protocols/buf/dxos/echo/indexing_pb';
...
export class Indexer extends Resource {
  ...
  async execQuery(filter: IndexQuery): Promise<FindResult[]> {
    ...
    if (filter.graph) {
      const graphIndex = this._engine.getIndex(create(IndexKindSchema, { kind: IndexKind_Kind.GRAPH }));
      if (!graphIndex) {
      ...
      const vectorIndex = this._engine.getIndex(create(IndexKindSchema, { kind: IndexKind_Kind.VECTOR }));
      if (!vectorIndex) {
        // TODO(dmaretskyi): This shouldn't be the default?
        return [];
      }
      ...
      const textIndex = this._engine.getIndex(create(IndexKindSchema, { kind: IndexKind_Kind.FULL_TEXT }));
      if (!textIndex) {
        // TODO(dmaretskyi): This shouldn't be the default?
        return [];
      }
      ...
      const typenameIndex = this._engine.getIndex(create(IndexKindSchema, { kind: IndexKind_Kind.SCHEMA_MATCH }));
      if (!typenameIndex) {
        // TODO(dmaretskyi): This shouldn't be the default?
        return [];
      }
