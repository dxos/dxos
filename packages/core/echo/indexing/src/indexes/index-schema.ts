...
export class IndexSchema extends Resource implements Index {
  ...
  public readonly kind = create(IndexKindSchema, { kind: IndexKind_Kind.SCHEMA_MATCH });
  ...
  async find(filter: IndexQuery): Promise<FindResult[]> {
    ...
    if (filter.inverted) {
      return Array.from(this._index.entries())
        .filter(([key]) => key !== null && filter.typenames.includes(key))
