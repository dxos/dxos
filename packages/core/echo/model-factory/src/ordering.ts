...
export const getInsertionIndex = (
  existing: ModelMessage<Uint8Array>[],
  newMutation: ModelMessage<Uint8Array>
) => {
  let start = existing.length - 1;
  for (
    const ourKey = PublicKey.from(newMutation.meta.feedKey);
    start >= 0;
    start--
  ) {
  ...
  for (let i = start + 1; i < existing.length; i++) {
    const existingTimeframe = Timeframe.merge(
      ...
      new Timeframe([
        [PublicKey.from(existing[i].meta.feedKey), existing[i].meta.seq - 1]
      ])
    );

    const deps = Timeframe.dependencies(
      newMutation.meta.timeframe,
      existingTimeframe
    );
    if (deps.isEmpty()) {
      if (
        PublicKey.from(newMutation.meta.feedKey).toHex() <
        PublicKey.from(existing[i].meta.feedKey).toHex()
      ) {
        return i;
      }
    }
