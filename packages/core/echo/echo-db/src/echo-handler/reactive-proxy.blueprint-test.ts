...
export type TestConfigurationFactory = (schema: Type.AnyObj) => TestConfiguration | null;
  ...
  for (const schema of [TestSchema.Expando, TestSchema.Example]) {
    ...
    describe(`Proxy properties(schema=${Type.getTypename(schema)})`, () => {
      ...
      test('validation failures', async (ctx) => {
        ...
        expect(() => Obj.update(obj, (obj) => (obj.string = 1 as any))).to.throw();
        expect(() => Obj.update(obj, (obj) => (obj.nested = { field: 1 } as any))).to.throw();
        Obj.update(obj, (obj) => (obj.nested = { field: 'bar' }));
        expect(() => Obj.update(obj, (obj) => (obj.nested!.field = 1 as any))).to.throw();
        expect(() => Obj.update(obj, (obj) => obj.nestedArray?.push({ field: 1 } as any))).to.throw();
        expect(() => Obj.update(obj, (obj) => obj.nestedArray?.unshift({ field: 1 } as any))).to.throw();
        expect(() => Obj.update(obj, (obj) => (obj.nestedArray![0] = { field: 1 } as any))).to.throw();
        expect(() => Obj.update(obj, (obj) => (obj.nestedArray![0].field = 1 as any))).to.throw();
        Obj.update(obj, (obj) => obj.nestedArray?.push({ field: 'bar' }));
        expect(() => Obj.update(obj, (obj) => obj.nestedArray?.splice(1, 0, { field: 1 } as any))).to.throw();
      ...
      test('getSchemaDXN', async () => {
        ...
        expect(Obj.getTypeURI(obj)?.toString()).to.deep.eq(schema && getSchemaDXN(schema)?.toString());
      });
