 ...
 */
export type AnyLiveRelation<T extends AnyProperties> = Live<T> & (Obj.Any | Relation.Any) & RelationSourceTargetRefs;

/**
 * @deprecated Use {@link @dxos/echo#Relation.isRelation} instead.
 */
export const isRelation = <T extends AnyProperties>(object: AnyLiveObject<T>): object is AnyLiveRelation<T> => {
...
export const getSource = (relation: AnyLiveObject<any>): AnyLiveObject<any> => {
  invariant(Relation.isRelation(relation));
