...
describe('LocalQueueServiceImpl', () => {
  ...
  it.effect('should support pagination', () =>
    Effect.gen(function* () {
      ...
      yield* Effect.promise(() => service.insertIntoQueue('default', spaceId, queueId, items));

      // Query first 5
      const page1 = yield* Effect.promise(() => service.queryQueue('default', spaceId, { queueId, limit: 5 }));
      ...
      const page2 = yield* Effect.promise(() =>
        service.queryQueue('default', spaceId, {
          queueId,
          limit: 5,
