...
describe('canonicalQueryKey', () => {
  test('two queries differing only in debugLabel produce the same key', ({ expect }) => {
  ...
  test('query without debugLabel and one with debugLabel produce the same key', ({ expect }) => {
  ...
  test('two queries with different filter types produce different keys', ({ expect }) => {
  ...
  test('two queries with different typenames produce different keys', ({ expect }) => {
    const ast1 = Query.select(Filter.typename('Foo')).ast;
    const ast2 = Query.select(Filter.typename('Bar')).ast;
    expect(canonicalQueryKey(ast1)).not.toEqual(canonicalQueryKey(ast2));
  });

  test('key is stable across repeated calls for the same AST', ({ expect }) => {
    const ast = Query.select(Filter.everything()).ast;
    expect(canonicalQueryKey(ast)).toEqual(canonicalQueryKey(ast));
  });

  test('key is stable regardless of JSON key insertion order', ({ expect }) => {
  ...
  test('queries with different deleted options produce different keys', ({ expect }) => {
  ...
  test('queries with different from scopes produce different keys', ({ expect }) => {
