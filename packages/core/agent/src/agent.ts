...
export class Agent {
  ...
  async start() {
    log('starting...', { gateway: this._config.values.runtime?.services?.faasd?.gateway });

    // Create client services.
    const services = fromHost(this._config);
    ...
    for (const address of this._options.listen) {
      ...
      switch (protocol) {
        ...
        case 'http': {
          const { port } = new URL(address);
          const server = createProxy(services, this._config, { port: parseInt(port) });
          await server.open();
          break;
        }
