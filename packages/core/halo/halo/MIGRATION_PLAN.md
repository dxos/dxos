...
| ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | -------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| plugin-client                                                                                                                                                                                                                                                                                                                                                                  | partial  | DevicesContainer (4, 9), app-graph-builder (6, 9), invitations UI (8), the client provider + `IdentityService`/`SpaceService` capabilities (intentional boundary) |
...
- **HALO remaining tier: Missing APIs 4–6 outstanding** (device-invitation/Shell,
  script write-op, graph-builder atom). APIs 1–3 are DONE. `plugin-payments` is
  now entirely off `@dxos/client`; `plugin-assistant` and `plugin-connector` keep
  only non-HALO pins (a `Client` type in `ConnectorSpec`/`testing`, CLI
  `ClientService`, `client.config`). What still touches the client for HALO is
  `plugin-client` (Shell device list, app-graph-builder, invitations UI, plus the
  client provider itself), `plugin-onboarding` (credential query + the EDGE
  recovery redirect), and `plugin-script` (the credential write, API 5).
