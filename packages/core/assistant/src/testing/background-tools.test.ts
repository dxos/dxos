  ...
  Tool.make('inspect-job', {
    ...
    parameters: {
      ...
      timeout: Schema.optional(Schema.Number).annotations({
        description: 'Maximum time to wait for the job to complete. If the job does not complete within the timeout, the current state is returned.',
        default: 10_000,
      }),
    },
  }),
...
const runJob = (effect: Effect.Effect<any, never>): Effect.Effect<string> => Effect.gen(function* () {
  const id: string = crypto.randomUUID();
  const fiber = yield* Effect.forkDaemon(Effect.gen(function* () {
...
const TestToolkitLayer = TestToolkit.toLayer({
  'research-organization': ({ website }) => Effect.gen(function* () {
    const jobId = yield* runJob(Effect.gen(function* () {
      yield* Effect.sleep(15_000 * (TEST_DATA.organizations.findIndex((organization) => organization.website === website) + 1));
      const research: string = TEST_DATA.research[website as keyof typeof TEST_DATA.research];
      return research;
    }));
    return { jobId };
  }),
  'inspect-job': ({ jobs, wait, timeout = 10_000 }) => Effect.gen(function* () {
    return yield* Effect.forEach(jobs, (jobId) => Effect.gen(function* () {
      ...
      const result: Option.Option<Exit.Exit<any, never>> = !wait ? yield* fiber.poll : yield* fiber.await.pipe(
...
const TestLayer = TestToolkitLayer.pipe(Layer.provideMerge(AssistantTestLayer({
...
describe('Background Tools', () => {
  it.live('should run a job', Effect.fnUntraced(function* (_) {
    ...
    while (inputQueue.length > 0) {
      history.push(...yield* new AiSession().run({
        history,
        prompt: inputQueue.join('\n'),
      ...
      if (inputQueue.length === 0 && activeJobs.size > 0) {
        const result = yield* Effect.raceAll(activeJobs.entries().map(([jobId, fiber]) => fiber.await.pipe(
          Effect.map(() => {
            activeJobs.delete(jobId);
  ...
  }, Effect.provide(TestLayer), TestHelpers.provideTestContext), { timeout: 120_000 });
});