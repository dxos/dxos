...
describe.skip('Profile Test Cases', () => {
  let alice: TaskApp;
  ...
  describe('As a new user starting the app then', () => {
    it('I should be prompted to create my profile',
      () => alice.profile.checkCreationIsPrompted())

    it('I should be able to create my profile',
      () => alice.profile.create('Alice').then(() => alice.checkAppIsLoaded())
    )
  });
});
