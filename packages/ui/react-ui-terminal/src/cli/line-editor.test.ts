...
describe('readLineResult', () => {
  test('ignores alt chords rather than typing their character', async ({ expect }) => {
    // Alt-f is a word motion the editor does not implement; it must not insert an `f`.
    const { result } = await readWith('ls\x1bf\r');
    expect(result).to.deep.eq({ type: 'line', value: 'ls' });
  });

  test('returns the typed line on return', async ({ expect }) => {
    const { result } = await readWith('space list\r');
    expect(result).to.deep.eq({ type: 'line', value: 'space list' });
  });

  test('returns an empty line for a bare return', async ({ expect }) => {
    const { result } = await readWith('\r');
    expect(result).to.deep.eq({ type: 'line', value: '' });
  });

  test('backspace removes the previous character', async ({ expect }) => {
    const { result } = await readWith('lists\x7f\r');
    expect(result).to.deep.eq({ type: 'line', value: 'list' });
  });

  test('backspace at the start is a no-op', async ({ expect }) => {
    const { result } = await readWith('\x7f\x7fok\r');
    expect(result).to.deep.eq({ type: 'line', value: 'ok' });
  });

  test('left arrow moves the insertion point', async ({ expect }) => {
    const { result } = await readWith('ac\x1b[Db\r');
    expect(result).to.deep.eq({ type: 'line', value: 'abc' });
  });

  test('right arrow moves back toward the end', async ({ expect }) => {
    const { result } = await readWith('ac\x1b[D\x1b[Cd\r');
    expect(result).to.deep.eq({ type: 'line', value: 'acd' });
  });

  test('delete removes the character under the cursor', async ({ expect }) => {
    const { result } = await readWith('abc\x1b[D\x1b[3~\r');
    expect(result).to.deep.eq({ type: 'line', value: 'ab' });
  });

  test('home and end jump to the buffer edges', async ({ expect }) => {
    const { result } = await readWith('bc\x1b[Ha\x1b[Fd\r');
    expect(result).to.deep.eq({ type: 'line', value: 'abcd' });
  });

  test('ctrl-a and ctrl-e jump to the buffer edges', async ({ expect }) => {
    const { result } = await readWith('bc\x01a\x05d\r');
    expect(result).to.deep.eq({ type: 'line', value: 'abcd' });
  });

  test('ctrl-u clears to the start and ctrl-k clears to the end', async ({ expect }) => {
