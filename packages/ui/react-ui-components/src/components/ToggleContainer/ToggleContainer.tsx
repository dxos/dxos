...
const IconBlock = ({ children }: PropsWithChildren) => {
  return <div className='grid block-[24px] inline-[24px] place-items-center'>{children}</div>;
};
...
const Root = ({
  ...
  return (
    <ToggleContainerContext
      ...
      setOpen={setOpen}
    >
      <div role='none' className={mx('overflow-hidden', !shrink && 'inline-full', classNames)}>
        {children}
      </div>
    </ToggleContainerContext>
  );
...
const Header = ({ classNames, children, icon }: HeaderProps) => {
  ...
  return (
    <div
      className={mx('inline-full p-1 grid grid-cols-[24px_1fr_24px] gap-1 cursor-pointer select-none', classNames)}
