...
describe('YJS', () => {
  ...
  test('absolute position', () => {
    ...
    {
      const absPos = Y.createAbsolutePositionFromRelativePosition(Y.decodeRelativePosition(relPos), obj.doc!);
      expect(absPos!.index).to.equal(index);
      expect(obj.text.substring(absPos!.index)).to.equal('world');
    }
    ...
    {
      const absPos = Y.createAbsolutePositionFromRelativePosition(Y.decodeRelativePosition(relPos), obj.doc!);
      expect(absPos!.index).not.to.equal(index);
      expect(obj.text.substring(absPos!.index)).to.equal('world');
    }
  });
});
