...
export const EditorStory = forwardRef<EditorController, StoryProps>(
  ({ debug, debugCustom, text, extensions: extensionsProp, ...props }, forwardedRef) => {
    ...
    return (
      <div className={mx('inline-full block-full grid overflow-hidden', debug && 'grid-cols-2 lg:grid-cols-[1fr_600px]')}>
        <EditorComponent ref={mergedRef} object={object} text={text} extensions={extensions} {...props} />

        {debug && (
          <div
            className='grid block-full auto-rows-fr border-l border-separator divide-y divide-separator overflow-hidden'
            {...attentionAttrs}
          >
