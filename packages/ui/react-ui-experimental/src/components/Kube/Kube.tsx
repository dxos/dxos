...
class KubeRenderer {
  ...
  setSize(width: number, height: number) {
    if (!this._renderer || width <= 0 || height <= 0) {
      return;
    }
   ...
   * governs buffer sizes set in `init` and cannot be changed live; any incoming
   * value is dropped so live `particleCount` updates stay clamped to the
  ...
  animate() {
    ...
    if (showLines) {
      ...
      // Clear stale connections so toggling `showLines` off immediately hides them.
      this._linesMesh.visible = false;
      this._linesMesh.geometry.setDrawRange(0, 0);
    }
