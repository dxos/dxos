 ...
 * 3. a **tool call** lands whole, then its **result** arrives as a second block after it.
 *
 * Yields the complete block array at each step; the caller replaces the message's blocks with it.
 */
export async function* streamTurn({ chunkDelay = 120, wordsPerChunk = 4 }: TurnOptions = {}): AsyncGenerator<
  ContentBlock.Any[],
  void
> {
  ...
  const called: ContentBlock.Any = { ...call, pending: false };
  const result: ContentBlock.Any = {
    _tag: 'toolResult',
    toolCallId,
    name: 'search',
  ...
  yield [reasoned, called, result];
  await pause();

  let text = '';
  for await (const chunk of textStream(createAnswer(), { chunkDelay, wordsPerChunk })) {
    text += chunk;
    yield [reasoned, called, result, { _tag: 'text', text, pending: true }];
  }
