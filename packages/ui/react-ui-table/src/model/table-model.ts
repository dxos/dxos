...
export class TableModel<T extends TableRow = TableRow> extends Resource {
  ...
  private readonly _viewState?: ViewState.ViewStateManager;
  // Per-user sort state - backed by the `tableSortAspect` view state (keyed by table URI) when a
  // ViewState.ViewStateManager is provided, else an ephemeral atom. Changes are local until saved to the view.
  ...
  constructor({
    ...
    // ephemeral fallback preserves prior behaviour when no ViewState.ViewStateManager is supplied.
  ...
  /**
   * Writes the per-user sort, routing through the ViewState.ViewStateManager (so the `local` backend persists)
