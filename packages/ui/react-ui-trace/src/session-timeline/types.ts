...
/**
 * A conversation the timeline draws as a session lane: the chat, reduced to what the join needs so
 * the builder does not depend on the chat's schema. `uri` is what the agent process targets and
 * `feedId` what its trace meta names; `taskIds` is the checklist.
 */
