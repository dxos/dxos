 ...
 * `parentId`. A band is contiguous by construction, which is the whole reason the order is settled
 * here rather than left to the caller.
 */
const orderRows = (groups: readonly GanttGroup[], lanes: readonly GanttLane[]): { rows: Row[]; bands: Band[] } => {
  const rows: Row[] = [];
  const bands: Band[] = [];
