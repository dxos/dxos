...
| `Treegrid.*`           | `role=treegrid` grid layout + tabster row nav.                    | **Moved up** into `react-ui-list` (§4.1). Consumers are now devtools, plugin-assistant and plugin-atproto — `Tree` was decoupled from it by the Ark rebuild. |
