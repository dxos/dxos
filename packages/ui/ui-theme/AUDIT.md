...
- [x] Audit the following react-ui components: Container, Dialog, Popover, Splitter, ScrollArea.
  ...
  - [x] Audit dx-container classes which deal with container/scrolling behavior.
...
Evaluated composability (Radix `asChild`/Slot pattern), usage of `dx-container`, and scroll containment robustness.
...
- No `dx-container` classes are applied by Container itself. It relies on CSS grid rather than flexbox, so these classes are not relevant here.
...
- `Dialog.Body` applies `h-full` via theme but `Dialog.Content` does not apply `dx-container`. For dialogs with scrollable body content, consumers must add `overflow-hidden` and nest a `ScrollArea` themselves.
...
- `Panel` applies `flex flex-col overflow-hidden`, so children can use `dx-container` to fill the panel and establish scroll containment. The `overflow-hidden` on the panel is important — it prevents content from overflowing during transitions.
- `Root` has `overflow-hidden` as well, providing a containment boundary.
- No `dx-container` is applied internally; consumers must apply them to panel children as needed.
