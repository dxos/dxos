...
export const menuViewport: ComponentFunction<MenuStyleProps> = (_props, ...etc) =>
  mx('rounded-sm p-1 max-bs-[var(--radix-dropdown-menu-content-available-height)] overflow-y-auto', ...etc);

export const menuContent: ComponentFunction<MenuStyleProps> = ({ elevation }, ...etc) =>
  mx(
    'is-48 rounded-sm md:is-56 border border-separator',
...
export const menuItem: ComponentFunction<MenuStyleProps> = (_props, ...etc) =>
  mx(
    'flex cursor-pointer select-none items-center gap-2 rounded-xs pli-2 plb-2 text-sm',
