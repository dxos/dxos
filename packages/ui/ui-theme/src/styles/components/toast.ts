...
export const toastViewport: ComponentFunction<ToastStyleProps> = (_props, ...etc) =>
  mx(
    ...
    'z-40 fixed block-end-[calc(env(safe-area-inset-bottom)+1rem)] inset-start-[calc(env(safe-area-inset-left)+1rem)] inset-end-[calc(env(safe-area-inset-right)+1rem)] inline-auto md:inline-end-[calc(env(safe-area-inset-right)+1rem)] md:inline-start-auto md:inline-full md:max-is-sm',
...
export const toastBody: ComponentFunction<ToastStyleProps> = (_props, ...etc) =>
  mx('grow flex flex-col gap-1 justify-center ps-2', ...etc);
