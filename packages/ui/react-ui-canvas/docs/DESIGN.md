...
| **Control frame**                                              | Shown for the selection: outline, eight resize handles (`resize` intent, respects `CellDef.minSize`). Ports show on the hovered and selected cells, and on every cell with the `link` tool. Hover is a model hit test with a port-sized margin, not the cell element's enter/leave, so it survives the pointer crossing onto a port on the frame edge. |
