...
| Drag from a port             | Rubber band; drop on a port pins both ends, drop on a node body leaves the target automatic, drop on empty canvas creates a rectangle and links to it (canvas-editor behaviour). The link's type is the active link tool's, or the last one picked under `select`. Emits a `link` intent carrying the whole link.                                  |
...
| Click a link / spline points | Links are selectable (a wide transparent twin of the stroke takes the press) and deletable. A selected spline shows diamond handles on its control points: drag moves, double-click on the stroke inserts at the nearest segment, alt-click removes; all `update` intents.                                                                         |
