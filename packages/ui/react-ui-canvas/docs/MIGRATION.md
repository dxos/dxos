...
| Link from anchor: nearest valid anchor within 32 units, live curve, drop on frame (rectangles only) or canvas (creates a rectangle) | ✓ port snap 16 px, drop on body = automatic port, drop on canvas creates a rectangle                                                                        | ✓      | M1 (decision 2): `Port.accepts` decides what a drop lands on                                                            |
...
| Shortcuts: meta+z/shift+meta+z (no-op), meta+x/c/v, Backspace/Delete, Escape, d, meta+a, meta+', Home                               | Escape, Delete/Backspace, ⌘A, ⌘X/C/V, ⌘Z/⇧⌘Z, arrows nudge, Shift+1/2/0, Home, Alt+←/→, G, D, tool keys                                                     | ✓      | M2                                                                                                                      |
...
| `createComputeGraph(canvasGraph)` and the `testing/circuits.ts` factories                 | unchanged; `sceneFromCircuit` turns a circuit into a scene (notes become text nodes, edges pin the property ports)                                                                                            | ✓      | M3; M4 replaces `sceneFromCircuit` with the ECHO store over `CanvasBoard.layout` |
