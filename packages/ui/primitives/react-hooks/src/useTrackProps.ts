 ...
 */
export const useTrackProps = <T extends Record<string, unknown>>(props: T, componentName = 'Component') => {
  ...
  useEffect(() => {
    ...
    if (changes.length > 0) {
      log.info('props changed', {
        ...
        props: Object.fromEntries(changes.map(([key]) => [key, { from: prevProps.current[key], to: props[key] }])),
      });
    }
