...
const deckLayout = `overflow-y-clip overflow-x-auto ${deckGrid}`;

const soloLayout =
  'grid grid-rows-[var(--rail-size)_[toolbar-start]_var(--rail-action)_[content-start]_1fr_[content-end]] grid-cols-1 overflow-clip min-bs-0 bs-full';
...
const DeckRoot = forwardRef<HTMLDivElement, DeckRootProps>(
  ({ classNames, children, asChild, solo, ...props }, forwardedRef) => {
    ...
    // TODO(thure): because `overflow-x-auto` causes this to become a scroll container, the clip value on the y-axis becomes a `hidden`
