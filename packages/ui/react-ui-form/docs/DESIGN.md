...
| `Form.FieldSet`  | `Fieldset.Root` + `Legend` + `HelperText` + `ErrorText`; `Collapsible.*` when `collapsible` | Presentation and semantics only: `label`, `description`, `collapsible`, with depth chrome from context (a top-level legend is a heading with the section gap, a nested one a plain legend with an indented body). Wraps anything: fields, walkers, other field sets, hand-written rows. No `path`, no enumeration.                                                                                                                                                                       |
| `Form.Fields`    | nothing                                                                                     | The walker: `path?`, `include`, `exclude`, `sort`, `filter`, resolved against the enclosing scope. Renders a `Form.Field` per property and, for a nested object, a `Form.FieldSet` (label from the schema title, collapsible) around a `Form.Fields` at that path. A group the walk produces for a property is nested by definition, whatever encloses the walk: the walk starts its output at depth 1, so a walk at the form's root still boxes its nested objects.                     |
