...
import { EchoURI, Entity, Feed, Filter, Format, Obj, Query, QueryAST, Ref, type SchemaRegistry, View } from '@dxos/echo';
...
export const ViewEditor = forwardRef<ProjectionModel, ViewEditorProps>(
  (
    ...
    const targetRef = useMemo(() => {
      ...
      return feed ? Ref.fromURI(Entity.getId(feed)) : undefined;
    }, [queueTarget, feeds]);
    ...
    const handleUpdate = useCallback(
      (values: any) => {
        ...
        if (Ref.isRef(targetValue)) {
          ...
          const feed = feeds.find((feed) => Obj.getURI(feed) === feedDxn);
          if (feed) {
