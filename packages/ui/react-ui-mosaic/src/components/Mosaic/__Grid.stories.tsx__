...
import { type MosaicEventHandler, Mosaic, dropHandler } from '@dxos/react-ui-mosaic';
...
const DefaultStory = () => {
  ...
  const searchHandler = useMemo<MosaicEventHandler>(
    () => ({
      id: 'search',
      canDrop: () => false,
      onTake: (item, cb) => {
  ...
  const objectHandler = useMemo<MosaicEventHandler>(() => {
  ...
  const documentHandler = useMemo<MosaicEventHandler>(() => {
    return {
      id: 'document',
      canDrop: () => true,
      onDrag: ({ position }) => {
