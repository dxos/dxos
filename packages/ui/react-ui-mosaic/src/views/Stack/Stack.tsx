...
const StackImpl = ({ id, items, direction }: Pick<StackProps, 'id' | 'direction'> & { items: MosaicDataItem[] }) => {
...
export const Stack = (props: StackProps) => {
  const { id, Component = Mosaic.DefaultComponent, onOver, onDrop, items = [], direction = 'vertical', debug } = props;
  return (
    <div className={mx('flex overflow-hidden', direction === 'vertical' && 'w-[300px]')}>
      ...
      >
        <Mosaic.Container {...{ id, Component, onOver, onDrop }}>
          <StackImpl id={id} items={items} direction={direction} />
        </Mosaic.Container>
        {debug && (
          <div className='grow'>
            <Mosaic.Debug data={{ id, items: items.length }} />
          </div>
        )}
      </div>
    </div>
