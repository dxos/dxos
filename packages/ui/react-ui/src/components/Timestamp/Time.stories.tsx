...
const DefaultStory = () => (
  <div className='grid grid-cols-[1fr_min-content] gap-x-4 gap-y-1 text-sm'>
    {LADDER.map(({ label, minutes }) => (
      <React.Fragment key={label}>
        <span className='text-description'>{label}</span>
        <Time date={minutesAgo(minutes)} classNames='text-right' />
      </React.Fragment>
    ))}
  </div>
);
...
export const Tooltips: Story = {
  render: () => <Time date={minutesAgo(90)} />,
  play: async ({ canvasElement }) => {
    const canvas = within(canvasElement);
...
export const Pinned: Story = {
  render: () => {
    ...
    return <Time date={new Date(now.getTime() - 90 * 60_000)} now={now} />;
  },
  play: async ({ canvasElement }) => {
