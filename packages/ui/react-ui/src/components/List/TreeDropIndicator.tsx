...
const orientationStyles: Record<Orientation, HTMLAttributes<HTMLElement>['className']> = {
  ...
  sibling:
    'h-(--line-thickness) left-(--horizontal-indent) right-0 bg-accent-fill before:left-(--negative-terminal-size)',
...
export const TreeDropIndicator = ({ instruction, gap = 0 }: DropIndicatorProps) => {
  ...
  return (
    <div
      ...
      className={`absolute z-10 pointer-events-none before:w-(--terminal-size) before:h-(--terminal-size) box-border before:absolute before:border-[length:--line-thickness] before:border-solid before:border-accent-fill before:rounded-full ${orientationStyles[orientation]} ${instructionStyles[desiredInstruction.type]}`}
    ></div>
  );
};
