...
export const ListDropIndicator = ({
  ...
  return (
    <div
      ...
      className={`absolute z-10 pointer-events-none bg-accent-fill before:content-[''] before:w-(--terminal-size) before:h-(--terminal-size) box-border before:absolute before:border-[length:--line-thickness] before:border-solid before:border-accent-fill before:rounded-full ${orientationStyles[orientation]} ${edgeStyles[edge]}`}
    />
  );
};
