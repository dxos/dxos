...
// A control row at the density's height, so a checkbox or switch takes the room an input would.
const block: ComponentFunction<InputStyleProps> = (_props, ...etc) =>
  mx('grid place-items-center min-w-(--dx-control) min-h-(--dx-control)', ...etc);
