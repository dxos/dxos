...
- Create a table of Components that contain Scrollable.Root as the FIRST element.
- Determine if they can be made Composable (see slot.ts); i.e., forwardRef and spread composableProps on the first element.
...
| ----------------------------- | ----------------------- | --------------------- | -------- | ---------- |
| `composer-crx`                | `Options.tsx`           | `Options`             | Direct   | Yes        |
| `testbench-app`               | `ItemList.tsx`          | `ItemList`            | Direct   | Yes        |
| `plugin-assistant`            | `Toolbox.tsx`           | `Toolbox`             | Direct   | Yes        |
| `plugin-inbox`                | `Event.tsx`             | `Event.Viewport`      | Compound | Yes        |
| `plugin-outliner`             | `Journal.tsx`           | `Journal`             | Direct   | Yes        |
| `plugin-registry`             | `PluginDetail.tsx`      | `PluginDetail`        | Direct   | Yes        |
| `plugin-registry`             | `PluginRegistry.tsx`    | `PluginRegistry`      | Direct   | Yes        |
| `plugin-thread`               | `ChatContainer.tsx`     | `ChatContainer`       | Direct   | No (1)    |
| `plugin-thread`               | `ThreadCompanion.tsx`   | `ThreadCompanion`     | Direct   | No (2)    |
...
| `react-ui-syntax-highlighter` | `SyntaxHighlighter.tsx` | `SyntaxHighlighter`   | Direct   | Yes        |
...
1. **ChatContainer** (plugin-thread) - ScrollArea.Root is not the root element; it is nested inside `ThreadComponent.Root`.
2. **ThreadCompanion** (plugin-thread) - ScrollArea.Root is not the root element; it is nested inside `Tabs.Root > Panel.Root`.
