...
export type ToolbarStyleProps = Partial<{
  ...
  /** An explicit level, from `elevation`: the bar paints it, even inside a Panel slot. */
  surface: Surface;
}>;
...
const root: ComponentFunction<ToolbarStyleProps> = ({ density, disabled, layoutManaged, surface }, ...etc) => {
  return mx(
    ...
    // floating inside the bar. (Merged via `asChild` there is one element, which must keep it.) An
    // explicit level is the bar's own and paints regardless.
    !surface && '[[data-slot=toolbar]_&]:bg-transparent',
    !surface && '[[data-slot=statusbar]_&]:bg-transparent',
