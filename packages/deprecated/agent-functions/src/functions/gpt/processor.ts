...
export class RequestProcessor {
  ...
  private async _createModelArgs(
    ...
    if (options.command) {
      const { objects: allPrompts = [] } = await space.db.query(Filter.schema(TemplateType)).run({ timeout: 3000 });
  ...
  private async getTemplateInput(
    ...
    switch (type) {
      ...
      // User message.
      //
      case TemplateInputType.PASS_THROUGH: {
      ...
      //
      case TemplateInputType.RETRIEVER: {
      ...
      //
      case TemplateInputType.RESOLVER: {
        const type = value;
        if (!type) {
          return null;
        }
      ...
      //
      case TemplateInputType.CONTEXT: {
        return () => {
          if (value) {
