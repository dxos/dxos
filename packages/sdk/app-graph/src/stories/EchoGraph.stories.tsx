				...
				Function.pipe(
					...
					Option.map(() => {
						...
						return spaces
							.filter(
								(space) =>
									get(rxFromObservable(space.state)) === SpaceState.SPACE_READY,
							)
							.map((space) => ({
								id: space.id,
		...
		connector: (node) => {
			...
			return Atom.make((get) =>
				Function.pipe(
					get(node),
					Option.flatMap((node) =>
						isSpace(node.data) ? Option.some(node.data) : Option.none(),
					),
					Option.map((space) => {
						if (!query) {
...
const randomAction = () => {
	...
	return actionDistribution[
		Math.floor(Math.random() * actionDistribution.length)
	];
};
...
const getSpaceWithObjects = async (
	client: Client,
): Promise<Space | undefined> => {
	const readySpaces = client.spaces
		.get()
		.filter((space) => space.state.get() === SpaceState.SPACE_READY);
	const spaceQueries = await Promise.all(
		readySpaces.map((space) =>
			space.db.query(Filter.type(Expando, { type: "test" })).run(),
		),
	);
	const spaces = readySpaces.filter(
		(space, index) => spaceQueries[index].objects.length > 0,
	);
	return spaces[Math.floor(Math.random() * spaces.length)];
};
		...
		case Action.REMOVE_OBJECT: {
			...
			if (space) {
				const { objects } = await space.db
					.query(Filter.type(Expando, { type: "test" }))
					.run();
		...
		case Action.RENAME_OBJECT: {
			...
			if (space) {
				const { objects } = await space.db
					.query(Filter.type(Expando, { type: "test" }))
					.run();
				objects[Math.floor(Math.random() * objects.length)].name =
			...
			<div className="flex shrink-0 p-2 space-x-2">
				...
				<div className="relative" title="mutation period">
					...
					<Icon
						...
						classNames={mx(
							"absolute inline-end-1 block-start-1 mt-[6px]",
							getSize(3),
						)}
					/>
				</div>
				<IconButton
					...
					label="Add"
					onClick={() => action && runAction(client, action)}
				/>
				<Select.Root
					value={action?.toString()}
					onValueChange={(action) => setAction(action as unknown as Action)}
				>
...
export const TreeView: Story = {
	render: () => {
		...
		const state = useMemo(
			() => new Map<string, Live<{ open: boolean; current: boolean }>>(),
			[],
		);
		...
		const useItems = useCallback(
			(node?: Node, options?: { disposition?: string; sort?: boolean }) => {
				const connections = useAtomValue(
					graph.connections(node?.id ?? ROOT_ID),
				);
				return options?.sort
					? connections.toSorted((a, b) =>
							byPosition(a.properties, b.properties),
						)
					: connections;
			},
			[graph],
		...
		const getProps = useCallback(
			(node: Node, path: string[]) => {
				...
				return {
					...
					label: node.id,
					icon:
						node.type === "dxos.org/type/Space"
							? "ph--planet--regular"
							: "ph--placeholder--regular",
					parentOf,
				};
			},
			[graph],
