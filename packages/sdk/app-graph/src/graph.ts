...
export class Graph implements WritableGraph {
	...
	readonly _node = Atom.family<string, Atom.Writable<Option.Option<Node>>>((id) => {
		...
		return Atom.make<Option.Option<Node>>(initial).pipe(
			Atom.keepAlive,
	...
	private readonly _edges = Atom.family<string, Atom.Writable<Edges>>((id) => {
		const initial = Record.get(this._initialEdges, id).pipe(
			Option.getOrElse(() => ({ inbound: [], outbound: [] })),
		);
		return Atom.make<Edges>(initial).pipe(
			Atom.keepAlive,
			Atom.withLabel(`graph:edges:${id}`),
		);
	});
	...
	private readonly _actions = Atom.family<
		string,
		Atom.Atom<(Action | ActionGroup)[]>
	>((id) => {
		return Atom.make((get) => {
	...
	async initialize(id: string) {
		const initialized = Record.get(this._initialized, id).pipe(
	...
	expand(id: string, relation: Relation = "outbound"): void {
		...
		const expanded = Record.get(this._expanded, key).pipe(
			Option.getOrElse(() => false),
		);
	...
	addNode({
		...
		edges,
		...nodeArg
	}: NodeArg<any, Record<string, any>>): void {
		...
		Option.match(node, {
			onSome: (node) => {
				...
				const propertiesChanged = Object.keys(properties).some(
					(key) => node.properties[key] !== properties[key],
				);
				log("existing node", {
					id,
					typeChanged,
	...
	removeEdge(edgeArg: Edge, removeOrphans = false): void {
		...
		if (removeOrphans) {
			...
			const target = this._registry.get(targetRx);
			if (
				source.outbound.length === 0 &&
				source.inbound.length === 0 &&
				edgeArg.source !== ROOT_ID
			) {
				this.removeNodes([edgeArg.source]);
			}
			if (
				target.outbound.length === 0 &&
				target.inbound.length === 0 &&
				edgeArg.target !== ROOT_ID
			) {
	...
	sortEdges(id: string, relation: Relation, order: string[]): void {
		...
		edges[relation].splice(
			0,
			edges[relation].length,
			...[...sorted, ...unsorted],
		);
	...
	traverse(
		{ visitor, source = ROOT_ID, relation = "outbound" }: GraphTraversalOptions,
		path: string[] = [],
	): void {
		// Break cycles.
