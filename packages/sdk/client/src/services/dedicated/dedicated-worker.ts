...
/** WebKit fails a first handle for what its own error calls transient, so the probe backs off and tries again. */
const OPFS_PROBE_RETRY = Schedule.exponential('250 millis').pipe(Schedule.upTo({ times: 3 }));
 ...
 */
const probeOpfs = Effect.tryPromise(async () => {
  ...
  Effect.tapError((err) => Effect.sync(() => log.warn('OPFS sync access handle not opened', { err }))),
  Effect.retry(OPFS_PROBE_RETRY),
  Effect.catch((err) => Effect.die(new Error(`OPFS storage is unusable: ${err}`))),
);
