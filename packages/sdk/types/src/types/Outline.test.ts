...
describe('Outline', () => {
  ...
  describe('addTask', () => {
    test('files the task into the given set, parented for cascade', async ({ expect }) => {
      ...
      const task = Outline.addTask(taskSet, db, '  Buy milk  ');
    ...
    test('files every task into the same set, in order', async ({ expect }) => {
      ...
      const first = Outline.addTask(taskSet, db, 'First');
      const second = Outline.addTask(taskSet, db, 'Second');
    ...
    test('extra props reach the task', async ({ expect }) => {
      ...
      const task = Outline.addTask(taskSet, db, 'Delegated', {
        status: 'in-progress',
