...
- 582fc22: `Surface` holds its previous `data` reference while the incoming one is shallow-equal to it, so an ancestor render no longer re-renders the plugin subtree below a surface.
...
- 892b718: Surfaces register with the mount registry under a profiler provider in production builds, so the devtools Surfaces card lists them and its highlight toggle works outside dev.
