...
const getHueValue = (identity?: Identity) =>
	...
	hexToHue(identity?.identityKey.toHex() ?? "0");
const getEmojiValue = (identity?: Identity) =>
	identity?.profile?.data?.emoji ||
	hexToEmoji(identity?.identityKey.toHex() ?? "0");
...
const IdentityHeading = ({
	...
	const [displayName, setDisplayNameDirectly] = useState(
		identity.profile?.displayName ?? "",
	);
	const [emoji, setEmojiDirectly] = useState<string>(getEmojiValue(identity));
	const [hue, setHueDirectly] = useState<string | undefined>(
		getHueValue(identity),
	);
	...
	return (
		<Heading
			titleId={titleId}
			title={title}
			corner={<CloseButton onDone={onDone} />}
		>
			<Avatar.Root>
				...
				>
					{identity.profile?.displayName ??
						generateName(identity.identityKey.toHex())}
				</Avatar.Label>
				...
				<Toolbar.Root classNames="justify-center pt-3">
					...
					<Toolbar.IconButton
						...
						onClick={() =>
							onChangeConnectionState?.(
								isConnected ? ConnectionState.OFFLINE : ConnectionState.ONLINE,
							)
						}
					/>
				</Toolbar.Root>
...
export const IdentityPanelImpl = (props: IdentityPanelImplProps) => {
	const {
		...
		onRecover,
		onJoinNewIdentity,
		IdentityActionChooser:
			IdentityActionChooserComponent = IdentityActionChooser,
...
export const IdentityPanel = ({
	...
	const [identityState, identitySend, identityService] = useIdentityMachine(
		client,
		{
	...
	const handleChangeConnectionState = async (nextState: ConnectionState) =>
		client?.mesh.updateConfig(nextState);
	...
	return identityState.context.invitation ? (
		<IdentityPanelWithInvitationImpl
			{...implProps}
