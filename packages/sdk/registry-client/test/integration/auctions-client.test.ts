...
describe('Auctions Client', function () {
  ...
  describe('Auction', function () {
    it('Creates an auction', async function () {
      ...
      await expect(auctionsApi.createAuction(auctionName, 100000)).to.be
        .fulfilled;
    });
    ...
    it('Does not allow to create already created auction', async function () {
      const auctionName = Math.random().toString(36).substring(2);
      const expectedError =
        apiPromise.errors.registry.AuctionAlreadyCreated.meta.name.toString();

      await expect(auctionsApi.createAuction(auctionName, 100000)).to.be
        .fulfilled;
      await expect(
        auctionsApi.createAuction(auctionName, 100000)
  ...
  describe('Auction Bid', function () {
    it('Allows to bid on name', async function () {
      ...
      await expect(auctionsApi.createAuction(auctionName, 100000)).to.be
        .fulfilled;
      await expect(auctionsApi.bidAuction(auctionName, 1000001)).to.be
        .fulfilled;
    });
    ...
    it('Does not allow to bid on non-existing name', async function () {
      const auctionName = Math.random().toString(36).substring(2);
      const expectedError =
        apiPromise.errors.registry.AuctionNotFound.meta.name.toString();

      await expect(
        auctionsApi.bidAuction(auctionName, 1000000)
      ).to.be.rejectedWith(expectedError);
    });
    ...
    it('Does not allow to bid too small value', async function () {
      const auctionName = Math.random().toString(36).substring(2);
      const expectedError =
        apiPromise.errors.registry.BidTooSmall.meta.name.toString();
  ...
  describe('Auction closing and claiming', function () {
    it('Cannot close or claim an unfinished auction', async function () {
      ...
      await expect(auctionsApi.createAuction(auctionName, 100000)).to.be
        .fulfilled;
      await expect(auctionsApi.closeAuction(auctionName)).to.be.eventually
        .rejected;
      await expect(auctionsApi.claimAuction(auctionName, account)).to.be
        .eventually.rejected;
    });
    ...
    it('Only the winner can claim an auction', async function () {
      ...
      await auctionsApi.forceCloseAuction(auctionName, sudoer);
      await expect(loser.claimAuction(auctionName, account)).to.be.eventually
        .rejected;
      await expect(winner.claimAuction(auctionName, account)).to.be.eventually
        .fulfilled;
    });
    ...
    it('Auction winner has a domain registered', async function () {
      ...
      const nativeAccountKey = (
        await apiPromise.query.registry.domains(domainKey.value)
      ).unwrapOr(undefined)?.owner;
