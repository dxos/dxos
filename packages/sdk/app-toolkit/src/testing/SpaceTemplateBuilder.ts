...
import * as SpaceTemplate from '../template/SpaceTemplate.ts';
...
export type SpaceTemplateArchive<A> = {
...
/**
 * Builds a space template headlessly and exports it as a JSON archive: no live client, no network.
 * Drop the archive into Composer (`client.spaces.import(...)`) to get the content as real data.
 */
export const buildArchive = <Phases extends SpaceTemplate.PhaseMap, A>(
  definition: SpaceTemplate.Definition<Phases, A>,
  options: {
    readonly identity?: string;
    /** Extra types to register beyond those the definition's phases declare. */
    readonly schemas?: ReadonlyArray<Type.AnyEntity>;
  } = {},
): Effect.Effect<SpaceTemplateArchive<A>, SpaceTemplate.SpaceTemplateError> =>
  Effect.gen(function* () {
    ...
    const result = yield* SpaceTemplate.applyTo(definition, space);
