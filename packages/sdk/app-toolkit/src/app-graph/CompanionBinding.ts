 ...
 * (not the workspace base), matched by the plank's `~<variant>` linked-segment child, so the graph
 * builder stamps and reverse-maps `~<variant>` nodes from this one declaration. It produces no nodes of
 * its own; companion nodes come from their plugins (via {@link AppNode.makeCompanion}).
 *
