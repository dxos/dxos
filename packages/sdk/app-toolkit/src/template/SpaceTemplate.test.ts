...
import * as SpaceTemplate from './SpaceTemplate.ts';
...
const Organizations = SpaceTemplate.phase('organizations', {
  schemas: [Organization.Organization],
  run: () => SpaceTemplate.seed(ORG_SEEDS, ({ name }) => Database.add(Obj.make(Organization.Organization, { name }))),
});

const People = SpaceTemplate.phase('people', {
  schemas: [Person.Person],
  run: (organizations: Organizations) =>
    SpaceTemplate.seed(PERSON_SEEDS, ({ fullName, org }) =>
      Database.add(Obj.make(Person.Person, { fullName, organization: Ref.make(organizations[org]) })),
    ),
});

const Work = SpaceTemplate.phase('work', {
  schemas: [TaskSet.TaskSet, Task.Task],
  run: () =>
    Effect.gen(function* () {
      const opened = yield* SpaceTemplate.daysAgo(3);
      ...
      yield* SpaceTemplate.children(taskSet, tasks, (taskSet, refs) => {
        taskSet.tasks = refs;
      });
...
const Journal = SpaceTemplate.phase('journal', {
  ...
  run: () =>
    Effect.gen(function* () {
      ...
      yield* SpaceTemplate.appendToFeed(feed, entries);
      return { feed, entries };
    }),
});

const definition = SpaceTemplate.make({
  ...
  build: (phases) =>
    Effect.gen(function* () {
      ...
      const collection = yield* SpaceTemplate.collection('Work', [Ref.make(taskSet)]);
...
describe('clock', () => {
  test('resolves offsets against the reference date, not the wall clock', ({ expect }) => {
    const clock = SpaceTemplate.makeClock('2026-05-20T15:00:00Z');
...
describe('applyTo', () => {
  ...
  test('runs every phase against a live space', async ({ expect }) => {
    ...
    const result = await EffectEx.runPromise(SpaceTemplate.applyTo(definition, space));
  ...
  test('wires collections under the space root', async ({ expect }) => {
    ...
    const root = await EffectEx.runPromise(
      Effect.flatMap(SpaceTemplate.Root, ({ get }) => get).pipe(
        Effect.provide(
          Layer.provideMerge(
            SpaceTemplate.layer({ properties: space.properties, reference: definition.reference }),
