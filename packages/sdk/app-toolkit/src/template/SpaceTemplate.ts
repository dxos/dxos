...
/**
 * Raised when a space template cannot be built — a missing space root, an unresolvable tag, or a
 * phase whose prerequisites were not met.
 */
export class SpaceTemplateError extends BaseError.extend('SpaceTemplateError', 'Space template build failed') {}

//
// Clock
//
// Template content is authored relative to a fixed reference date rather than the wall clock, so a
...
>()('@dxos/app-toolkit/SpaceTemplate/Clock') {}
...
/** ISO timestamp `days` before the template's reference date. */
...
/** ISO timestamp `days` after the template's reference date. */
...
>()('@dxos/app-toolkit/SpaceTemplate/Feeds') {}
...
export class Root extends Context.Service<
  Root,
  {
    ...
    readonly get: Effect.Effect<Collection.Collection, SpaceTemplateError, Database.Service>;
  }
>()('@dxos/app-toolkit/SpaceTemplate/Root') {}
...
export const makeRoot = (properties: Obj.Any): Root['Service'] => {
  ...
  return {
    get: Effect.gen(function* () {
      ...
      if (!root) {
        return yield* Effect.fail(new SpaceTemplateError({ context: { reason: 'root-collection-unresolved' } }));
      }
      cached = root;
      return root;
    }),
...
) => Effect.Effect<Collection.Collection, SpaceTemplateError, Database.Service | Root> = Effect.fnUntraced(function* (
  name: string,
  objects: Ref.Ref<Obj.Unknown>[],
) {
...
>()('@dxos/app-toolkit/SpaceTemplate/Tags') {}
...
export const tagBatch = Effect.fnUntraced(function* (
  ...
  for (const { object, key } of entries) {
    ...
    if (tagId === undefined) {
      return yield* Effect.fail(new SpaceTemplateError({ context: { reason: 'tag-unresolved', key } }));
    }
...
) => Effect.Effect<T[], SpaceTemplateError, Database.Service> = Effect.fnUntraced(function* <
  P extends Obj.Any,
  T extends Obj.Any,
...
// Seeds
//
// Template content is authored as a table of seed rows keyed by a short discriminant, so later phases
...
export const seed = Effect.fnUntraced(function* <Key extends string, Props, T, R>(
  ...
  make: (seed: Seed<Key, Props>) => Effect.Effect<T, SpaceTemplateError, R>,
) {
...
/**
 * A named, reusable unit of template content. Phases declare the schemas their objects need so a
...
export interface Phase<A, In = void> {
  ...
  readonly run: (input: In) => Effect.Effect<A, SpaceTemplateError, Services>;
}
...
export interface Definition<Phases extends PhaseMap, A> {
  ...
  readonly schemas: ReadonlyArray<Type.AnyEntity>;
  readonly build: (phases: PhaseRunners<Phases>) => Effect.Effect<A, SpaceTemplateError, Services>;
}

/**
 * Defines a space template: which space to create, the reference date its content is authored
...
/** Layer providing every space-template service for a space's properties and reference date. */
 ...
 * which is what lets a template double as a debug-plugin preset.
 */
...
): Effect.Effect<A, SpaceTemplateError> => {
