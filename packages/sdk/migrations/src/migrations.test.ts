...
Migrations.define('test', [
  {
    version: '1970-01-01',
    next: async ({ builder }) => {
      await builder.addObject(Type.Expando, { namespace: 'test', count: 1 });
    },
  },
  {
    version: '1970-01-02',
    next: async ({ space, builder }) => {
      // TODO(dmaretskyi): Is this intended to query only expando objects? Change to `Filter.type(Type.Expando, { namespace: 'test' })`
...
describe('Migrations', () => {
  ...
  test('if no migrations have been run before, runs all migrations', async () => {
    ...
    const objects = await space.db.query(Filter.type(Type.Expando, { namespace: 'test' })).run();
  ...
  test('if some migrations have been run before, runs only the remaining migrations', async () => {
    ...
    await space.db.graph.schemaRegistry.register([Type.Expando]);
