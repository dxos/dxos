...
export const getSchema = async (
	...
	const echoSchema = await echoRegistry
		?.query({ typename: type, version })
		.firstOrUndefined();
...
export const getSchemaFromPropertyDefinitions = (
	...
	const fields: any = Object.fromEntries(
		properties
			...
			.map((prop) => [prop.name, typeToSchema[formatToType[prop.format]]]),
	);

	const schema = createEchoSchema(
		TypedObject({ typename, version: "0.1.0" })(fields),
	);

	for (const prop of properties) {
		if (prop.config?.options) {
			if (prop.format === Format.TypeFormat.SingleSelect) {
				makeSingleSelectAnnotations(schema.jsonSchema.properties![prop.name], [
					...prop.config.options,
				]);
			}
			if (prop.format === Format.TypeFormat.MultiSelect) {
				makeMultiSelectAnnotations(schema.jsonSchema.properties![prop.name], [
					...prop.config.options,
				]);
			}
		}
