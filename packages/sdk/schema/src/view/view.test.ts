...
describe("Projection", () => {
	...
	test("create view from schema", async ({ expect }) => {
		...
		expect(view.query.ast.filter.typename).to.eq(
			Type.getDXN(schema)?.toString(),
		);
		...
		{
			const { props } = projection.getFieldProjection(
				projection.getFieldId("fullName")!,
			);
		...
		{
			const { props } = projection.getFieldProjection(
				projection.getFieldId("organization")!,
			);
			expect(props).to.deep.eq({
	...
	test("static schema definitions with references", async ({ expect }) => {
		...
		expect(Obj.getTypename(organization)).to.eq(
			Type.getTypename(DataType.Organization.Organization),
		);
		expect(Obj.getTypename(contact)).to.eq(
			...
			jsonSchema: Type.toJsonSchema(
				Schema.Struct({
					...
					salary: Schema.optional(
						Type.Format.Currency({ code: "usd", decimals: 2 }),
					),
				}),
			),
		});
