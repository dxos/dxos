...
const exportFeedData = async (space: DataSpace, echoHost: EchoHost, objects: Obj.JSON[]): Promise<SerializedFeed[]> => {
  ...
  for (const obj of objects) {
    ...
    const typeDxnNsid = DXN.isDXN(typeStr) ? DXN.getNsid(typeStr) : undefined;
    ...
    try {
      const messages = await collectQueueMessages(echoHost, spaceId, obj.id as ObjectId, namespace);
