...
export const TreeViewContainer = observer(() => {
  ...
  const { sidebarOpen } = useSidebar(TreeViewPlugin.meta.id);
...
//
// Temporary fake drag & drop.
//

type FakeDragAndDropValue = {
  source: GraphNode | null;
  target: GraphNode | null;
  setSource: (source: GraphNode | null) => void;
  setTarget: (target: GraphNode | null) => void;
};

const FakeDragAndDropContext = createContext<FakeDragAndDropValue>({
  source: null,
  target: null,
  setSource: () => {},
  setTarget: () => {},
...
const FakeDragAndDropProvider = ({ children }: { children: ReactNode }) => {
  ...
  useEffect(() => {
    if (source && target) {
      const timeout = setTimeout(async () => {
        const sourceOutput = source.onReceivedTarget?.(target);
        if (sourceOutput) {
          await target.onReceivedSource?.(sourceOutput);
        }
  ...
  return (
    <FakeDragAndDropContext.Provider value={{ source, target, setSource, setTarget }}>
      {children}
    </FakeDragAndDropContext.Provider>
  );
};

export const useFakeDragAndDrop = () => useContext(FakeDragAndDropContext);
