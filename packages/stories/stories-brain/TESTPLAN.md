...
Env knobs: `MODELS` (`local` | `remote` | name substrings), `LIMIT` (message cap → `partial/`),
`TESTS` (comma-separated bench names), `SUBJECT`, `SAMPLES`, `DRAFT_INSTRUCTIONS`. Results land in
`fixtures/local/results/` (git-ignored) as `<name>.json` (stats) + `<name>.md` (responses).
