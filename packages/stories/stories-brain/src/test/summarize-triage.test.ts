...
describe('summaryKindFor', () => {
  test('person mail earns a full summary', ({ expect }) => {
    expect(summaryKindFor(msg({ email: 'alice@unknown.com', name: 'Alice' }))).toBe('summary');
  });

  test('org / bulk mail gets a one-line label', ({ expect }) => {
  ...
  test('an explicit senderClass overrides the heuristic', ({ expect }) => {
