...
export default class Create extends BaseCommand<typeof Create> {
  ...
  async run(): Promise<any> {
    ...
    if (outputDirExists && !isOutputEmpty) {
      this.catch(`Output directory ${outputDirectory} is not empty`, { exit: 1 });
    }

    try {
      await exec('which pnpm');
    } catch {
      this.catch('pnpm not found. Please run "npm i -g pnpm" first.', { exit: 1 });
    }
    ...
    if (os.platform() === 'darwin') {
      try {
        ...
        this.catch('XCode Command Line Tools not found. Please run "xcode-select --install" first.', { exit: 1 });
      }
    }
