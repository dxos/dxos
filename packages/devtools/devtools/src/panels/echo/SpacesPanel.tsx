...
import { mx, Table } from '@dxos/react-components';
...
const columns: Column<PipelineTableRow>[] = [
  {
    Header: 'FeedKey',
    width: 120,
    accessor: (block) => {
    ...
    <div>
      <div>
        ...
        {controlKeys.map((feedKey) => (
          <FeedView key={feedKey.toHex()} feedKey={feedKey} processed={state.currentControlTimeframe?.get(feedKey)} target={state.targetControlTimeframe?.get(feedKey)} />
        ))}
      </div>
      <div>
        <h4>Data feeds</h4>
        {dataKeys.map((feedKey) => (
          <FeedView key={feedKey.toHex()} feedKey={feedKey} processed={state.currentDataTimeframe?.get(feedKey)} target={state.targetDataTimeframe?.get(feedKey)} />
        ))}
      </div>
    </div>
  )
...
const FeedView = ({ feedKey, processed = 0, target = 0 }: FeedViewProps) => {
  ...
  return (
    <div className='flex flex-row overflow-auto gap-0.5'>
      ...
      {range(total).map(i => {
        const color = i <= processed ? 'bg-green-400' :
          i <= target ? 'bg-orange-400'
            : 'bg-gray-400';

        return (
          <div key={i} className={mx('w-[3px] h-[20px]', color)}></div>
        )
      })}
    </div>
  )
