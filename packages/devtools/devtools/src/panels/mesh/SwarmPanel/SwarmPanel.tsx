...
import { createBooleanColumn, createKeyColumn, createTextColumn, Grid, GridColumn } from '@dxos/aurora-grid';
...
export const SwarmPanel = () => {
  ...
  const columns: GridColumn<Connection>[] = [
    ...
    createKeyColumn('session', { accessor: 'sessionId' }), // TODO(burdon): Select.
    createKeyColumn('remote peer', { accessor: 'remotePeerId' }),
  ];

  return (
    <PanelContainer>
      <div className='h-1/2 overflow-auto'>
        <Grid<Connection> columns={columns} data={items} />
      </div>
      <div className='h-1/2 overflow-auto'>
        {sessionId && (
          <ConnectionInfoView
