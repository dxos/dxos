...
export const WorkflowDebugPanel = (props: WorkflowDebugPanelProps) => {
		...
		props.loader
			...
			.then((workflow) => {
				...
				if (workflowMeta.inputs.length) {
					const inputTemplate = inputTemplateFromAst(
						workflowMeta.inputs[0].schema.ast,
					);
	...
	const handleRequest = async (input: string) => {
		...
		try {
			...
			setHistory((history) => [
				...history,
				{ type: "request", text: JSON.stringify(requestBody, null, 2) },
			]);
			...
			if (props.mode === WorkflowDebugPanelMode.REMOTE) {
				response = await edgeClient.executeWorkflow(
					space.id,
					props.graph.id,
					requestBody,
				);
			} else {
				const compiled = await props.loader.load(
					DXN.fromLocalObjectId(props.graph.id),
				);
