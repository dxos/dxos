...
export const QueriesPanel = ({ queries, ...props }: CustomPanelProps<{ queries: QueryInfo[] }>) => {
  ...
  return (
    ...
    >
      <table className='table-fixed inline-full text-xs font-mono'>
        <tbody>
          {queries.map((query, i) => (
            <tr key={i}>
              <td className='p-1 inline-[24px]'>
                <Icon
              ...
              <td className='p-1 inline-[80px] text-right'>{query.metrics.objectsReturned.toLocaleString()}</td>
              <td className='p-1 inline-[80px] text-right'>
