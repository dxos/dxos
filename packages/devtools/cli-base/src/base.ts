...
import { matchKeys, selectSpace, waitForSpace } from './util';
...
export type Flags<T extends typeof Command> = Interfaces.InferredFlags<
  (typeof AbstractBaseCommand)['baseFlags'] & T['flags']
>;
 ...
 */
export abstract class AbstractBaseCommand<T extends typeof Command = any> extends Command {
      ...
      if (options.types) {
        for (const type of options.types) {
          if (!space.db.graph.schemaRegistry.hasSchema(type)) {
            space.db.graph.schemaRegistry.addSchema(type as any);
          }
        }
      }
