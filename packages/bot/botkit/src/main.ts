...
import { createApiPromise, PolkadotRegistryClientBackend, RegistryClient } from '@dxos/registry-client';
...
const main = async () => {
  ...
  if (dxnsServer) {
    ...
    const registry = new RegistryClient(new PolkadotRegistryClientBackend(apiPromise));
