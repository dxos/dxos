...
describe('FeedDescriptor', () => {
  ...
  beforeEach(async () => {
    ...
    fd = new FeedDescriptor({
      directory: createStorage('', StorageType.RAM).directory(),
  ...
  test('Can create feed descriptor with public key but without private key', async () => {
    ...
    const fd = new FeedDescriptor({ key, directory: createStorage('', StorageType.NODE).directory(), hypercore: defaultHypercore });
  ...
  test('Close and open again', async () => {
    ...
    const { publicKey, secretKey } = createKeyPair();
    const fd = new FeedDescriptor({
      directory: createStorage(root, StorageType.NODE).directory(),
      key: PublicKey.from(publicKey),
      secretKey,
      valueEncoding: 'utf-8',
      hypercore: defaultHypercore
