...
export class FeedDescriptor {
  ...
  constructor(options: FeedDescriptorOptions) {
    ...
    assert(!!signer !== !!secretKey, 'Either signer or secretKey must be provided');
  ...
  private async _open() {
    ...
    if (this._signer) {
      // Keys generated by keyring are 65 bytes long, but hypercore expects maximum of 64 bytes.
      // First byte seems to always be 04 so its safe to just slice it off.
      // NOTE: This might cause a bug where descriptor.key !== descriptor.feed.key.
      const key = this._key.asBuffer().slice(1);
