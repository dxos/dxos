...
export class AuthPlugin extends EventEmitter {
  ...
  private async _onHandshake (protocol: Protocol /* code , context */) { // TODO(burdon): ???
    ...
    try {
      ...
      wrappedCredentials = schema.getCodecForType('dxos.halo.credentials.Auth').decode(Buffer.from(credentials, 'base64'));
    } catch (err: any) {
      protocol.stream.destroy();
    ...
    const credsPeerId = wrappedCredentials.crednetials.credentials?.[0]?.subject.id;
    ...
    const authenticated = await this._authenticator.authenticate(wrappedCredentials as any); // TODO(dmaretskyi): Fix type import issue.
    if (!authenticated) {
      protocol.stream.destroy();
