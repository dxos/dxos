...
/**
 * The prerelease channels, by the environment name the deploy sets: each names its own artwork
 * (`assets/favicons-<channel>`, `src-tauri/icons-<channel>`, generated by `scripts/gen-icon-variants.mjs`)
 * and its own mark colour. Production is the released app and is not a variant.
 */
export type ChannelVariant = 'dev' | 'preview' | 'staging';

const CHANNELS: readonly ChannelVariant[] = ['dev', 'preview', 'staging'];

const isChannel = (environment: string): environment is ChannelVariant =>
  CHANNELS.some((channel) => channel === environment);
...
/**
 * Each channel's colour as `scripts/gen-icon-variants.mjs` defines it for the raster icons: a hue that
 * replaces the ramp's, and a saturation scale. Kept in step by hand; the numbers are the channel's
 * identity. Preview is the channel people run beside production, so it gets the hue that reads least
 * like the released blue; the others share a rust, since telling them apart from each other matters
 * less than telling any of them apart from the app holding real data.
 */
const MARK_VARIANTS: Record<ChannelVariant, { hue: number; saturation: number }> = {
  preview: { hue: 282, saturation: 1 },
  dev: { hue: 20, saturation: 0.75 },
  staging: { hue: 20, saturation: 0.75 },
};
