...
import { LOG_STORE_DB_NAME, LOG_STORE_MAX_BYTES, exportManualLogDownload, triggerLogDownload } from '../util/index.ts';

/** Export gzipped NDJSON logs from the IDB log collector and save them to disk. */
export const downloadRecoveryLogs = async (): Promise<{ byteLength: number; saved: boolean }> => {
  ...
  try {
    ...
    const saved = await triggerLogDownload(blob);
