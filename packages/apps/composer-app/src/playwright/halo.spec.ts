...
const probing = async (peers: Record<string, AppManager>, assertion: () => Promise<void>) => {
  try {
    ...
    for (const [name, peer] of Object.entries(peers)) {
      ...
      console.log(
        `SETTINGS-PROBE ${name} id=${probe.settingsId} sharedStack=${probe.shared?.['org.dxos.app-framework.plugins']?.[stack]} enabled=${probe.enabled?.includes(stack)} pins=${JSON.stringify(probe.local)} failed=${JSON.stringify(probe.failed)} spaces=${JSON.stringify(probe.settingsSpaces)}`,
      );
