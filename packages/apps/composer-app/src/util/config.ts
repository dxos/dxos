...
export const initializeObservability = async (
  ...
  /**
   * Handle posting to this realm's log-writer worker. When present, OTel log and metric
   ...
   * synchronous task.
   */
  logWriter?: { post: (message: ObservabilityExtension.Otel.OtelWorkerMessage) => void },
) =>
  Function.pipe(
    Observability.make(),
