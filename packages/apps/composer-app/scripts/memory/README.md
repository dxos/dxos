...
| `read-heap-snapshot.mjs` | The streaming `.heapsnapshot` reader the two above share; a loaded page's snapshot passes V8's ~512 MB string cap, so `JSON.parse` of the file cannot read it |
