...
| `startup-timing.mjs`     | Time to a usable shell on a returning tab (the account item in the tree, since the app emits no ready mark), with first paint, the first and last module activation, and how many scripts and bytes had arrived by ready; three runs and the mean                                                                                                          |
| `boot-errors.mjs`        | Boots a profile copy and prints console errors, page errors and failed requests, plus whether the app reached ready — the fastest check that a bundling change still boots, and the only one here that sees worker errors                                                                                                                                       |
