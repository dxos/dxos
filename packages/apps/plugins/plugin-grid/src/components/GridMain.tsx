...
import { Grid, createColumnBuilder, defaultGridSlots, GridColumnDef } from '@dxos/aurora-grid';
...
const columns: GridColumnDef<TypedObject, any>[] = [
  helper.accessor('id', {
    size: 100,
    cell: (cell) => <div className='font-mono text-green-500'>{cell.getValue().slice(0, 8)}</div>,
  }),
  helper.accessor((item) => item.__typename, {
    id: 'type',
    ...builder.string({ className: 'font-mono text-green-500' }),
  }),
];
