...
export const HaloButton = (props: HaloButtonProps) => {
  ...
  return (
    <button className={mx(focusRing, 'rounded grid place-items-center')} onClick={onClick}>
      <Avatar.Root size={size} variant='circle' status={internal ? 'internal' : 'active'} hue={fallbackValue.hue}>
        <Avatar.Frame data-testid='treeView.haloButton' data-joyride='welcome/halo'>
          {internal ? <Avatar.Image href={INTERNAL} /> : <Avatar.Fallback text={fallbackValue.emoji} />}
        </Avatar.Frame>
      </Avatar.Root>
    </button>
  );
