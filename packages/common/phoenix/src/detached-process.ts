 ...
 */
export const startWatchdog = async (params: WatchDogParams) => {
  {
    // Create log folders.
...

  {
    ...
    if (existsSync(params.pidFile)) {
      await stopWatchDog(params.pidFile);
    }
  ...
  return getInfo(params.pidFile);
};

/**
 * Stops detached watchdog process by PID info written down in PID file.
 */
export const stopWatchDog = async (pidFile: string, force = 'false') => {
...
export const getInfo = (pidFile: string): ProcessInfo => JSON.parse(readFileSync(pidFile, { encoding: 'utf-8' }));
