...
test('can send a request', async () => {
  ...
  const response = await bob.call(Buffer.from('request'));
...
test('can send multiple requests', async () => {
  ...
  expect(await bob.call(Buffer.from('request'))).toEqual(Buffer.from('request'));

  const parallel1 = bob.call(Buffer.from('p1'));
  const parallel2 = bob.call(Buffer.from('p2'));
  const error = bob.call(Buffer.from('error'));
...
test('errors get serialized', async () => {
  ...
  let error!: Error;
  try {
    await bob.call(Buffer.from('request'));
  } catch (err) {
    error = err;
  }
...
test('closing local endpoint stops pending requests', async () => {
  ...
  const req = bob.call(Buffer.from('request'));
  bob.close();
