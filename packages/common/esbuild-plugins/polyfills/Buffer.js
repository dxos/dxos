...
const Buffer = (arg, encodingOrOffset, length) => {
  // Common case.
 ...
 **/
Buffer.from = (value, encodingOrOffset, length) => from(value, encodingOrOffset, length)
...
const fromArrayBuffer = (array, byteOffset, length) => {
...
Buffer.isBuffer = (b) => b != null && b._isBuffer === true &&
    b !== Buffer.prototype
...
const utf8Write = (buf, string, offset, length) => blitBuffer(utf8ToBytes(string, buf.length - offset), buf, offset, length);
...
const asciiSlice = (buf, start, end) => {
  let ret = ''
  end = Math.min(buf.length, end)
...
const latin1Slice = (buf, start, end) => {
  let ret = ''
  end = Math.min(buf.length, end)
...
const utf16leSlice = (buf, start, end) => {
...
const wrtBigUInt64LE = (buf, value, offset, min, max) => {
...
const wrtBigUInt64BE = (buf, value, offset, min, max) => {
  checkIntBI(value, min, max, buf, offset, 7)
...
const checkIntBI = (value, min, max, buf, offset, byteLength) => {
...
const isInstance = (obj, type) => obj instanceof type ||
    (obj != null && obj.constructor != null && obj.constructor.name != null &&
      obj.constructor.name === type.name);
const numberIsNaN = (obj) => obj !== obj;
...
const defineBigIntMethod = (fn) => typeof BigInt === 'undefined' ? BufferBigIntNotDefined : fn;

const BufferBigIntNotDefined = () => {
