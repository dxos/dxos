...
): Mapper => createMessageMapperCached(type, substitutions, {}).map;
...
const createMessageMapperCached = (
  ...
  if (!cache[type.fullName]) {
    ...
    cache[type.fullName].map = codegen(
      `${type.name}$map`,
      ['obj', 'extraArgs'],
      (c) => {
        c`const res = {};`;
        for (const field of type.fieldsArray) {
          ...
          c`if(obj.${field.name} !== undefined && obj.${field.name} !== null) {`;
          {
            const genMapScalar = (value: string) => {
              const substitution =
                field.resolvedType &&
                substitutions[field.resolvedType.fullName.slice(1)];
              if (substitution) {
                ...
                const mapper = createMessageMapperCached(
                  field.resolvedType,
                  substitutions,
                  cache
                );
