...
import { BackpressureExecutor, type BackpressureExecutorParams } from './backpressure-executor';

describe('BackpressureExecutor', () => {
  const getPlanner = async (params: Partial<BackpressureExecutorParams> = {}) => {
    const planner = new BackpressureExecutor({
      budget: 500,
      budgetPeriod: 1000,
      restTime: 500,
      maxParallelTasks: 1,
  ...
  test('should execute task', async ({ expect }) => {
    const planner = await getPlanner();
    const result = await Promise.all(planner.execute([{ run: async () => 1 }]));
  ...
  test('respects timeout', async ({ expect }) => {
    ...
    const [result1, result2] = planner.execute([
