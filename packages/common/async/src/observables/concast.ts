...
import { ZenObservable, Observer, ObservableSubscription, Subscriber } from './observable';
import { fixObservableSubclass } from './subclassing';

type MaybeAsync<T> = T | PromiseLike<T>;

const isPromiseLike = <T>(value: MaybeAsync<T>): value is PromiseLike<T> =>
  value && typeof (value as any).then === 'function';

// Any individual Source<T> can be an Observable<T> or a promise for one.
type Source<T> = MaybeAsync<ZenObservable<T>>;
...
export class Concast<T> extends ZenObservable<T> {
    ...
    complete: () => {
      ...
      if (sub !== null) {
        ...
        if (!value) {
          ...
          void value.then((obs) => (this.sub = obs.subscribe(this.handlers)));
        } else {
  ...
  private notify(method: Parameters<NextResultListener>[0], arg?: Parameters<NextResultListener>[1]) {
