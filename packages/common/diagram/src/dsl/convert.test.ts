...
describe('convert', () => {
  ...
  for (const [name, source] of Object.entries({ BASIC, CLASS_DIAGRAM })) {
    test(`${name} converts to DSL that parses back cleanly`, async ({ expect }) => {
      const text = await EffectEx.runPromise(convert(source.trim()));
  ...
  test('the emitted form is post-layout — every object carries an origin', async ({ expect }) => {
    const { commands } = parse(await EffectEx.runPromise(convert(BASIC.trim())));
  ...
  test('an explicit source overrides detection', async ({ expect }) => {
    const elk = await EffectEx.runPromise(convert(BASIC.trim()));
    const layered = await EffectEx.runPromise(convert(BASIC.trim(), { source: 'mermaid-layered' }));
  ...
  test('an unusable input fails with a typed error rather than throwing', async ({ expect }) => {
    await expect(EffectEx.runPromise(convert('not a diagram'))).rejects.toThrow(UndetectedSourceError);
    await expect(EffectEx.runPromise(convert(BASIC.trim(), { source: 'd2' }))).rejects.toThrow(UnknownSourceError);
  });
});
