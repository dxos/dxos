...
export type PatternReplacement = { from: number; to: number; text: string };

/**
 * Finds document spans to replace. Each pattern must include a capture group for the visible text.
 */
export const computePatternReplacements = (text: string, patterns: RegExp[]): PatternReplacement[] => {
  ...
  for (const pattern of patterns) {
    for (const match of text.matchAll(globalRegexp(pattern))) {
      ...
      replacements.push({ from: match.index, to: match.index + match[0].length, text: replacement });
    }
  }
 ...
 */
export const replacePatterns = (patterns: RegExp[]): Extension => {
  const buildDecorations = (text: string): DecorationSet => {
    ...
    for (const { from, to, text: replacement } of computePatternReplacements(text, patterns)) {
      builder.add(from, to, Decoration.replace({ widget: new ReplacementTextWidget(replacement) }));
    }
