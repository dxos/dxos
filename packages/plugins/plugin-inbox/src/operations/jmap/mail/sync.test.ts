...
describe('runJmapSync against a mock JMAP API', () => {
  ...
  test('syncs generated emails into the feed with contacts, threads, tags, and cursor', { timeout: 30_000 }, async ({
    expect,
  }) => {
  ...
  test('initial backward, incremental forward, and backfill (cursor stays put)', { timeout: 30_000 }, async ({
    ...
    const mid = generateJmapDataset({ count: 10, seed: 1, start: subDays(now, 13), end: subDays(now, 8), idPrefix: 'mid' });
    const recent = generateJmapDataset({ count: 8, seed: 2, start: subDays(now, 5), end: subDays(now, 2), idPrefix: 'new' });
    const older = generateJmapDataset({ count: 6, seed: 3, start: subDays(now, 25), end: subDays(now, 20), idPrefix: 'old' });
    ...
    const r1 = await EffectEx.runPromise(
      runJmapSync({ binding: Ref.make(binding), after: day(14) }).pipe(Effect.provide(inboxJmapSyncTestServices(db, mid))),
    );
