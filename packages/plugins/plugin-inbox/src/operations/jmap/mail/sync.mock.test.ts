...
// The JMAP sync driven end-to-end against a real ECHO db + a mock JMAP API — no live account. This is
// the seam mirroring Gmail's (`runJmapSync` requires `JmapMailApi` rather than providing the live
// HTTP client itself), so the whole pipeline — session, query, dedup, decode, map, contact/thread
// extraction, folder-tag application, commit, cursor advance — exercises against generated data.
