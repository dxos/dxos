...
// Benchmark: measures per-stage / per-message sync cost across dataset sizes to surface O(n²) growth
// (e.g. per-message commit/append time rising with N). Gated by `DX_BENCH` so it never runs in normal
// CI; sizes overridable via `DX_BENCH_SIZES=1000,5000,10000`. Run:
//
...
describe.runIf(process.env.DX_BENCH)('runGmailSync benchmark', () => {
  ...
  test('per-stage / per-message durations across dataset sizes', { timeout: 600_000 }, async ({ expect }) => {
    ...
    // rising column is the O(n²) signal Stage 6 targets.
    if (SIZES.length > 1) {
