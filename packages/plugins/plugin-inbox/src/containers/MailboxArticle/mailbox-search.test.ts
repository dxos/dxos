...
describe('buildMailboxSelection', () => {
  ...
  // The selection is always the whole-thread semi-join filter: `Filter.type(Message, { threadId:
  // Filter.in(subquery.project('threadId')) })`. These tests drill into the embedded subquery's
  // own filter (`ast.props.threadId.subquery.query.filter`) to assert the view-filter behavior
  // that previously lived at the top level.
  const viewFilterAst = (selection: ReturnType<typeof buildMailboxSelection>) =>
    (selection.ast as any).props.threadId.subquery.query.filter;
