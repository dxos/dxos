...
import {
	type MailboxActionHandler,
	Mailbox as MailboxComponent,
} from "./Mailbox";
...
export const MailboxContainer = ({
	...
	useEffect(
		() => setFilter(parser.build(filterText).filter),
		[filterText, parser],
	);
			...
			switch (action.type) {
				...
				case "select-tag": {
					setFilterText((prevFilterText) => {
						...
						return (
							[prevFilterText.trim(), "#" + action.label]
								.filter(Boolean)
								.join(" ") + " "
						);
					});
	...
	return (
		<StackItem.Content
			...
			classNames={[
				"relative grid",
				filterVisible.value
					? "grid-rows-[var(--toolbar-size)_min-content_1fr]"
					: "grid-rows-[var(--toolbar-size)_1fr]",
			]}
		>
						...
						>
							...
							<div role="none" className="flex shrink-0 gap-1 items-center">
								<IconButton
									...
									iconOnly
									onClick={() =>
										filter && handleAction({ type: "save", filter: filterText })
									}
								/>
								<IconButton
			...
			Atom.make((context) =>
				MenuBuilder.make()
					...
					.action(
						...
						{
							type: "sortDescending",
							icon: context.get(sortDescending)
