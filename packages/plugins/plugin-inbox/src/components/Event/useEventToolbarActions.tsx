    ...
    (get) =>
      MenuBuilder.make()
        ...
        .menu('more', (group) => {
          ...
          // `graphActions` returns a subgraph value (no side effect), so it must be merged via `subgraph`,
          // rooted at the `more` group so the contributed actions nest inside the overflow.
          group.subgraph(graphActions(graph, get, nodeId, { filter: isToolbarAction, rootId: 'more' }));
          if (onSave) {
            group.action(
              'save',
              {
