...
/** Email properties fetched by {@link emailGet} — enough for {@link mapEmail} to build a Message. */
...
/**
 * Creates a draft and submits it for delivery in a single batched request: `Email/set` creates the
 * draft (referenced as `#draft`), then `EmailSubmission/set` sends it and — via
 * `onSuccessUpdateEmail` — moves it from Drafts to Sent and clears the `$draft` keyword. Not retried
 * (a retry could double-send). Returns the created email's id and thread id.
 */
