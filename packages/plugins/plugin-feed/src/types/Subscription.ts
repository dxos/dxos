...
/** Creates a Subscription.Subscription with a backing ECHO feed. */
export const makeSubscription = (props: Omit<Obj.MakeProps<typeof Subscription>, 'feed'> = {}): Subscription => {
  const echoFeed = EchoFeed.make();
  const subscriptionFeed = Obj.make(Subscription, { feed: Ref.make(echoFeed), ...props });
