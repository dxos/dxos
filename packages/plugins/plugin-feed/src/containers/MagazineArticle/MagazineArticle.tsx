...
export const MagazineArticle = ({ role, subject, attendableId }: MagazineArticleProps) => {
  ...
  const id = attendableId ?? Obj.getEchoId(magazine);
  ...
  // Index feeds by bare object id (last DXN segment) — `Obj.getEchoId(feed)`
          ...
          <Masonry.Root Tile={TileAdapter} minColumnWidth={20} maxColumnWidth={25}>
            <Masonry.Content thin centered padding>
              <Masonry.Viewport
                classNames='py-2'
                items={tileItems}
                getId={(data) => (data?.post ? Obj.getEchoId(data.post) : '')}
              />
            </Masonry.Content>
          </Masonry.Root>
        )}
...
const useMagazinePosts = (
  ...
  return useMemo<Subscription.Post[]>(() => {
    ...
    for (const ref of subject.posts) {
      ...
      const dxn = Obj.getEchoId(target);
      if (
        seenDxn.has(dxn) ||
