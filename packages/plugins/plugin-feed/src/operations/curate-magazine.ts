 ...
 * The base methodology skill is referenced by its registry DXN (no clone into the space); the
...
const selectPostIds = (
  ...
  Effect.gen(function* () {
    ...
    // Resolve the base methodology skill from the registry by its key and hold it by value. A
    ...
    const skill = yield* Skill.resolve(Magazine.SKILL_KEY).pipe(Effect.option);
    const routine = Routine.make({
      input: CurationInput,
      output: CurationOutput,
      instructions: topic,
      skills: Option.toArray(skill).map((value) => Ref.make(value)),
    });
