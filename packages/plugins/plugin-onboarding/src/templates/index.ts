  ...
  SpaceTemplate.make({
    ...
    build: (phases) =>
      Effect.gen(function* () {
        ...
        yield* SpaceTemplate.collection('Welcome', [Ref.make(docs.tour), Ref.make(docs.about)]);
        yield* SpaceTemplate.collection('Spring Blend Launch', [
          Ref.make(notes.tastingProtocol),
          Ref.make(drawings.flavorWheel),
        ]);
        yield* SpaceTemplate.collection('Roastery Notes', [
