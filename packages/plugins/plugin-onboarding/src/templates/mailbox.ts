...
export const Inbox: SpaceTemplate.Phase<InboxResult, PersonMap> = SpaceTemplate.phase('inbox', {
  ...
  run: (people: PersonMap) =>
    Effect.gen(function* () {
      ...
      if (!index || !feed) {
        return yield* Effect.fail(new SpaceTemplate.SpaceTemplateError({ context: { reason: 'mailbox-incomplete' } }));
      }
      ...
      yield* SpaceTemplate.appendToFeed(feed, messages);
      yield* SpaceTemplate.appendToFeed(annotations, makeSummaries(messages));
