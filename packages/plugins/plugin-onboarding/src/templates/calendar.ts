...
export const Schedule: SpaceTemplate.Phase<ScheduleResult, ScheduleInput> = SpaceTemplate.phase('schedule', {
    ...
    Effect.gen(function* () {
      ...
      if (!feed) {
        return yield* Effect.fail(
          new SpaceTemplate.SpaceTemplateError({ context: { reason: 'calendar-feed-missing' } }),
        );
      }
      yield* SpaceTemplate.appendToFeed(feed, events);
      return { calendar, events };
    }),
});
