...
// The template's main character — the mailbox's primary user. Mail this person authored is
...
export const People: SpaceTemplate.Phase<PersonMap, OrgMap> = SpaceTemplate.phase('people', {
  schemas: [Person.Person],
  run: (organizations: OrgMap) =>
    SpaceTemplate.seed(PEOPLE_SEEDS, (seed) =>
      Database.add(
        Person.make({
