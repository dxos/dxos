...
- 881f900: `@dxos/plugin-google` and `@dxos/plugin-jmap` now publish to npm. Both were held back as private while the mail providers were extracted out of `@dxos/plugin-inbox`; they are stable enough to consume directly, and EDGE needs to resolve their operation handler sets to run mail sync remotely.
...
- 1e2a300: Lowered the mail sync per-run budget so a run's resident set fits the memory limit of the smallest host that runs it. Gmail and JMAP now consider 100 candidate messages per run instead of 500; a capped run still requests `Operation.runAgain()`, so a larger delta is drained across runs rather than lost.
