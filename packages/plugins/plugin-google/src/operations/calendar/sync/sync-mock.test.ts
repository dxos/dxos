...
/**
 * Google Calendar sync against a mocked API — the offline peer of the Gmail sync suite. The live-API
 * smoke tests in `sync.test.ts` still need `ACCESS_TOKEN`; these do not, so calendar sync is covered
 * in CI.
 */
...
describe('syncCalendar against a mock Google Calendar API', () => {
  ...
  test('an initial sync commits every event in the window and advances the cursor', async ({ expect }) => {
    ...
    const now = new Date();
    const { db, calendar, binding } = await seedCalendarBinding(builder);
    const dataset = generateCalendarDataset({ count: 4, now });

    const { newEvents } = await EffectEx.runPromise(
      syncCalendar({ binding: Ref.make(binding) }).pipe(Effect.provide(services(db, dataset))),
    );
  ...
  test('a re-run commits nothing new (dedup on event id)', async ({ expect }) => {
    ...
    await EffectEx.runPromise(syncCalendar({ binding: Ref.make(binding) }).pipe(Effect.provide(services(db, dataset))));
    const { newEvents } = await EffectEx.runPromise(
      syncCalendar({ binding: Ref.make(binding) }).pipe(Effect.provide(services(db, dataset))),
    );
  ...
  test('events outside the sync window are not fetched', async ({ expect }) => {
    ...
    const { newEvents } = await EffectEx.runPromise(
      syncCalendar({ binding: Ref.make(binding), syncForwardDays: 30, syncBackDays: 30 }).pipe(
        Effect.provide(services(db, dataset)),
      ),
    );
  ...
  test('an expanded recurring series collapses to one event on the initial sync', async ({ expect }) => {
    ...
    const { newEvents } = await EffectEx.runPromise(
      syncCalendar({ binding: Ref.make(binding), syncForwardDays: 60 }).pipe(Effect.provide(services(db, dataset))),
    );
  ...
  test('paging fetches every event across multiple pages', async ({ expect }) => {
    ...
    const { newEvents } = await EffectEx.runPromise(
      syncCalendar({ binding: Ref.make(binding), pageSize: 2 }).pipe(Effect.provide(services(db, dataset))),
    );
