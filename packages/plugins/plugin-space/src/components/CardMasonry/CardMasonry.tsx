...
export type CardMasonryProps = Pick<AppSurface.CardMasonryData, 'objects' | 'size' | 'inline' | 'onRemove' | 'pending'>;

type Tile =
  | { kind: 'object'; object: Obj.Unknown; ref: Ref.Ref<Obj.Unknown> }
  | { kind: 'pending'; pending: AppSurface.CardMasonryPending };

type TileActions = {
  onOpen: (subject: Entity.Unknown) => void;
  onRemove?: (ref: Ref.Ref<Obj.Unknown>) => void;
};

/** The masonry fixes a tile's props to its own signature, so the card actions reach tiles this way. */
const TileActionsContext = createContext<TileActions | undefined>(undefined);
 ...
 * body from the type's own `CardContent` surface, and a title that opens the object. A host may
 * offer removal, and placeholder cards for objects it is still adding.
 *
 ...
 */
export const CardMasonry = ({ objects: refs, size = 'default', inline, onRemove, pending = [] }: CardMasonryProps) => {
  ...
  const tilesAtom = useMemo(
    () =>
      Atom.make((get) =>
        refs
          .map((ref) => {
            const object = get(ref.atom);
            return object ? { kind: 'object' as const, object, ref } : undefined;
          })
          .filter(isNonNullable),
      ),
    [refs],
  );
  const resolved = useAtomValue(tilesAtom);
  const tiles = useMemo<Tile[]>(
    () => [...resolved, ...pending.map((entry) => ({ kind: 'pending' as const, pending: entry }))],
    [resolved, pending],
  );
  ...
  const handleOpen = useCallback(
    (subject: Entity.Unknown) => {
      if (Obj.isObject(subject)) {
        void invokePromise(LayoutOperation.Open, { subject: [GraphPath.getObjectPathFromObject(subject)] });
      }
    },
    [invokePromise],
  );
  const actions = useMemo<TileActions>(() => ({ onOpen: handleOpen, onRemove }), [handleOpen, onRemove]);
...
const CardMasonryTile = ({ data: tile }: { data: Tile }) => {
  const actions = useContext(TileActionsContext);
  if (tile.kind === 'pending') {
    return <PendingCard label={tile.pending.label} />;
  }

  const onRemove = actions?.onRemove;
  const { ref } = tile;
  return <ObjectCard data={tile.object} onOpen={actions?.onOpen} onRemove={onRemove && (() => onRemove(ref))} />;
};
