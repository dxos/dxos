...
export const CreateObjectDialog = ({
	...
	const [target, setTarget] = useState<Space | DataType.Collection | undefined>(
		initialTarget,
	);
	...
	const queryCollections = useQuery(
		space,
		Query.type(DataType.QueryCollection),
	);
	...
	const handleCreateObject = useCallback<
		NonNullable<CreateObjectPanelProps["onCreateObject"]>
	>(
		({ form, data = {} }) =>
			Effect.gen(function* () {
				if (!target) {
				...
				if (
					...
					const hidden =
						form.hidden ||
						hiddenTypenames.includes(Type.getTypename(form.objectSchema));
