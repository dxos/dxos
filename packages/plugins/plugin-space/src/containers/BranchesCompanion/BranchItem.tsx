...
export type BranchItemProps = {
  ...
  current: boolean;
  /** Whether the object's type supports a diff view (enables the compare control). */
  diffSupported?: boolean;
  /** Whether the article is currently showing a diff against this branch. */
  comparing?: boolean;
  /** Switch the subtree to this branch (no-op affordance when it is already current). */
  onSwitch: (name: string) => void;
  /** Toggle an inline diff of the current branch against this branch. */
...
/**
 * A single branch row: name plus switch / compare / merge / delete controls. Pure and presentational
 * — the affordance rules are intrinsic to the branch (`'main'` cannot be merged or deleted; the
 * current branch cannot be switched to, compared against, or deleted) so the row is self-describing
 * and the parent only wires handlers. The compare control appears only when the type supports diffs.
 */
      ...
      {diffSupported && (
        <ListItem.Endcap asChild>
          <IconButton
            ...
            icon={comparing ? 'ph--git-diff--fill' : 'ph--git-diff--regular'}
            label={t('branch-compare.label')}
            disabled={current}
