...
export default (context: PluginContext) => {
	const resolve = (get: Atom.Context) => (typename: string) =>
		get(context.capabilities(Capabilities.Metadata)).find(
			({ id }) => id === typename,
		)?.metadata ?? {};
	...
	const spacesNode = {
		...
		properties: {
			...
			onRearrangeChildren: async (nextOrder: Space[]) => {
				...
				const {
					objects: [spacesOrder],
				} = await client.spaces.default.db
					.query(Filter.type(Type.Expando, { key: SHARED }))
					.run();
				if (spacesOrder) {
			...
			actions: (node) =>
				Atom.make((get) =>
					Function.pipe(
						get(node),
						Option.flatMap((node) =>
							node.id === ROOT_ID ? Option.some(node) : Option.none(),
						),
						Option.map(() => [
							{
							...
							},
							{
								...
								data: async () => {
									...
									const client = context.getCapability(
										ClientCapabilities.Client,
									);
									const space =
										getActiveSpace(context) ?? client.spaces.default;
									await dispatch(
										createIntent(SpaceAction.OpenMembers, { space }),
									);
								},
								properties: {
			...
			connector: (node) => {
				...
				return Atom.make((get) =>
					Function.pipe(
						get(node),
						Option.flatMap((node) =>
							node.id === SPACES ? Option.some(node) : Option.none(),
						),
						Option.map(() => {
							...
							const settings = get(
								context.capabilities(Capabilities.SettingsStore),
							)[0]?.getStore<SpaceSettingsProps>(meta.id)?.value;

							// TODO(wittjosiah): During client reset, accessing default space throws.
							try {
								if (!query) {
									query = client.spaces.default.db.query(
										Filter.type(Type.Expando, { key: SHARED }),
									);
								}
									...
									rxFromSignal(() => {
										...
										return [
											...
											.filter((space) =>
												settings?.showHidden
													? true
													: space.state.get() !== SpaceState.SPACE_INACTIVE,
											)
											.map((space) =>
												constructSpaceNode({
													space,
			...
			actions: (node) =>
				Atom.make((get) =>
					Function.pipe(
						get(node),
						Option.flatMap((node) =>
							node.type === SPACE_TYPE && isSpace(node.data)
								? Option.some(node.data)
								: Option.none(),
						),
						Option.flatMap((space) => {
							const [dispatcher] = get(
								context.capabilities(Capabilities.IntentDispatcher),
							);
							const [client] = get(
								context.capabilities(ClientCapabilities.Client),
							);
							const [state] = get(
								context.capabilities(SpaceCapabilities.State),
							);
					...
					Function.pipe(
						...
						Option.map((space) => {
							...
							const collection = get(
								rxFromSignal(
									() =>
										space.properties[Collection.Collection.typename]?.target as
				...
				return Atom.make((get) =>
					Function.pipe(
						...
						Option.flatMap((collection) => {
							...
							return typename && space
								? Option.some({ typename, space })
								: Option.none();
						}),
			...
			connector: (node) => {
				...
				return Atom.make((get) =>
					Function.pipe(
						...
						Option.flatMap((collection) => {
							const space = getSpace(collection);
							return space?.properties.staticRecords
								? Option.some(space)
								: Option.none();
						}),
						Option.map((space) => {
							return get(
								rxFromSignal(
									() => (space.properties.staticRecords ?? []) as string[],
								),
							)
								.map((typename) =>
									client.graph.schemaRegistry.schemas.find(
			...
			actions: (node) => {
				...
				return Atom.make((get) =>
					Function.pipe(
						get(node),
						Option.flatMap((node) => {
							const space = isSpace(node.properties.space)
								? node.properties.space
								: undefined;
							return space && Schema.isSchema(node.data)
						...
						Option.map(({ space, schema }) => {
							...
							const filteredViews = get(
								rxFromSignal(() =>
									...
									views.filter(
										(view) =>
											getTypenameFromQuery(view.query.ast) ===
				...
				return Atom.make((get) =>
					Function.pipe(
						...
						Option.flatMap((node) => {
							const space =
								getSpace(node.data) ??
								(isSpace(node.properties.space)
									? node.properties.space
									: undefined);
							return space &&
								(Obj.instanceOf(StoredSchema, node.data) ||
									Schema.isSchema(node.data))
								? Option.some({ space, schema: node.data })
								: Option.none();
						}),
						Option.map(({ space, schema }) => {
							...
							return (
								get(rxFromQuery(query))
									.filter(
										(view) => getTypenameFromQuery(view.query.ast) === typename,
									)
				...
				return Atom.make((get) =>
					Function.pipe(
						...
						Option.flatMap(({ space, object }) => {
							...
							const [appGraph] = get(
								context.capabilities(Capabilities.AppGraph),
							);
							const [state] = get(
								context.capabilities(SpaceCapabilities.State),
							);
							const objectForms = get(
							...
							if (!dispatcher || !appGraph || !state) {
								...
								return Option.some({
									...
									deletable,
									navigable: get(
										rxFromSignal(() => state.navigableCollections),
									),
								});
							}
						}),
			...
			connector: (node) =>
				Atom.make((get) =>
					Function.pipe(
						...
						Option.map((node) => [
							{
								id: [node.id, "selected-objects"].join(
									ATTENDABLE_PATH_SEPARATOR,
								),
