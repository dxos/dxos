...
export const handler = (args: ExportArgs) =>
  exportSpaceToFile({ ...args, format: SpaceArchive.Format.JSON, label: 'Snapshot' });

export const snapshot = Command.make(
  'snapshot',
  {
...
).pipe(Command.withDescription('Write a JSON snapshot of a space to disk.'));
