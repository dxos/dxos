...
export const printMember = (member: SpaceMember) =>
  FormBuilder.make({
    title:
      ...
      (member.identity?.identityKey ? decodePublicKey(member.identity.identityKey).truncate() : undefined) ??
      '<unknown>',
  }).pipe(
    FormBuilder.set(
      'identityKey',
      member.identity?.identityKey ? decodePublicKey(member.identity.identityKey).truncate() : '',
    ),
