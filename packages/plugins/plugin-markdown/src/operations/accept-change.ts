...
import { createDocAccessor, getObjectOnBranch, getRangeFromCursor } from '@dxos/echo-client';
  ...
  CollaborationOperation.AcceptChange.pipe(
    Operation.withHandler(
      Effect.fnUntraced(function* ({ subject, anchor, branch }) {
        ...
        // Pull the LATEST compare-branch text and recompute the diff (not a stored snapshot).
        const compareData = yield* Effect.promise(() => getObjectOnBranch(content, branch));
