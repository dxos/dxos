...
type MarkdownEditorContextValue = {
	...
	Pick<
		...
		"editorView" | "onFileUpload" | "onViewModeChange"
	>);
...
type MarkdownEditorRootProps = PropsWithChildren<
	{
		...
		"id" | "onFileUpload" | "onViewModeChange" | "viewMode"
	> &
		Pick<UsePopoverMenuOptionsProps, "slashCommandGroups" | "onLinkQuery"> &
		Pick<
			ExtensionsOptions,
			"editorStateStore" | "selectionManager" | "settings"
		>
>;
		...
		() => ({
			...
			removeBlockContainer: ({ link }) => {
				setPreviewBlocks((prev) =>
					prev.filter(({ link: prevLink }) => prevLink.ref !== link.ref),
				);
			},
		}),
		[],
...
type MarkdownEditorMainProps = Omit<
	NaturalMarkdownEditorMainProps,
	"id" | "extensions" | "toolbarState"
>;
...
const MarkdownEditorToolbar = (props: MarkdownEditorToolbarProps) => {
	...
	return (
		<NaturalMarkdownToolbar {...props} {...rootProps} state={toolbarState} />
	);
};
...
const PreviewBlock = ({ el, link }: PreviewBlock) => {
	...
	return createPortal(
		<Surface role="card--transclusion" data={data} limit={1} />,
		el,
	);
};
