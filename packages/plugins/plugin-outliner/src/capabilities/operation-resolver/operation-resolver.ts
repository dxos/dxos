    ...
    Capability.contributes(Capabilities.OperationResolver, [
      OperationResolver.make({
        ...
        handler: Effect.fnUntraced(function* ({ rootCollection }) {
          const journalCollection = ManagedCollection.makeManagedCollection({ key: Type.getTypename(Journal.Journal) });
          const outlineCollection = ManagedCollection.makeManagedCollection({ key: Type.getTypename(Outline.Outline) });
