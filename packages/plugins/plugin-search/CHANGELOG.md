...
- 003f2d8: Open search with ⌘K / Ctrl+K and the command palette with ⇧⌘K / Ctrl+Shift+K, swapping the two shortcuts so the more frequently used search takes the primary binding.
- 8a8c633: The Search deck companion no longer goes missing for a whole session when the app graph builds before the client finishes initializing. The connector now builds the companion with no space until initialization completes, then re-evaluates and resolves the active space.
