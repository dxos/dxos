...
export const meta = Plugin.makeMeta({
  key: DXN.make('org.dxos.plugin.extension', '0.8.3'),
  name: 'Extension',
  author: 'DXOS',
  description: trim`
    ...
    switch that lets plugins use the extension to render pages, the per-render timeout, whether to
    render in a focused tab (some sites defer loading when backgrounded), and a developer-mode flag
    that enables verbose logging and debug previews. Settings are persisted to the app-framework
    settings store and exposed to other plugins via a plugin-scoped capability.
  `,
