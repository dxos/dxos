...
export class ChatRoom extends Server<Env> {
  ...
  async trackPeakUserCount() {
    const meetingId = await this.getMeetingId();
    const meeting = meetingId ? await this.getMeeting(meetingId) : await this.createMeeting();
    ...
    if (this.db) {
      ...
      if (meeting.ended !== null) {
        await this.db.update(Meetings).set({ ended: null }).where(eq(Meetings.id, meeting.id));
      }
  ...
  async getMeeting(meetingId: string) {
    ...
    const [meeting] = await this.db.select().from(Meetings).where(eq(Meetings.id, meetingId));
  ...
  async onMessage(connection: Connection<User>, message: WSMessage): Promise<void> {
    try {
      const meetingId = await this.getMeetingId();
      ...
      switch (data.type) {
        case 'userLeft': {
          ...
          await this.ctx.storage.delete(`session-${connection.id}`).catch(() => {
            console.warn(`Failed to delete session session-${connection.id} on userLeft`);
          });
        ...
        case 'directMessage': {
          ...
          const fromUser = await this.ctx.storage.get<User>(`session-${connection.id}`);
          ...
          console.warn(`User with id "${to}" not found, cannot send DM from "${fromUser!.name}"`);
          break;
        }
        case 'muteUser': {
          ...
          for (const otherConnection of this.getConnections<User>()) {
            if (otherConnection.id === data.id) {
              const otherUser = await this.ctx.storage.get<User>(`session-${data.id}`);
          ...
          if (!mutedUser) {
            console.warn(`User with id "${data.id}" not found, cannot mute user from "${user!.name}"`);
          }
          break;
        }
  ...
  async cleanupOldConnections() {
    const meetingId = await this.getMeetingId();
    if (!meetingId) {
      log({ eventName: 'meetingIdNotFoundInCleanup' });
    }
    const connections = [...this.getConnections()];
    const sessionsToCleanUp = await this.getUsers();

    connections.forEach((connection) => sessionsToCleanUp.delete(`session-${connection.id}`));
