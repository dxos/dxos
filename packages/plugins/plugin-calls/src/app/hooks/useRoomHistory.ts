...
export const useRoomHistory = (peer: RxjsPeer, room: ReturnType<typeof useRoom>): RoomHistory => {
  const [apiHistory, setApiHistory] = useState<ApiHistoryEntry[]>([]);
  const [userSessionsWithoutTraceLinks, setUserSessions] = useState<UserSession[]>([]);
  ...
  useEffect(() => {
    room.otherUsers.forEach((user) => {
      if (user.transceiverSessionId === undefined) {
        return;
      }
      if (sessionIdsRef.current.has(user.transceiverSessionId)) {
        return;
      }
