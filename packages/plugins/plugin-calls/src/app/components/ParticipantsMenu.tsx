  ...
  return (
    <li className='flex items-center gap-4 text-base h-9'>
      <div className='mr-auto overflow-hidden whitespace-nowrap text-ellipsis'>
        <OptionalLink
          href={user.transceiverSessionId ? populateTraceLink(user.transceiverSessionId, traceLink) : undefined}
          target='_blank'
...
export const participantCount = (count: number) => `${count} Participant${count > 1 ? 's' : ''}`;
      ...
      <Portal>
        ...
        <DialogContent>
          <div className='space-y-4'>
            <h2 className='text-xl font-bold'>{participantCount(otherUsers.length + 1)}</h2>
            <ul className='space-y-2'>
              {identity && (
                <UserListItem user={identity} audioTrack={userMedia.audioStreamTrack}>
                  {identity.name}
                </UserListItem>
              )}
              {otherUsers.map((u) => (
