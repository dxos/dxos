 ...
 * find the device id.
 */
export const getUserMediaExtended = async (constraints?: MediaStreamConstraintsWithLabels) => {
  ...
  if (typeof audio === 'object') {
    ...
    const foundDevice =
      devices.find((d) => d.deviceId === deviceId) ?? devices.find((d) => d.label === label && d.kind === 'audioinput');
  ...
  if (typeof video === 'object') {
    ...
    const foundDevice =
      devices.find((d) => d.deviceId === deviceId) ?? devices.find((d) => d.label === label && d.kind === 'videoinput');
