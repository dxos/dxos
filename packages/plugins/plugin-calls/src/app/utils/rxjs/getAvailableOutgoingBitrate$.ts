...
export const getAvailableOutboundEstimate$ = (peerConnection$: Observable<RTCPeerConnection>) => {
    ...
    map((newStatsReport) => {
      newStatsReport.forEach((report) => {
        if (report.type === 'candidate-pair' && 'availableOutgoingBitrate' in report) {
          availableOutboundBitrateEwma.insert(Number(report.availableOutgoingBitrate));
        }
      });
