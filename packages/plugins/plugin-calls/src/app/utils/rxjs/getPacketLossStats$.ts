...
export const getPacketLossStats$ = (peerConnection$: Observable<RTCPeerConnection>) => {
  ...
  return combineLatest([peerConnection$, interval(1000)]).pipe(
    ...
    map(([previousStatsReport, newStatsReport]) => {
      ...
      if (inboundPacketsReceived > 0) {
        ...
        inboundPacketLossPercentageEwma.insert(Math.max(0, packetLossPercentage));
      }

      if (outboundPacketsSent > 0) {
        const packetLossPercentage = outboundPacketsLost / (outboundPacketsSent + outboundPacketsLost);
        outboundPacketLossPercentageEwma.insert(Math.max(0, packetLossPercentage));
      }
