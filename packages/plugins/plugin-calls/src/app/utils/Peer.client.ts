...
const logCallsApiUsage = (message: string) => {
  console.debug(`%c 📞 Calls API: ${message}`, 'color: orange; background: black;');
};
...
export default class Peer {
  ...
  pushTrackDispatcher: BulkRequestDispatcher<PushTrackRequestEntry, TracksResponse>;
  ...
  static async #sendRequest(
    ...
    try {
      ...
      if (remainingAttempts === 0) {
        ...
        return Peer.#sendRequest(method, request, history, endpoint, extraParams, remainingAttempts - 1);
      }
    }
  }
  ...
  async #init() {
    ...
    this.transceivers.push(this.pc.addTransceiver('audio', { direction: 'inactive' }));
    ...
    await this.pc.setRemoteDescription(new RTCSessionDescription(response.sessionDescription));
    await connectedState;
  }

  #getTransceiverFor(track: MediaStreamTrack, sendEncodings?: RTCRtpEncodingParameters[]) {
  ...
  async pushTrack(
    ...
    const trackResponse = bulkResponse.tracks?.find((track) => track.trackName == trackName);
    if (!trackResponse) {
      throw new Error(`No response for trackName=${trackName}`);
    }
    if (trackResponse.errorCode) {
      throw new Error(`${trackResponse.errorCode}: ${trackResponse.errorDescription}`);
    }
  ...
  async pullTrack(trackObject: TrackObject): Promise<MediaStreamTrack> {
    console.debug(`Peer.pullTrack: ${trackObject.trackName} from peer ${trackObject.sessionId}`);
    await this.initialization;
    ...
    const trackResponseIdx = bulkResponse.tracks?.findIndex(
      (track) => track.sessionId == trackObject.sessionId && track.trackName == trackObject.trackName,
    );
    if (!bulkResponse.tracks || trackResponseIdx === undefined || trackResponseIdx == -1) {
      throw new Error(`No response for sessionId=${trackObject.sessionId}, trackName=${trackObject.trackName}`);
    }
  ...
  async closeTrack(track: MediaStreamTrack) {
    ...
    await this.initialization;
    const bulkResponse = await this.closeTrackDispatcher.doBulkRequest(mid, async (batchCopy: string[]) => {
      return await this.taskScheduler.schedule(async () => {
    ...
    const trackResponse = bulkResponse.tracks?.find((track) => track.mid === mid);
    if (!trackResponse) {
    ...
    this.transceivers = this.transceivers.filter((transceiver) => transceiver.mid != mid);
  }
  ...
  checkStats = async (timeout: number) => {
    ...
    now.forEach((nowReport: Stats) => {
      if (nowReport.type === 'candidate-pair' && 'availableOutgoingBitrate' in nowReport) {
        this.availableOutboundBitrate.insert(Number(nowReport.availableOutgoingBitrate));
      }
      ...
      const packetsLost = Math.max((nowReport.packetsLost || 0) - (baseReport.packetsLost || 0), 0);

      if (nowReport.type === 'remote-inbound-rtp' && nowReport.fractionLost !== undefined) {
  ...
  destroy() {
    this.pc.removeEventListener('iceconnectionstatechange', this.handleIceFailure);
    this.pc.close();
  }
}
