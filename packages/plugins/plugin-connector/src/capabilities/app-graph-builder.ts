  ...
  const requestProviders = Effect.suspend(() => {
    ...
    return Effect.forkDetach(
      pluginManager
        ...
        .pipe(
          Effect.catch((error) => Effect.sync(() => log.warn('connector activation failed', { error: String(error) }))),
        ),
    );
  });
...
export const ConnectorAppGraphBuilder = AppCapability.appGraphBuilder(activate, {
