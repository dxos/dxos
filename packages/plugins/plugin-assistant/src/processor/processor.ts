...
export class AiChatProcessor {
	...
	private readonly _streaming = Rx.make<
		Option.Option<DataType.Message.Message>
	>(Option.none());

	/** Streaming state. */
	public readonly streaming = Rx.make<boolean>((get) =>
		Option.isSome(get(this._streaming)),
	);
	...
	constructor(
		...
		if (this._options.model && !this._options.system) {
			const capabilities =
				this._options.modelRegistry?.getCapabilities(this._options.model) ?? {};
	...
	async request(requestParam: AiRequest): Promise<void> {
		...
		try {
			...
			this._fiber = request.pipe(
				Effect.provide(
					AiService.model(this._options.model ?? DEFAULT_EDGE_MODEL),
				),
			...
			if (
				!Exit.isSuccess(response) &&
				!Cause.isInterruptedOnly(response.cause)
			) {
			...
			this._rx.set(
				this.error,
				Option.some(new Error("AI service error", { cause: error })),
			);
		} finally {
			this._fiber = undefined;
