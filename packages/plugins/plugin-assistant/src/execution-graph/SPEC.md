...
| `Trace.OperationEnd` (incomplete) | `outcome === 'failure' && (errorCode === RUN_AGAIN_ERROR_CODE \|\| (errorCode === undefined && error === RUN_AGAIN_MESSAGE))` | `ph--arrows-clockwise--regular` | `WARN`    | `${name ?? key} - Incomplete`   | `${data.key}:end`   |
