...
import { Effect, Option, Stream, type Layer } from 'effect';
...
export class ChatProcessor {
  ...
  private readonly __session = Rx.make<AiSession | undefined>(undefined);
  ...
  readonly __streaming: Rx.Rx<Result.Result<Option.Option<DataType.Message>, Error>> = Rx.make((get) => {
    ...
    return session
      ? Stream.fromQueue(session.blockQueue).pipe(
          Stream.scan<Option.Option<DataType.Message>, Option.Option<ContentBlock.Any>>(
            Option.none(),
            (acc, blockOption) =>
                  ...
                  Option.match({
                    ...
                    onSome: (message) =>
                      Obj.make(DataType.Message, {
                        created: new Date().toISOString(),
                        sender: { role: 'assistant' },
                        blocks: [...message.blocks.filter((b) => !b.pending), block],
                      }),
                  }),
                  Option.some,
                ),
  ...
  async request(message: string, options: RequestOptions = {}): Promise<DataType.Message[]> {
    ...
    this.__registry.set(this.__session, session);
