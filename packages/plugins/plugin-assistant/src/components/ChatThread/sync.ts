 ...
 * Update document.
 */
export type TextModel = Pick<
	MarkdownStreamController,
	"reset" | "append" | "updateWidget"
>;
...
export class MessageThreadContext
	implements Pick<MarkdownStreamController, "updateWidget">
{
	...
	sync(messages: DataType.Message.Message[]) {
		...
		for (const message of messages.slice(this._currentMessageIndex)) {
			...
			for (const block of message.blocks.slice(this._currentBlockIndex)) {
				...
				const currentBlockContent = this._blockRenderer(
					this._context,
					message,
					block,
				);
				if (currentBlockContent) {
					let content: string = "";
					if (
						this._currentBlockContent &&
						currentBlockContent.startsWith(this._currentBlockContent)
					) {
						content = currentBlockContent.slice(
							this._currentBlockContent.length,
						);
					} else {
