...
const handler: Operation.WithHandler<typeof SyncPosts> = SyncPosts.pipe(
  Operation.withHandler(
    Effect.fnUntraced(function* ({ publication: publicationRef, connection, publisherId }) {
      ...
      // push reconciliation (cold refs would otherwise push an empty body).
      const posts = yield* Effect.forEach(publication.posts ?? [], (ref) => Database.load(ref));
      yield* Effect.forEach(posts, (post) =>
        Database.load(post.content).pipe(Effect.flatMap((doc) => Database.load(doc.content))),
      );
      const result = yield* runSyncPosts(service, publication, connection);
