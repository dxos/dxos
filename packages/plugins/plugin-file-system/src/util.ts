...
// Convert a single DirEntry into a FileSystemCapabilities.FileSystemEntry, reading children/content as needed.
const processEntry = (
  entry: DirEntry,
  parentPath: string,
): Effect.Effect<FileSystemCapabilities.FileSystemEntry | null> =>
  Effect.gen(function* () {
    if (entry.isSymlink) {
      return null;
    }
    ...
    if (entry.isDirectory) {
      ...
      return {
        ...
        path: entryPath,
        children,
      } satisfies FileSystemCapabilities.FileSystemDirectory;
    }
    ...
    return {
      ...
      modified: false,
      type: 'markdown',
    } satisfies FileSystemCapabilities.FileSystemFile;
  });

// Recursively read a directory tree, sorting directories before files.
const readDirectoryContents = (path: string): Effect.Effect<FileSystemCapabilities.FileSystemEntry[]> =>
  Effect.gen(function* () {
    ...
    return processedEntries
      .filter((entry): entry is FileSystemCapabilities.FileSystemEntry => entry !== null)
      .sort((entryA, entryB) => {
...
export const loadWorkspace = (path: string): Effect.Effect<FileSystemCapabilities.FileSystemWorkspace | null> => {
  if (!isTauriAvailable()) {
    return Effect.sync(() => {
      log.warn('Tauri APIs not available');
      return null;
...
export const refreshWorkspace = (
  workspace: FileSystemCapabilities.FileSystemWorkspace,
): Effect.Effect<FileSystemCapabilities.FileSystemWorkspace | null> => {
...
const findFileInWorkspace = (
  workspace: FileSystemCapabilities.FileSystemWorkspace,
  fileId: string,
): FileSystemCapabilities.FileSystemFile | undefined => {
  const searchEntries = (
...
export const findFileById = (
  workspaces: FileSystemCapabilities.FileSystemWorkspace[],
  fileId: string,
):
  | { workspace: FileSystemCapabilities.FileSystemWorkspace; file: FileSystemCapabilities.FileSystemFile }
  | undefined => {
...
export const findDirectoryById = (
  workspaces: FileSystemCapabilities.FileSystemWorkspace[],
  directoryId: string,
): { directory: FileSystemCapabilities.FileSystemDirectory; workspaceId: string } | undefined => {
  for (const workspace of workspaces) {
...
export const findDirectoryInEntries = (
  entries: FileSystemCapabilities.FileSystemEntry[],
  directoryId: string,
): FileSystemCapabilities.FileSystemDirectory | undefined => {
  for (const entry of entries) {
...
export const updateFileInWorkspace = (
  workspace: FileSystemCapabilities.FileSystemWorkspace,
  fileId: string,
  updates: Partial<FileSystemCapabilities.FileSystemFile>,
): FileSystemCapabilities.FileSystemWorkspace => {
  const updateEntries = (
    entries: FileSystemCapabilities.FileSystemEntry[],
