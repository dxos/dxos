...
export class MirrorSpaceManager {
  ...
  getOrCreateSpace(workspace: FileSystemCapabilities.FileSystemWorkspace): Effect.Effect<Space> {
    return Effect.promise(() => this._getOrCreateSpaceAsync(workspace));
  }

  private async _getOrCreateSpaceAsync(workspace: FileSystemCapabilities.FileSystemWorkspace): Promise<Space> {
  ...
  private async _doCreateSpace(workspace: FileSystemCapabilities.FileSystemWorkspace): Promise<Space> {
    ...
    if (config.spaceId) {
      ...
      if (existing && existing.tags.includes(FILE_SYSTEM_MIRROR_TAG)) {
