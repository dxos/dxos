...
export type MarkdownDocuments = {
  ...
  syncFromDisk: (workspace: FileSystemCapabilities.FileSystemWorkspace) => Effect.Effect<void>;
  /** Evict all cached documents and file watchers for a workspace. */
  evictForWorkspace: (workspace: FileSystemCapabilities.FileSystemWorkspace) => void;
};
...
export const createMarkdownDocuments = (
  ...
  const applyExternalDiskText = (fileId: string, path: string): Effect.Effect<void> =>
    Effect.gen(function* () {
      ...
      registry.update(stateAtom, (currentState: FileSystemCapabilities.FileSystemState) => {
        ...
        return {
          ...currentState,
          workspaces: currentState.workspaces.map((ws: FileSystemCapabilities.FileSystemWorkspace) =>
   ...
   * immediately.
   */
  const startFileWatcher = (file: FileSystemCapabilities.FileSystemFile): Effect.Effect<void> =>
    Effect.gen(function* () {
  ...
  const ensureFileWatcher = (file: FileSystemCapabilities.FileSystemFile): void => {
   ...
   */
  const persistFileIdentity = (file: FileSystemCapabilities.FileSystemFile, workspaceId: string, uri: string): void => {
    const state = registry.get(stateAtom);
   ...
   */
  const ensureDocumentForFile = (file: FileSystemCapabilities.FileSystemFile, workspaceId: string): Text.Text => {
   ...
   */
  const restoreWorkspaceDocuments = (workspace: FileSystemCapabilities.FileSystemWorkspace): Effect.Effect<void> =>
    Effect.gen(function* () {
