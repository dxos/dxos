...
export type DirectoryWatcher = {
  startWatching: (workspace: FileSystemCapabilities.FileSystemWorkspace) => Effect.Effect<void>;
...
export const createDirectoryWatcher = (onRefresh: (workspaceId: string) => Effect.Effect<void>): DirectoryWatcher => {
  ...
  const startWatching = (workspace: FileSystemCapabilities.FileSystemWorkspace): Effect.Effect<void> =>
    Effect.gen(function* () {
