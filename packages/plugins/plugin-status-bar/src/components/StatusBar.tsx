...
const StatusBarButton = forwardRef<HTMLButtonElement, StatusBarButtonProps>(
  ({ classNames, children, asChild, ...props }, forwardedRef) => {
    const classes = mx(
      'flex items-center justify-center gap-2 p-1 pli-2 rounded-xs',
...
const StatusBarItem = forwardRef<HTMLDivElement, StatusBarItemProps>(
  ({ classNames, children, ...props }, forwardedRef) => (
    <div
      role='menuitem'
      className={mx('flex items-center justify-center gap-2 p-1 pli-2 rounded-xs select-none', classNames)}
      ref={forwardedRef}
      {...props}
    >
      {children}
