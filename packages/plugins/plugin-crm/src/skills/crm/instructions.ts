...
/**
 * Agent instructions for the CRM skill. Composable with the research,
 * web-search, database, and markdown skills — the agent is expected
 * to invoke their tools directly.
 *
 * Edit the prose here (or through the skill editor) to tune research
...
export const makeInstructions = (researchSources: ReadonlyArray<ResearchSource> = []): string => {
  ...
  return trim`
    ...
    <research>
      Use the research skill's \`Research\` tool to gather public
      ...
      When the web-search skill is available, use its \`Fetch\` tool and
    ...
    <profile_document>
      Create a markdown Profile document using the research skill's
    ...
    <reply>
      ...
      format (refer to the research skill's reply conventions).
    </reply>
  `;
};
