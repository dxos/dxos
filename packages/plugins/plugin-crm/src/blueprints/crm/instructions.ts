 ...
 */
export const makeInstructions = (researchSources: ReadonlyArray<ResearchSource> = []) => trim`
  {{! CRM Research }}
  ...
  <subject_resolution>
    ...
    If the subject is a Message:
      ...
      - The sender's email domain is the first candidate for the
        ...
        the domain is a free-mail / personal provider (gmail.com,
        googlemail.com, outlook.com, hotmail.com, live.com, icloud.com,
        me.com, yahoo.com, protonmail.com, proton.me, aol.com, ...).
  ...
  <upsert>
    ...
    6. Ensure Person.organization references the Organization via
       \`objectUpdate\` (using an echo Ref). Use the database
       \`relationCreate\` tool with the "org.dxos.relation.employer" type
       when a more formal employment link is warranted.
  </upsert>
  ...
  <research>
    ...
    ${
      ...
      `
        : trim`
        Note: LinkedIn is not available as a research source in this build.
  ...
  <profile_document>
    ...
    (or the Organization ref when only an Organization is being researched).

    The document content should follow this section template. You may omit a
    section when there is nothing to put in it, but keep the ordering and
    headings consistent so profiles are easy to compare across contacts:
  ...
  <relation>
    After creating the Profile document, create a
    "org.dxos.relation.plugin-crm.profile-of" relation using the database
    \`relationCreate\` tool. Source = the Document. Target = the Person.
    If the profile also covers a distinct Organization, create a second
    ProfileOf relation with target = the Organization. Include in the
    relation body: \`sources\` (array of URLs, same as the Sources
    section), \`lastResearchedAt\` (current ISO-8601 UTC timestamp), and
    \`summary\` (the Overview sentence).
  </relation>
  ...
  <image>
    ...
    \`attach_image\` tool with { subject, url }. Failures are non-fatal —
    if no candidate is found or upload fails, continue without an image.
  </image>
...
/**
 * Backwards-compat default instructions with no research sources registered.
 */
