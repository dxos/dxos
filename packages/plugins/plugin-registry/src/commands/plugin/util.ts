...
export const printPlugin = (plugin: FormattedPlugin): Doc.Doc<any> => {
  const builder = FormBuilder.make({ title: plugin.name }).pipe(
  ...
  return (plugin.failure ? builder.pipe(FormBuilder.set('error', plugin.failure)) : builder).pipe(FormBuilder.build);
};
