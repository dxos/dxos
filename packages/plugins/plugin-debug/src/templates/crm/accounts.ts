...
export const Accounts: SpaceTemplate.Phase<AccountMap> = SpaceTemplate.phase('accounts', {
  schemas: [Organization.Organization],
  run: () =>
    SpaceTemplate.seed(ACCOUNT_SEEDS, (seed) =>
      Database.add(
        Organization.make({
          name: seed.name,
...
export const Contacts: SpaceTemplate.Phase<ContactMap, AccountMap> = SpaceTemplate.phase('contacts', {
  schemas: [Person.Person],
  run: (accounts: AccountMap) =>
    SpaceTemplate.seed(CONTACT_SEEDS, (seed) =>
      Database.add(
        Person.make({
