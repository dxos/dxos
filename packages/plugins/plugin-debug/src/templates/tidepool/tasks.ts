 ...
 * are still the set's flat `tasks` array, which is what `SpaceTemplate.children` writes.
 */
export const Tasks: SpaceTemplate.Phase<TasksResult, PersonMap> = SpaceTemplate.phase('tasks', {
  ...
  run: (people: PersonMap) =>
    Effect.gen(function* () {
      ...
      const milestones = yield* SpaceTemplate.seed(MILESTONE_SEEDS, (seed) =>
        Effect.succeed(Milestone.make({ name: seed.name, description: seed.description })),
      );
      yield* SpaceTemplate.children(taskSet, Object.values(milestones), (taskSet, refs) => {
