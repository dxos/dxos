 ...
 * runs with.
 *
 * The other space templates depict a project mid-flight; this one is meant to be RUN. Every task is
  ...
  SpaceTemplate.make({
    ...
    build: (phases) =>
      Effect.gen(function* () {
        ...
        yield* SpaceTemplate.collection('Documents', [Ref.make(docs.brief)]);
      }),
  });
