...
/** Applies a template by label, then waits for the object tree to show something it built. */
const applyTemplate = async (canvasElement: HTMLElement, label: string, expected: string) => {
  ...
  // `getAllByText`: a template may name two objects the same (Tidepool's task set and its
...
const meta = {
  title: 'plugins/plugin-debug/stories/SpaceTemplates',
  render: DefaultStory,
  decorators: [
...
/** Empty space plus the picker — click a template to fill it. */
export const Default: Story = {};

export const Tidepool: Story = {
  play: async ({ canvasElement }) => {
    await applyTemplate(canvasElement, 'Tidepool — Offline sync v2', 'Offline sync v2');
  },
};

export const Northwind: Story = {
  play: async ({ canvasElement }) => {
    await applyTemplate(canvasElement, 'Northwind Sales', 'Sales pipeline');
  },
};
