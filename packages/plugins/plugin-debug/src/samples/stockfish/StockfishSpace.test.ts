...
import * as StockfishSpace from './StockfishSpace.ts';
 ...
 */
describe('Chess MCP template', () => {
  test(
    'builds an archive with the project, its plan, its skill and the test position',
    { timeout: 120_000 },
    async ({ expect }) => {
      const { json, objectCount } = await EffectEx.runPromise(buildArchive(StockfishSpace.make()));
      ...
      const declared = new Set(StockfishSpace.make().schemas.map((schema) => Type.getTypename(schema)));
  ...
  test('the plan is one root, five stages, and their steps', { timeout: 120_000 }, async ({ expect }) => {
    const { json } = await EffectEx.runPromise(buildArchive(StockfishSpace.make()));
    const objects: Array<{
      '@type'?: string;
      'id': string;
      'title'?: string;
