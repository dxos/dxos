...
export type MigrateCanvas = (current: SerializedStore<TLRecord>, version?: number) => Promise<SerializedStore<any>>;

/**
 * Attempt to migrated data.
 * https://tldraw.dev/reference/store/Store#migrateSnapshot
 */
export const migrateCanvas: MigrateCanvas = async (current, version = DEFAULT_VERSION) => {
