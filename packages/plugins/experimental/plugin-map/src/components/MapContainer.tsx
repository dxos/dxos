...
import { useControlledValue, useThemeContext } from '@dxos/react-ui';
...
const MapContainer = ({ role, type: _type = 'map', map }: { role?: string; type?: MapControlType; map: MapType }) => {
  const { tx } = useThemeContext();
  console.log('======== MapContainer', getDebugName(tx));
  ...
  return (
    <div role='none' className={mx('flex overflow-hidden', role === 'article' && 'row-span-2')}>
      ...
      {type === 'map' && <MapControl markers={markers} onToggle={() => setType('globe')} />}
    </div>
  );
};
