...
export class CallTranscription {
   ...
   */
  saveNetworkState(states: (TranscriptionState & { id: string })[]) {
    const maxTimestamp = Math.max(...states.map((transcription) => transcription?.lamportTimestamp ?? 0));
    const newTranscriptionState = states
      .filter((transcription) => transcription && transcription.lamportTimestamp === maxTimestamp)
      .sort((transcription1, transcription2) => transcription1.id.localeCompare(transcription2.id));

    if (maxTimestamp > this._state.value.lamportTimestamp! && newTranscriptionState.length > 0) {
      this._state.value = {
        ...this._state.value,
        ...newTranscriptionState[0],
      };
    }
  }
