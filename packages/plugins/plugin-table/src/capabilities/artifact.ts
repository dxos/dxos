...
import { PropertySchema, ViewProjection } from '@dxos/schema';
...
export default () => {
  const definition = defineArtifact({
    ...
    tools: [
      ...
      // TODO(ZaymonFC): We should move these to a general purpose view manipulation tool set.
      defineTool({
        name: 'table_add_column',
        description: 'Add a new column to a table with the specified format.',
        schema: S.Struct({
          id: QualifiedId,
          column: PropertySchema.annotations({
            description: 'The column configuration including property name, format, and other settings.',
          }),
        }),
        execute: async ({ id, column }, { extensions }) => {
          invariant(extensions?.space, 'No space');
          invariant(extensions?.dispatch, 'No intent dispatcher');
          ...
          const schema = space.db.schemaRegistry.getSchema(view.query.type);
          invariant(schema);

          const projection = new ViewProjection(schema, view);

          // Check if property name is unique
          if (view.fields.find((f) => f.path === column.property)) {
            return ToolResult.Error(`Column with name '${column.property}' already exists`);
          }

          const field = projection.createFieldProjection();
          projection.setFieldProjection({ field, props: column });

          return ToolResult.Success('Column added successfully');
        },
      }),
      defineTool({
        ...
        schema: S.Struct({
          id: QualifiedId,
          fieldId: S.String.annotations({
            description: 'The ID of the field (column) to delete.',
          }),
        }),
        execute: async ({ id, fieldId }, { extensions }) => {
          invariant(extensions?.space, 'No space');
          invariant(extensions?.dispatch, 'No intent dispatcher');
          ...
          const intent = createIntent(TableAction.DeleteColumn, {
            table,
            fieldId,
          });
          ...
          const { error } = await extensions.dispatch(intent);
          if (error) {
            return ToolResult.Error(error?.message ?? 'Failed to delete column');
          }

          return ToolResult.Success('Column deleted successfully');
        },
      }),
    ],
  });
