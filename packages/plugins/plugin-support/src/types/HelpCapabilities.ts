...
export const StateSchema = Schema.Struct({
  ...
  /**
   * Tours already run for this profile, so an `auto` tour fires on its first trigger only. Optional
   * because profiles persisted before tours existed have no such key, and a required one would fail
   * the whole blob's decode.
   */
...
/** The recorded tours, tolerating a state blob persisted before the key existed. */
