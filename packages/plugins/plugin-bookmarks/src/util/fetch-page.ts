 ...
 * See `plugin-video/src/util/fetch-page.ts` for the analogous declaration.
 */
...
/**
 * Fetch a raw resource body as text through the EDGE CORS proxy. The fallback path of
 * {@link fetchPage} when the extension render-proxy is unavailable.
 */
const fetchResource = (url: string): Effect.Effect<string, FetchError> =>
  Effect.tryPromise({
    try: async () => {
