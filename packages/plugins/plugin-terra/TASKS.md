...
- [x] **Task 10b — react-ui `Slider` + `TerraForm` panel** — new Radix-based `Slider` primitive in `@dxos/react-ui`; `TerraForm` via `react-ui-form` `fieldMap`; `labelEnd` slot added to `FormRow` so the value sits on the label line; schema `title` annotations for human labels; panel background. Sliders verified to regenerate the planet live.
  ...
  - Fixed: slider thumb circle was invisible because `is-4`/`bs-4` generate no CSS — the
    `tailwindcss-logical` plugin was dropped in the Tailwind v4 migration (#10611) without
    replacing its sizing utilities. `Slider.test.tsx` guards against the logical dialect returning.
...
- [x] **Rocket points where it is going** (user directive, 2026-07-31) — its pitch was a scripted cosine (+90° off the pad), not the angle it actually flew, so it skidded; it is now the flight-path angle of its own ballistic profile, and `motion.test.ts` pins it against a numerically differentiated trajectory at five points along the arc, which is the property the user could see was broken. Exhaust is orange (`TrailSpec.color`, per kind — a wake is spray, a plume is flame). New `scene/RocketArc` story: the arc seen edge-on with the planet dropped to the bottom of frame, looping launch → apex → impact → blast, with a `progress` control to hold any instant of it. That control exists because the loop is unreviewable otherwise — screenshot after screenshot landed on the blast and never the flight.

- [x] **Rocket presentation pass** (user directive, 2026-07-31) — silver body (red read as a toy against terrain), a short orange plume whose puffs start large at the nozzle and taper to nothing (`TrailSpec.endScale: 0`), and no plume at all once the engine cuts: the trail is gated on `state.phase === 'boost'`, and `BOOST_FRACTION` is now a quarter of the arc, well short of apogee, rather than 0.15. `scene/RocketArc` gained a toolbar with a `Launch` action that rewinds the sim clock — the rocket waits on the pad at its departure point between flights — and its camera is lifted 28° toward the zenith so the globe rolls toward the viewer and the ground under the flight reads as ground. The `progress` scrub control it briefly had is gone; it was scaffolding for a verification problem, not a feature.
