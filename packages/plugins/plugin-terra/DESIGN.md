...
- **routed** (boat, tank, plane): position is the point at arc length `speed × (t − windowStart)` along the current route polyline; radius is sea level, terrain height, or cruise altitude by kind.
