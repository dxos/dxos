...
- [x] **Hidden canvas showed as a second navtree item** — creating a sketch _inside a Collection_ listed the canvas beside it. Not variant-specific: `ContainerModel.add` pushed into `target.objects` unconditionally when the target was a collection, ignoring `HiddenAnnotation` (the non-collection branch calls `Database.add`, which is why creating at the space root looked fine). **Fixed systematically** in `@dxos/app-toolkit`: `ContainerModel.add` now persists a hidden object without filing it into any collection, so no plugin can leak an implementation-detail object into the navtree. Regression tests in `ContainerModel.test.ts`.
- [x] **Same latent bug in plugin-game** — `create-object.ts` routes the hidden `Chess.State` through `AddObject`; covered by the `ContainerModel.add` fix above, so plugin-game needs no change. Illustrator likewise keeps its plain `AddObject` call.
