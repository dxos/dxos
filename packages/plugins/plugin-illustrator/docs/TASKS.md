...
- [x] **Onboarding sample seeds `org.dxos.type.drawing`** — resolved upstream by the one-builder-per-sample-space change: `plugin-onboarding/src/content/sample/space.dx.json` now carries 2 `org.dxos.type.drawing` refs and no legacy `org.dxos.type.sketch`, so a first open no longer depends on the migration converting them.
