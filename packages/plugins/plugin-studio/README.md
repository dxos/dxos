...
A `MediaArtifact` is a media-agnostic unit of creative work — a prompt
...
owns the cross-cutting ontology (`MediaArtifact` / `Variant` / `Generation`) and the
...
Galleries reuse existing types: a masonry gallery is a `Collection<MediaArtifact>`;
