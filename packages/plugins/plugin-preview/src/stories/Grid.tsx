...
const [GridContextProvider, _useGridContext] =
	createContext<GridContextValue>("Grid");
...
const GridViewport = ({ classNames, children }: GridViewportProps) => {
	...
	const tabsterAttrs = useMergedTabsterAttributes_unstable(
		focusableGroupAttrs,
	...
	return (
		<div
			role="none"
			className={mx(
				"flex bs-full is-full overflow-x-auto p-2 gap-4",
				classNames,
			)}
			{...tabsterAttrs}
		>
	...
	({ classNames, children }: GridColumnProps, forwardedRef) => {
		...
		return (
			<div
				...
				{...tabsterAttrs}
				className={mx(
					"flex flex-col is-full overflow-hidden",
					classes.borderFocus,
					classNames,
				)}
			>
...
const GridStack = forwardRef<HTMLDivElement, GridStackProps>(
	(
		{ classNames, id, objects, Cell, canDrag, canDrop, ...props },
		forwardedRef,
	) => {
		...
		return (
			<div
				...
				role="none"
				className={mx(
					"flex flex-col is-full plb-2 overflow-y-auto",
					classNames,
				)}
				{...props}
			>
				...
				<div role="none" className="relative mli-2">
					<GridCellPrimitive
						...
						classNames={[
							...
							state.type === "dragging" && "opacity-25",
							classNames,
						]}
					>
						<Cell object={object} />
