...
// Kept out of `testing.tsx`: react-refresh only fast-refreshes a module whose exports are all
// components, so the object factories exported beside the story force a full page reload on every edit.
...
const TableLike = Schema.Struct({
  ...
  sizes: Schema.Record(Schema.String, Schema.Number).pipe(Schema.mutable, Annotation.FormInputAnnotation.set(false)),
