...
describe('rollup', () => {
  test('rolls up to the worst outcome', () => {
    ...
    expect(
      TestRun.rollup(
        ['QA-1', 'QA-2', 'QA-3'],
        [result('QA-1', 'passed'), result('QA-2', 'blocked'), result('QA-3', 'passed')],
      ),
    ).to.equal('blocked');
  });
  ...
  test('a failure dominates a block', () => {
    // T-2.
    expect(TestRun.rollup(['QA-1', 'QA-2'], [result('QA-1', 'failed'), result('QA-2', 'blocked')])).to.equal('failed');
  });
  ...
  test('an unreported case cannot seal as passed', () => {
    ...
    expect(TestRun.rollup(['QA-1', 'QA-2', 'QA-3'], [result('QA-1', 'passed')])).to.equal('skipped');
  });

  test('every captured case passing is the only route to passed', () => {
    expect(TestRun.rollup(['QA-1', 'QA-2'], [result('QA-1', 'passed'), result('QA-2', 'passed')])).to.equal('passed');
  });
  ...
  test('a result for a case the run did not capture does not count', () => {
    ...
    expect(TestRun.rollup(['QA-1'], [result('QA-2', 'passed')])).to.equal('skipped');
  });
});

describe('unreported', () => {
  test('names the captured cases that never reported', () => {
    expect(TestRun.unreported({ cases: ['QA-1', 'QA-2', 'QA-3'], results: [result('QA-2', 'failed')] })).to.deep.equal([
      'QA-1',
      'QA-3',
    ]);
  });
...
describe('tally', () => {
  test('counts passes against the captured total, not the reported one', () => {
    expect(TestRun.tally({ cases: ['QA-1', 'QA-2', 'QA-3'], results: [result('QA-1', 'passed')] })).to.deep.equal({
      passed: 1,
      total: 3,
    });
  });
