...
export const NativeSettings = (_props: NativeSettingsProps) => {
  ...
  const updateSpotlightShortcut = useCallback(
    (shortcut: string) => registry.set(settingsAtom, { ...registry.get(settingsAtom), spotlightShortcut: shortcut }),
    [registry, settingsAtom],
  );
  ...
  return (
    <SettingsForm.Viewport>
      <SettingsForm.Section title={t('settings.title')}>
        <SettingsForm.Item title={t('settings.spotlight.label')} description={t('settings.spotlight.description')}>
          <ShortcutInput shortcut={settings.spotlightShortcut} onChange={updateSpotlightShortcut} />
        </SettingsForm.Item>
