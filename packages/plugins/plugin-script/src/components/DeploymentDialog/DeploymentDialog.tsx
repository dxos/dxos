...
export const DeploymentDialog = ({
	...
	const { handleCreateAndDeployScripts, status } =
		useCreateAndDeployScriptTemplates(space, scriptTemplates);
			...
			void dispatch(
				createIntent(LayoutAction.AddToast, {
					...
					subject: {
						...
						duration: Infinity,
						title: [
							"script deployment toast label",
							{ ns: meta.id, count: scriptTemplates.length },
						],
						description: [
							"script deployment toast description",
							{ ns: meta.id, count: scriptTemplates.length },
						],
						closeLabel: [
							"script deployment toast close label",
							{ ns: meta.id, count: scriptTemplates.length },
						],
					},
				}),
			...
			void dispatch(
				createIntent(LayoutAction.AddToast, {
					...
					subject: {
						...
						duration: Infinity,
						title: [
							"script deployment error toast label",
							{ ns: meta.id, count: scriptTemplates.length },
						],
						description: [
	...
	return (
		<Dialog.Content>
			...
			<div role="none" className="flex flex-row-reverse gap-1">
				<Button
					variant="primary"
					onClick={handleCreateAndDeployScripts}
				...
				<Dialog.Close asChild disabled={status === "pending"}>
					<Button disabled={status === "pending"}>
						{t("deployment dialog skip button label")}
					</Button>
				</Dialog.Close>
			</div>
		</Dialog.Content>
