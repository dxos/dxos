...
export const TestPanel = ({ classNames, functionUrl }: TestPanelProps) => {
	...
	const handleResponse = ({
		...
		setHistory((history) => [
			...history,
			{ type: "response", text, data, error } satisfies Message,
		]);
		setResult("");
		setState(null);
		handleScroll();
			...
			>
				{history.map((message, i) => (
					<div key={i} className="grid grid-cols-[2rem_1fr_2rem]">
						<div className="p-1">
							{message.type === "response" && <RobotAvatar />}
						</div>
						...
						<div className="p-1">
							{(state === "pending" && (
								...
								<Icon
									...
									size={6}
									classNames="animate-[pulse_1s_ease-in-out_infinite]"
								/>
							)}
						</div>
						<div className="overflow-auto">
...
const MessageItem = ({
	classNames,
	message,
}: ThemedClassName<{ message: Message }>) => {
	...
	return (
		<div
			className={mx(
				"flex",
				type === "request" ? "ml-[1rem] justify-end" : "mr-[1rem]",
				classNames,
			)}
		>
			{error && (
				<div className={mx(wrapper, "whitespace-pre", errorText)}>
					{String(error)}
				</div>
			)}
			...
			{text !== undefined && (
				<div
					className={mx(
						wrapper,
						type === "request" && "bg-primary-400 dark:bg-primary-600",
					)}
				>
					{text || "\u00D8"}
				</div>
