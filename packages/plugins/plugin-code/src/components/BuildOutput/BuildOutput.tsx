...
export const BuildOutput = ({ state }: BuildOutputProps) => {
  ...
  if (!build && !run) {
    return (
      <div className='dx-expander grid p-2 overflow-auto text-xs text-description'>
  ...
  return (
    <div className='dx-container grid grid-rows-[auto_1fr] text-xs'>
...
const DiagnosticsList = ({ diagnostics }: DiagnosticsListProps) => {
  const { t } = useTranslation(meta.profile.key);
  return (
    <div className='dx-expander flex flex-col overflow-auto'>
