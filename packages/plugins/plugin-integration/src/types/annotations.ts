...
/**
 * Marks a `Schema.String` field as the integration-provider picker.
 *
 * Used in {@link Integration}'s create-dialog `inputSchema` to flag the
 * `providerId` field. A `role: 'form-input'` Surface contributed by
 ...
 */
export const IntegrationProviderAnnotationId = Symbol.for('@dxos/plugin-integration/annotation/IntegrationProvider');
