...
const _Channel = Schema.Struct({
  ...
  defaultThread: Type.Ref(Thread.Thread).pipe(Annotation.FormInputAnnotation.set(false)),
  // TODO(wittjosiah): Should be an "ordered collection".
  threads: Type.Ref(Thread.Thread).pipe(Schema.Array, Schema.mutable, Annotation.FormInputAnnotation.set(false)),
