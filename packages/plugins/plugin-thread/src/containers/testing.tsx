...
/**
 * The tile rendering a message, found by its body text — scoped queries land on one message even
 * when a run shares its avatar, and even when a quote repeats the text of the message it answers
 * (the quote lives inside the replying tile, not the quoted one).
 */
export const tileFor = async (canvasElement: HTMLElement, text: string): Promise<HTMLElement> => {
  const canvas = within(canvasElement);
  await waitFor(async () => {
    await expect((await canvas.findAllByText(text))[0]).toBeVisible();
  }, STORY_TIMEOUT);
  const tile = (await canvas.findAllByText(text))[0].closest('[data-testid="thread.message"]');
