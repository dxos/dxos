...
const getName = (
	...
	if (doc.content) {
		...
		return getTextInRange(
			createDocAccessor(doc.content.target!, ["content"]),
			start,
			end,
		);
	}
...
export const threads = (
	...
	const query = space.db.query(
		Query.select(Filter.ids(doc.id)).targetOf(DataType.AnchoredTo.AnchoredTo),
	);
	...
	const anchors = computed(() =>
		query.objects
			.filter((anchor) => {
				...
				return (
					Obj.instanceOf(Thread.Thread, thread) && thread.status !== "resolved"
				);
			})
	...
	return [
		...
		comments({
			...
			onDelete: ({ id }) => {
				...
				if (draft) {
					const index = draft.findIndex(
						(thread) => fullyQualifiedId(thread) === id,
					);
					if (index !== -1) {
				...
				const thread = query.objects.find(
					(object) => getSource(object).id === id,
				);
				if (thread) {
			...
			onUpdate: ({ id, cursor }) => {
				const draft = state.drafts[fullyQualifiedId(doc)]?.find(
					(thread) => fullyQualifiedId(thread) === id,
				);
				if (draft) {
				...
				const relation = query.objects.find(
					(object) => getSource(object).id === id,
				);
				if (relation) {
