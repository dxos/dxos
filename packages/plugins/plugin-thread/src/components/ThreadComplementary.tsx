...
export const ThreadComplementary = ({ subject }: { subject: any }) => {
	...
	const { state, getViewState } = useCapability(
		ThreadCapabilities.MutableState,
	);
	const drafts = state.drafts[subjectId];
	const viewState = useMemo(
		() => getViewState(subjectId),
		[getViewState, subjectId],
	);
	...
	const objectsAnchoredTo = useQuery(
		space,
		Query.select(Filter.ids(subject.id)).targetOf(AnchoredTo),
	);
	const anchors = objectsAnchoredTo
		.toSorted((a, b) => sort?.(a, b) ?? 0)
		.filter((anchor) =>
			Obj.instanceOf(Thread.Thread, Relation.getSource(anchor)),
		)
		.concat(drafts ?? []);
	...
	const handleThreadDelete = useCallback(
		(anchor: AnchoredTo) =>
			dispatch(createIntent(ThreadAction.Delete, { anchor, subject })),
		[dispatch, subject],
	);
	...
	const handleAcceptProposal = useCallback<
		NonNullable<CommentsContainerProps["onAcceptProposal"]>
	>(
		async (anchor, messageId) => {
			const thread = Relation.getSource(anchor) as Thread.Thread;
			const messageIndex = thread.messages.findIndex(
				Ref.hasObjectId(messageId),
			);
			const message = thread.messages[messageIndex]?.target;
			const proposal = message?.blocks.find(
				(block) => block._tag === "proposal",
			);
	...
	return (
		<StackItem.Content toolbar>
			...
			>
				...
				<Tabs.Tablist
					classNames={mx("bg-toolbarSurface border-b border-subduedSeparator")}
				>
				...
				<div className="overflow-y-auto">
					<Tabs.Tabpanel value="all">
						{showResolvedThreads && comments}
					</Tabs.Tabpanel>
					<Tabs.Tabpanel value="unresolved">
						{!showResolvedThreads && comments}
					</Tabs.Tabpanel>
				</div>
			</Tabs.Root>
