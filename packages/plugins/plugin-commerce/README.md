...
- A **Provider** is a template describing how to search one site: a typed search-field schema, a
  ...
  template is authored by an LLM (the provider skill analyses a real page and writes it) and is
  user-editable.
...
```bash
moon run plugin-commerce:build
moon run plugin-commerce:test
moon run plugin-commerce:lint -- --fix
```
...
- **LLM generates a parser** (`generate-template.skill.test.ts`): the headline end-to-end test.
  The provider skill runs against a real (cleaned) AutoTrader page with the edge fetch mocked;
...
The skill test replays a **recorded** conversation (`*.conversations.json`) so CI runs offline
and deterministically — no API key, no network. To re-record after changing the prompt, skill,
or tool schemas:
...
`generate-template.skill.conversations.json`. Commit the regenerated fixture. Replay matching is
