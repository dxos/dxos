...
export const NavTreeItemColumns = memo(({ path, item, open, density = 'fine' }: NavTreeItemColumnsProps) => {
  ...
  const { actions: actionsParam, groupedActions } = useActions(item);
  const [primaryAction, ...secondaryActions] = actionsParam.toSorted((a, _b) =>
    a.properties?.disposition === 'list-item-primary' ? -1 : 1,
  );

  const actions = (primaryAction?.properties?.disposition === 'list-item-primary' ? secondaryActions : actionsParam)
