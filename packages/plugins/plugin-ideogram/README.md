...
available as an image-generation provider for `@dxos/plugin-studio`.
...
2. An **`GenerationService`** implementation (the abstraction owned by
   `@dxos/plugin-studio`) that calls the Ideogram HTTP API.
