...
- [`src/SamplePlugin.ts`](../src/SamplePlugin.ts) — Plugin definition using `Plugin.define(meta).pipe(...)`. Each `.pipe()` call registers a capability module that activates at the appropriate lifecycle event.
...
1. **Root-level action** — `NodeMatcher.whenRoot` + `actions` adds a "Create Sample Item" action to the global menu.
...
5. **Plank companion** — `createTypeExtension` + `relation: AppGraphNode.companionRelation()` + `connector` attaches a "Related" side panel to SampleItem objects.
6. **Deck companion** — `createExtension` + `NodeMatcher.whenRoot` + `relation: AppGraphNode.companionRelation()` + `connector` adds a workspace-wide panel.
