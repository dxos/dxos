...
describe('importOBJ', () => {
  ...
  test('loads race.glb and parses mesh data', ({ expect }) => {
    const glbPath = resolve(dirname(fileURLToPath(import.meta.url)), '../..', 'assets/models/race.glb');
    const glbData = readFileSync(glbPath);

    // The race car GLB is likely not manifold (game model).
    const solid = importGLBDirect(glbData.buffer as ArrayBuffer, wasm);

    // Non-manifold meshes return null — verify the parser at least ran.
    if (solid) {
      expect(solid.getMesh().numTri).toBeGreaterThan(0);
      solid.delete();
    } else {
      // Parser ran but mesh wasn't manifold. Verify the GLB header is valid.
