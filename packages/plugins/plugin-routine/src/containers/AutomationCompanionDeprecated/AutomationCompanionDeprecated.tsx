...
export type AutomationCompanionDeprecatedProps = {
 ...
 * @deprecated
 */
export const AutomationCompanionDeprecated = ({ db, object }: AutomationCompanionDeprecatedProps) => {
  const { t } = useTranslation(meta.profile.key);
  const templates = useCapabilities(RoutineCapabilities.Template);
    ...
    <Panel.Root>
      ...
      <Panel.Content>
        {items.length === 0 ? (
          ...
          <Accordion.Root<Routine.Routine> items={items} value={open} onValueChange={setOpen}>
            {({ items }) => (
                      ...
                      <Accordion.ItemHeader icon='ph--lightning--regular'>
                        <span className='flex-1 truncate'>
                          {Obj.getLabel(automation) ??
                            t('object-name.placeholder', { ns: Type.getTypename(Routine.Routine) })}
                        </span>
                        {status !== 'associated' && (
                          <Tooltip.Trigger
                            asChild
          ...
          <DropdownMenu.Root>
            <DropdownMenu.Trigger asChild>
              ...
              <button type='button' className='group flex items-center p-2 dx-focus-ring-inset w-full text-start'>
                ...
                <span className='flex-1 truncate'>
                  {t('add-object.label', { ns: Type.getTypename(Routine.Routine) })}
                </span>
              </button>
...
const useConnectedAutomations = (db: Database.Database, object: Obj.Unknown) => {
  ...
  const handleCreate = useCallback(
    async (templateId: string) => {
      const { data, error } = await invokePromise(RoutineOperation.CreateRoutine, {
        db,
        templateId,
        subject: object,
      });
