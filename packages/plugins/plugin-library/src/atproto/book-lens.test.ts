...
const makeSampleBook = () =>
  Book.make({
    ...
    // Reading dates are date-only in ECHO (Format.Date); the lens widens them to the wire datetime.
...
describe('bookLens', () => {
  test('encodes the public face into a buzz.bookhive.book record', async ({ expect }) => {
    const record = await Panproto.encode(makeSampleBook(), bookLens);
    ...
    // status maps to the buzz.bookhive.defs knownValue reference.
  ...
  test('round-trips a book through the wire record', async ({ expect }) => {
    const record = await Panproto.encode(makeSampleBook(), bookLens);
    const decoded = await Panproto.decode(record, bookLens);

    // Identity nests back under catalog including the hiveId (a catalog identifier); authors decode to
    // an array (not the raw tab string), or `Form.ArrayField` throws when the imported Book renders.
