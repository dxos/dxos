# Evaluate queries in SQLite, behind a flag

The query planner gains a path that compiles a run of plan steps into a single SQL statement run against the index tables directly, instead of materializing a working set and walking it in memory. The compiler itself (`echo-host/src/query/sql/compile.ts`) and its wiring into the executor are covered elsewhere; this document covers the supporting changes the compiler depends on: the plan representation it emits, the index-core building blocks it reuses, and the schema it needs those building blocks to work against.

## A plan step that stands for a compiled statement

`QueryPlan.Step` gains a `SqlStep` variant. It carries the statement text, its bound parameters, and the original steps it replaces. Keeping the original steps means traces and scope analysis still see the plan they'd see without compilation, so the compiled path runs as a plan like any other rather than a separate execution mode bolted on the side.
...
## Reusing index conditions between executors

The compiled and in-memory executors have to agree on which rows a filter matches, so the conditions live in one place and both call it. `buildFtsCondition` moves the free-text matching logic out of `FtsIndex.query` into a standalone function returning a SQL fragment. Trigram tokenization can't rank terms shorter than three characters, so those fall back to `LIKE` and report `ranked: false`; the compiler needs that flag to decide whether it can order by BM25.

```diff file=packages/core/index-core/../../index-core/src/indexes/fts-index.ts lines=270-294

```

`index-core`'s barrel now also exports the equivalent helpers already living in `entity-meta-index.ts` (`buildQueueWindow`, `buildSourceCondition`, `buildTypeDxnCondition`, `localEntityId`) alongside a new `normalizePropPath`. The compiler needs these to build the same `WHERE` fragments the in-memory index builds, without reimplementing index internals in `echo-host`.

```diff file=packages/core/echo/index-core/src/index.ts lines=8-25
...
`normalizePropPath` strips array-index segments so `['items', '0', 'assignee']` and `['items', 'assignee']` compare equal; it backs the new `propPathNormalized` column described below.
...
## Schema support for the compiled joins

Compiling a plan step into SQL only helps if the join it needs can use an index. `parent`, `source` and `target` on `objectMeta` hold full EID strings, so a join to `objectId` would need string concatenation on the indexed side, which the existing `(spaceId, objectId)` index can't serve. Migration `0008` adds `parentId`, `sourceId` and `targetId`: the bare entity id when the reference is local to the row's space, `NULL` otherwise, each backed by its own index.
...
The same problem applies to reverse-reference lookups by property: matching against `propPath` means walking every row's path. `propPathNormalized` is written alongside `propPath` at insert time and indexed with `targetDXN`, so a property-scoped reverse lookup becomes an equality check.
...
Because the new column is `NULL` on rows written before it existed, the reverse-reference index name is bumped to `reverseRef3` so every object gets re-presented and the column backfilled.
...
Bumping the index name leaves cursor rows under the old `reverseRef2` name that would otherwise sit unread forever; this migration deletes them, which is a no-op on databases that never had them.
...
The column list the migration test checks picks up all six new columns.
...
## Guarding partial snapshot coverage

Full-text and other snapshot-dependent conditions in the compiled path assume every indexed object has a snapshot row. That's only true once the snapshot store has finished filling after its introduction, so `countMissingSnapshots` gives callers a way to check readiness before relying on it, rather than silently under-matching.
...
## Public surface for the compiler

The compile module's types and entry points are re-exported through a small barrel, giving `echo-host` one import path for the compiler instead of reaching into `compile.ts` directly.
...
58 hunks across 22 files the walkthrough above does not describe.
...
```diff file=packages/core/echo/echo-client/src/query/working-set-executor.ts lines=540-636
...
```diff file=packages/core/echo/echo-client/src/testing/echo-test-builder.ts lines=15-169
...
```diff file=packages/core/echo/index-core/src/index-engine.ts lines=102-214
...
```diff file=packages/core/echo/index-core/src/index.ts lines=27-30
@@ -18,4 +27,4 @@ export {
...
```diff file=packages/core/echo/index-core/src/indexes/entity-meta-index.test.ts lines=9-905
...
```diff file=packages/core/echo/index-core/src/indexes/entity-meta-index.ts lines=61-572
...
```diff file=packages/core/echo/index-core/src/migrations/entity-meta/index.ts lines=13-61
