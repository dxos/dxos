# Evaluate ECHO queries in SQLite behind a flag

This change introduces a SQL compiler that translates ECHO query plans into SQLite queries, evaluated behind a feature flag, alongside the index schema changes (normalized IDs, migrations) needed to support it. It also wires the new execution path through the query service/executor, adds extensive tests and benchmarks, and documents the design.

## SQL query compiler

This is the new module at the center of the change: `SqlPlanCompiler` walks a `QueryPlan.Plan` — the same plan the in-memory executor consumes — and lowers it to one recursive SQLite statement over the index tables (`objectMeta`, `objectSnapshot`, `reverseRef`, `ftsIndex`). Each plan step (`SelectStep`, `FilterStep`, `TraverseStep`, `UnionStep`, `AggregateStep`, etc.) becomes a CTE built on the previous one, so the compiler stays a single forward pass with no backtracking; the constraint this satisfies is that every relation in the chain carries the same working-set shape (`recordId`, `objectId`, `spaceId`, `rank`, `ord`, plus grouped columns once an `AggregateStep` runs) so steps compose regardless of order.

Two behaviors are load-bearing rather than incidental. First, `planReadsObjectMeta` declines to compile a plan whose filters touch `@meta` (foreign keys, registry key/version, tags): `objectSnapshot` strips `@meta` from document bodies, so those predicates have nothing to read in SQL, and the caller (covered in the pipeline wiring chapter) falls back to the in-memory path instead of silently returning wrong rows. Second, the compiler hand-picks indexes (`INDEXED BY`, `NOT INDEXED`) at specific joins — a deliberate override of SQLite's planner, which without `ANALYZE` statistics rates a `queueId` equality as cheap as a `(spaceId, objectId)` seek and ends up scanning every document per traversal step.
...
The predicate compilation mirrors the in-memory matcher term for term — `#compileRootPredicate` and `#compileValuePredicate` reproduce `makeFilterMatcher`'s typed comparisons (a number operand never matches a string value), `in`/`contains`/`range` semantics, and JSON-null vs SQL-null handling via `COALESCE`:
...
Three test files validate this from different angles instead of just one. `compile.test.ts` is the unit suite: it seeds the three index tables the way `IndexEngine.#update` would and checks filtering, ordering, traversal (references, relations, hierarchy), unions/set-differences, and aggregation (including group collapsing and per-zone day truncation) against expected object sets.
...
`explain.test.ts` asserts on `EXPLAIN QUERY PLAN` output directly, pinning the index-seek behavior the comment above calls out — a regression test for the 17x slowdown the authors measured when SQLite chose the wrong index:
...
`differential.test.ts` is the strongest correctness check: it generates random object bodies and random filter ASTs, compiles and runs each through SQL, and compares the result set against `filterMatchObjectJSON`, the in-memory matcher, over 200 rounds — catching any place the SQL translation diverges from the reference semantics that the rest of the system still depends on.
...
## Index schema normalization for SQL evaluation

The SQL compiler needs to express joins and property lookups as plain SQL predicates on indexed columns, but the existing `objectMeta` and `reverseRef` schemas store references as full EID strings and property paths with array-index segments, neither of which is directly joinable or matchable without per-row computation. This part adds normalized columns computed at write time so the compiled query path can join and filter with indexed equality instead of scanning and parsing.

For `objectMeta`, `parent`/`source`/`target` hold full EID strings (possibly space-qualified), which would require string concatenation against `objectId` to join — something the `(spaceId, objectId)` index can't serve. `localEntityId` strips a reference down to its bare entity id only when it resolves to the same space as the row, since a cross-space id could otherwise collide with a local one after joining:
...
These bare ids are written into new `parentId`/`sourceId`/`targetId` columns alongside the existing insert/update logic, and a code migration adds the columns plus supporting indexes (SQLite has no `ADD COLUMN IF NOT EXISTS`):
...
For `reverseRef`, an incoming-reference lookup by property name previously required walking every path variant for each array position. `normalizePropPath` strips numeric segments so `items.0.assignee` and `items.assignee` collapse to the same stored value, turning the lookup into an indexed equality on a new `propPathNormalized` column:
...
Because old rows lack this column, the reverse-ref index name is bumped from `reverseRef2` to `reverseRef3` so every object gets re-presented to indexing and the column backfilled; a tracker migration drops the now-dead cursor rows under the old name:
...
Two smaller pieces round out the schema work. `buildTypeDxnCondition` is rewritten from an `OR`/`LIKE` predicate into a bounded range on `(spaceId, typeDXN)` so SQLite can seek instead of scanning the whole space partition, and a new `hasCompleteSnapshots`/`countMissingSnapshots` pair on `ObjectSnapshotIndex` lets a reader detect a still-filling snapshot store (introduced after `objectMeta`) and wait rather than serve partial results. The compiled query path also depends on `json_type()` semantics only guaranteed from SQLite 3.45, so `migrate()` now checks the running version and dies with a clear error below that:
...
## Wire SQL execution into the query pipeline

This part connects the SQL compiler built elsewhere to the existing plan/executor machinery so a query either runs entirely in memory, as before, or as one compiled SQLite statement, chosen by a `QueryExecutorMode` that flows from the host down to each `QueryExecutor` instance. The mode is not a fork of the executor: a `SqlStep` is a new `QueryPlan.Step` variant, so the compiled path reuses the same step-execution loop, tracing, and result-shipping code as the in-memory steps, only skipping document loads.

`QueryPlan.SqlStep` stands in for the run of steps it replaces, carrying the statement text, bound params, and the original steps (kept for scope analysis and traces):
...
`QueryExecutor` resolves its mode once at construction — explicit option, else `DX_ECHO_QUERY_EXECUTOR` env var, defaulting to `memory` — and separately tracks whether the plan is actually compilable, since a plan reading `@meta` on document rows can't be answered from the snapshot store alone:

```diff file=packages/core/echo/echo-host/src/query/query-executor.ts lines=42-66
...
`execQuery` dispatches on the effective mode (`sql` only if requested _and_ compilable), keeping the two paths as separate private methods behind one entry point rather than branching deep inside the shared loop:
...
The compiled path (`#execCompiled`) calls the SQL compiler to turn the planner's output into a plan containing a `SqlStep`, then runs it through the ordinary `_execPlan`; `#execInMemory` is the renamed original body, unchanged in logic. Execution of the `SqlStep` itself (`_execSqlStep`) issues the statement via `SqlClient`, optionally runs `EXPLAIN QUERY PLAN` when tracing is enabled, and converts each returned row directly into a shippable `QueryItem` via `compiledRowToItem` — bypassing the document-derived result construction in `getResults` entirely, since compiled rows already carry the final shape.

`QueryServiceImpl` plumbs the mode through to every executor it creates and adds a snapshot-readiness gate: because the compiled path reads only the snapshot store (no document loading), a query opened before the store has finished backfilling after an upgrade must wait for indexing rather than return incomplete results.
...
`EchoHost` exposes the mode as a top-level constructor option (`queryExecutor`) and supplies the snapshot-completeness check to `QueryServiceImpl` from its index engine:

```diff file=packages/core/echo/echo-host/src/db-host/echo-host.ts lines=230-243
...
## Client test support for the new query path

`EchoTestPeer` gains a `queryExecutor` option so tests can pin a peer to either evaluation path instead of relying solely on the `DX_ECHO_QUERY_EXECUTOR` environment variable. It threads the `QueryExecutorMode` type straight through to `EchoHost`, so a test can construct a peer that forces SQL evaluation to exercise the new compiler deterministically, or force the memory path for comparison, without touching global env state.
...
Alongside this, `WorkingSetQueryExecutor` — the in-memory (host) executor used as the baseline for comparison tests — is adjusted so its ordering matches SQLite's collation exactly. String comparisons switch from `localeCompare` to code-unit comparison, since `localeCompare` disagrees with SQLite's binary collation on mixed-case ids and would otherwise produce spurious diffs between the two paths on the same query. The natural-order fallback and generic value comparator both make this switch, and the `MAX_DEPTH_FOR_CHILD_OF_TRACING` constant is lowered from 16 to 10 to match the depth limit used by the host executor and `DeletionResolver`, so recursive `child-of` queries resolve identically on both paths.
...
The `_compareValues` helper takes the same code-unit treatment for its string-coercion fallback, one more instance of the same fix.
...
## Benchmarks and design documentation

This chapter adds the two artifacts a reviewer needs to judge whether the new SQL executor is worth shipping and what it is meant to become: a benchmark suite comparing it against the legacy in-memory executor, and a design document explaining the compiler's shape, the semantic tradeoffs it makes, and which parts of the original proposal actually landed.

The benchmark, `query-executor.bench.ts`, runs the same mixed workload (tasks, notes, events, persons, organizations) through both executors, selected via `DX_ECHO_QUERY_EXECUTOR`, and measures three things per query shape: wall-clock time (tinybench), host-side execution cost (via `ExecutionTrace`, showing `docsLoaded` and `indexHits`), and process memory (peak/retained heap and RSS, sampled around forced GCs). It also covers a cold-start path (reload + reopen + first query) since that is where document loading costs are most visible. The results, appended to `BENCHMARKS.md`, are not a single number but a sequence of runs across commits, because the first run exposed a 17x regression on reference traversal that a later commit (`c5294281`) fixed by pinning index seeks — the benchmark caught a planner mis-pick that a correctness test alone would not.
...
The design document, `SQL_QUERY_EXECUTION.md`, states the problem (JS-side document loading dominates query cost), the constraints (three SQLite runtimes with different feature floors, bound-variable caps, no compatibility shims), and the compiler's per-step SQL shape. Critically, it documents where the shipped code diverges from the original proposal in an "Implementation notes" section: the compiler ended up in `echo-host` rather than `index-core` because of a dependency-direction constraint (`index-core` cannot depend on `echo-host`, and the plan types are `echo-host` modules), backfill uses fresh index cursors rather than cursor retirement, and the legacy in-memory executor was deleted outright once both benchmark runs were recorded, rather than kept behind the flag longer.
...
The rollout section frames the change as phase 1–3 of a five-phase plan (EDGE integration and property indexes are explicitly out of scope here), and the "Semantics" table enumerates every behavioral difference between the two executors (string collation, mixed-type comparisons, null ordering) so a reviewer can see that each is a deliberate, documented tradeoff rather than an oversight caught later by the differential test.
...
47 hunks across 21 files the walkthrough above does not describe.
...
```diff file=packages/core/echo/echo-host/src/db-host/echo-host.ts lines=83-193
...
```diff file=packages/core/echo/index-core/src/indexes/entity-meta-index.ts lines=93-572
...
```diff file=packages/core/echo/index-core/src/indexes/reverse-ref-index.ts lines=13-214
