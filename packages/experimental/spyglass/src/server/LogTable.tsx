...
import { type Log, type Message } from '../common';
...
export const LogTable = ({ log, compact = false }: { log: Log; compact: boolean }) => {
  const { messages } = log;
  if (!messages.length) {
    return (
      <div
  ...
  return compact ? <Compact log={log} /> : <Timeline log={log} />;
};

export const Compact = ({ log }: { log: Log }) => {
  const { messages } = log;
...
export const Timeline = ({ log }: { log: Log }) => {
  const { messages } = log;
      ...
      <thead>
        ...
        {messages.map(({ key, data }, i) => (
          <tr key={i} style={{ backgroundColor: i % 2 === 0 ? '#f5f5f5' : '#fff' }}>
            {keys.map((id) => (
              <td key={id} style={{ padding: '4px 8px' }}>
