...
export interface ModelInvokerFactoryApi {
  ...
  createChainResources(client: Client, dataDir?: string): ChainResources;
}
...
export class ModelInvokerFactory {
  ...
  public static createChainResources(client: Client, dataDir?: string): ChainResources {
    return (
      this._factory?.createChainResources(client, dataDir) ??
