...
resolve_backend() {
  case "$backend" in
    file)
      ...
      ;;
    mcp)
      printf 'BACKEND: mcp · DXOS Composer — the store is a live object graph reached through this session'"'"'s MCP tools, NOT a file. Do not read or write `%s`; if it exists it is a stale mirror. Object mapping: a registry entry IS a `Project`; the project'"'"'s TASKS.md ledger IS `Project.outline`, one markdown document holding the phases and `- [ ]` items; a durable task is a `Task` under the project'"'"'s TaskSet; DESIGN.md and other specs are artifact documents. THE VERBS ARE NOT TOOLS: the server advertises a small fixed surface — `queryOperations` (find operations; a `keys` lookup returns full input/output schemas), `invokeOperation` (`{ key, input, spaceId? }`, the only way to run one), and `loadSkill` (the workflow prose; call it before invoking). Operation KEYS for this workflow, under `org.dxos.plugin.projects.operation.` / `org.dxos.plugin.tasks.operation.` and resolvable by their final segment: `projectList` (list), `projectGet` (bare/resume/status), `projectUpdate` (end, status, pointer), `outlineGet` (tasks/resume), `outlineUpdate` (track, hydrate, checking items off), `projectCreate` (new — it scaffolds the project'"'"'s instructions, artifacts collection and TaskSet in one call, and returns the `taskSet` reference to pass to `taskCreate`), and `taskList`/`taskCreate`/`taskComplete`/`taskAssign` for durable tasks. Look a key up with `queryOperations` rather than guessing its input shape. Resolve the space with `listSpaces` unless DX_PROJECT_SPACE names one (%s). If `invokeOperation` is not available in this session, or a key it needs is absent from the catalog, say so and STOP — never silently fall back to the file backend, because a write that lands in a file the user believes is dead is exactly the divergence this backend exists to prevent.' "$registry" "${DX_PROJECT_SPACE:-unset}"
      ;;
    *)
