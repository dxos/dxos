...
resolve_backend() {
  case "$backend" in
    file)
      ...
      ;;
    mcp)
      printf 'BACKEND: mcp · DXOS Composer — the store is a live object graph reached through this session'"'"'s MCP tools, NOT a file. Do not read or write `%s`; if it exists it is a stale mirror. Object mapping: a registry entry IS a `Project`; the project'"'"'s TASKS.md ledger IS `Project.outline`, one markdown document holding the phases and `- [ ]` items; a durable task is a `Task` under the project'"'"'s TaskSet; DESIGN.md and other specs are artifact documents. TWO HOST SHAPES, so look at what this session actually advertises before assuming: a LOCAL host (`dx mcp serve`) exposes four tools — `loadSkill` (the workflow prose; call it first), `queryOperations` (find operations; a `keys` lookup returns full input/output schemas), `invokeOperation` (`{ key, input, spaceId? }`, the ONLY way to run a verb there), and `whoami`; the DEPLOYED host projects each verb as its own tool and adds generic object tools (`listSpaces`, `queryObjects`, `createObject`, `updateObject`, `getObject`). Use whichever seam is present. Operation KEYS, under `org.dxos.operation.projects.` / `org.dxos.operation.tasks.`: `.projects.create` (new — it scaffolds the project'"'"'s instructions, artifacts collection and TaskSet in one call, and returns the `taskSet` reference to pass to task create), `.projects.get` (bare/resume/status), `.tasks.getOutline` (tasks/resume), `.tasks.updateOutline` (track, hydrate, checking items off), and `.tasks.list`/`.create`/`.update` for durable tasks — `.update` carries `status` and `assignee`, which no longer have verbs of their own. Three things this workflow needs have NO project-specific verb and use the generic object ones: LISTING projects is a query by typename `org.dxos.type.project` (`queryObjects`, or `.space.queryObjects` through `invokeOperation`); PATCHING a project (end, status) is `updateObject` / `.space.updateObject`; creating a loose object is `createObject` / `.space.addObject`. Look a key up with `queryOperations` rather than guessing its input shape — the catalog is the authority and this list rots. Resolve the space with `listSpaces` where it exists, else `whoami`, whose `spaces` are the ones this session can write to, unless DX_PROJECT_SPACE names one (%s). If neither seam is present in this session — no `invokeOperation` and no projected verb — or a verb this operation needs is absent from the catalog, say so and STOP — never silently fall back to the file backend, because a write that lands in a file the user believes is dead is exactly the divergence this backend exists to prevent.' "$registry" "${DX_PROJECT_SPACE:-unset}"
      ;;
    *)
