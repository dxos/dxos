...
emit_setup() {
  emit "$(printf 'TASK-PLANNING PROJECT DIRECTIVE: `/dxos:project setup` — args: "%s". Bind this repo to the ECHO spaces its projects may live in, by writing the committed `.agents/projects/space.yml` (`spaces: [<id>, ...]` plus a `default: <id>` naming the one used unasked). The procedure belongs to the `project` skill the MCP server serves, so LOAD THAT SKILL FIRST (`loadSkill` where present, else the projected `project` prompt) and follow its setup section rather than improvising one: it lists the spaces by name, asks which, cross-checks the pick against the spaces this session can actually write to, and writes the file — adding the pick to `spaces` and making it the `default`, leaving any entries already there alone. The space is ALWAYS an explicit choice by the user — never infer one from a name resembling the repo, and a single space is not consent. Args, when present, name the space, and skip the question only when they match exactly one. If the BACKEND line below says `file`, write the binding anyway and say plainly that nothing reads it until DX_PROJECT_BACKEND is `mcp`. If the session advertises no MCP tools at all, say so and STOP rather than hand-writing a spaceId you cannot verify.' "$1")"
}
