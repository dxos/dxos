 ...
 */
export const apiDocGenerateDirective: Plugin = () => {
  ...
  return async (tree: any, vfile: any) => {
    ...
    await Promise.all(
      requests.map(async (node) => {
        ...
        const pkg = packagesInProject(api)?.find((p) => p.name === packageName);
        if (!pkg) {
          warn(`problem in file ${vfile.path}: package ${packageName} not found.`);
          return tree;
        }

        let symbol = findReflection(pkg, (node) => node.name === symbolName) as JSONOutput.DeclarationReflection;
        if (!symbol) {
...
export const apiDocRenderDirective: MarkdownIt.PluginSimple = (md) => {
  md.use(directive);
  md.use((md) => {
...
export const showcaseRenderDirective: MarkdownIt.PluginSimple = (md) => {
