...
export class EchoDataSource {
    ...
    async getRelationships({ label }) {
        ...
        const syntheticRefRelationships = (await this._getAllSchema()).filter((schema)=>getSchemaTypename(schema) !== StoredSchema.typename).flatMap((schema)=>{
            ...
            return relationships.map(([key, value])=>{
                const relationshipLabel = formatInferredRelationshipLabel(jsonSchema.typename ?? raise(new Error('Schema must have typename')), key);
                return {
        ...
        return (await Promise.all(objects.map((object)=>this._projectRefRelationship(object, relationship.property)))).flat();
    }
    async _getAllSchema() {
        return [
    ...
    async _projectRefRelationship(object, prop) {
        ...
        const value = Array.isArray(object[prop]) ? object[prop] : [
            object[prop]
        ];
        return Promise.all(value.map(async (ref)=>{
...
], EchoDataSource.prototype, "getNodes", null);
_ts_decorate([
    log.method(void 0, void 0, {
        F: __dxlog_file,
        L: 0,
...
], EchoDataSource.prototype, "getRelationships", null);
