...
const (
	...
	dxrpcPackage   = protogen.GoImportPath("github.com/dxos/dxos/dxrpc")
)
...
func (serviceGenerateHelper) generateClientStruct(g *protogen.GeneratedFile, clientName string) {
	...
	g.P("cc ", dxrpcPackage.Ident("ClientConnInterface"))
	g.P("}")
	g.P()
}
...
// generateFile generates a _dxrpc.pb.go file containing wsRPC service definitions.
func generateFile(gen *protogen.Plugin, file *protogen.File) *protogen.GeneratedFile {
	if len(file.Services) == 0 {
		return nil
	}
	filename := file.GeneratedFilenamePrefix + "_dxrpc.pb.go"
	g := gen.NewGeneratedFile(filename, file.GoImportPath)
	g.P("// Code generated by protoc-gen-go-dxrpc. DO NOT EDIT.")
	g.P("// versions:")
	g.P("// - protoc-gen-go-dxrpc v", version)
...
func generateFileContent(gen *protogen.Plugin, file *protogen.File, g *protogen.GeneratedFile) {
	...
	g.P("// is compatible with the dxrpc package it is being compiled against.")
	g.P("const _ = ", dxrpcPackage.Ident("SupportPackageIsVersion1")) // When changing, update version number above.
	g.P()
...
func genService(gen *protogen.Plugin, file *protogen.File, g *protogen.GeneratedFile, service *protogen.Service) {
	...
	g.P("func New", clientName, " (cc ", dxrpcPackage.Ident("ClientConnInterface"), ") ", clientName, " {")
	...
	g.P("func Register", service.GoName, "Server(s ", dxrpcPackage.Ident("ServiceRegistrar"), ", srv ", serverType, ") {")
...
func clientSignature(g *protogen.GeneratedFile, method *protogen.Method) string {
	...
	s += ", opts ..." + g.QualifiedGoIdent(dxrpcPackage.Ident("CallOption")) + ") ("
	if !method.Desc.IsStreamingClient() && !method.Desc.IsStreamingServer() {
...
func genClientMethod(gen *protogen.Plugin, file *protogen.File, g *protogen.GeneratedFile, method *protogen.Method, index int) {
	...
	g.P(dxrpcPackage.Ident("ClientStream"))
	g.P("}")
	g.P()
...
func genServiceDesc(file *protogen.File, g *protogen.GeneratedFile, serviceDescVar string, serverType string, service *protogen.Service, handlerNames []string) {
	// Service descriptor.
	g.P("// ", serviceDescVar, " is the ", dxrpcPackage.Ident("ServiceDesc"), " for ", service.GoName, " service.")
	g.P("// It's only intended for direct use with ", dxrpcPackage.Ident("RegisterService"), ",")
	g.P("// and not to be introspected or modified (even as a copy)")
	g.P("var ", serviceDescVar, " = ", dxrpcPackage.Ident("ServiceDesc"), " {")
	g.P("ServiceName: ", strconv.Quote(string(service.Desc.FullName())), ",")
	g.P("HandlerType: (*", serverType, ")(nil),")
	g.P("Methods: []", dxrpcPackage.Ident("MethodDesc"), "{")
	...
	g.P("},")
	g.P("Streams: []", dxrpcPackage.Ident("StreamDesc"), "{")
	for i, method := range service.Methods {
		if !method.Desc.IsStreamingClient() && !method.Desc.IsStreamingServer() {
			continue
		}
...
func genServerMethod(gen *protogen.Plugin, file *protogen.File, g *protogen.GeneratedFile, method *protogen.Method, hnameFuncNameFormatter func(string) string) string {
	...
	if !method.Desc.IsStreamingClient() && !method.Desc.IsStreamingServer() {
		g.P("func ", hnameFuncNameFormatter(hname), "(srv interface{}, ctx ", contextPackage.Ident("Context"), ", dec func(", protoPackage.Ident("Message"), ") error) (", protoPackage.Ident("Message"), ", error) {")
	...
	g.P("func ", hnameFuncNameFormatter(hname), "(srv interface{}, stream ", dxrpcPackage.Ident("ServerStream"), ") error {")
	if !method.Desc.IsStreamingClient() {
	...
	g.P(dxrpcPackage.Ident("ServerStream"))
	g.P("}")
	g.P()
