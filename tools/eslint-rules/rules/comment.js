...
const TODO_REMOVE_PATTERN = new RegExp(
  /TODO\(/.source + GITHUB_USERNAME.source + /\):/.source,
  ''
);
const FORMATTED_TODO_PATTERN = new RegExp(
  /^\s/.source + TODO_REMOVE_PATTERN.source + /\s/.source,
  ''
);
 ...
 */
const getNormalizedOptions = (rawOptions, which) =>
  Object.assign({}, DEFAULTS, rawOptions[which] || rawOptions);
...
module.exports = {
  ...
  create: (context) => {
    ...
    const isCommentValid = (comment, options) => {
      ...
      const commentWithoutTodo = commentWordCharsOnly.replace(
        TODO_REMOVE_PATTERN,
        ''
      );
