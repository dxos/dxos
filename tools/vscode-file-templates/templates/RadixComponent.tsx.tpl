...
const ${name}Viewport = ({ classNames, children }: ${name}ViewportProps) => {
  ...
  return <div className={mx('flex h-full w-full overflow-y-auto', classNames)}>{children}</div>;
};
...
const ${name}Content = ({ classNames }: ${name}ContentProps) => {
  ...
  return <div className={mx('flex flex-col w-full', classNames)}>{JSON.stringify(context)}</div>;
};
