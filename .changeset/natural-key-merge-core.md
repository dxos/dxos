...
The merge is not yet run automatically on space open. Detection has no way to ask which entities declare a natural key, so it scans, which would hydrate the whole space into the working set on every open; making it automatic needs `meta.naturalKey` indexed first.
