...
**The backend is not browser-only.** The protocol code lives in `@dxos/blob-s3` and its database bindings in `@dxos/compute-runtime`, whose `FunctionContext` registers the backend on the `EchoClient` it builds for operations. So a function — including one invoked by an agent over MCP — writes to the bucket its space is connected to, rather than falling back to inline storage and its 4 MiB cap. Nothing Cloudflare-specific is involved: the backend makes an outbound request to the customer's own endpoint.
