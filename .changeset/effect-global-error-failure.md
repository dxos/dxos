...
`DevtoolsHost`'s seven unimplemented stubs fail with `NotImplementedError` naming the method, rather than a fallback error whose message asserted that an operation failed. `ClientServiceError` had no other caller and is removed.
