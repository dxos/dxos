...
Migration definitions now carry `Migration.TypeId` and a `kind` discriminant: `Migration.ObjectMigration` (from `Migration.define`) and `Migration.RenameMigration` (from `Migration.defineRename`) both extend the `Migration.Migration` base, with `Migration.Any` as their union and `Migration.isMigration` as the type guard. `EchoDatabase.runMigrations` accepts both kinds.
