...
the second re-tokenizes the full-text index from what the first wrote, tracking the monotonic
`objectMeta.version` counter as an ordinary cursor in `indexCursor`.
...
index and a burst of edits is caught up once rather than per save. Editing a 113KB document with a
