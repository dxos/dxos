...
`SkillsAnnotation` moves from `@dxos/app-toolkit/AppAnnotation` to `@dxos/compute/Skill` (the annotation is id-keyed, so stored data is unaffected — only the import path changes). `Project` now carries the annotation (its project skill plus the artifact-type skills), which is what scopes a project's companion chats and template-created routines; the redundant `ProjectOperation.CreateRoutine` verb is removed — create routines through the automation companion's template menu (or `RoutineOperation.CreateRoutine`) instead.
