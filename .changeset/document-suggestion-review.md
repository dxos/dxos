...
- **@dxos/plugin-comments**: a unified review companion — comment threads and suggestion cards in one surface. `SuggestionThread` renders each grouped change (`useSuggestionSources` → the document's `kind:'suggestion'` branches) as a change-block tile, routing Accept/Reject to the durable `AcceptChange`/`RejectChange` ops; empty suggestion branches are swept on open.
