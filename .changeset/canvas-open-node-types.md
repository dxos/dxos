...
Persistence and the plugin switch: `createEchoStore(board)` makes a `CanvasBoard`'s own `layout` the scene store, deriving z-order from array order so a board written by the previous editor opens unchanged, and `plugin-conductor`'s `CanvasArticle` now renders `SceneView` over that store with the compute projection instead of the editor's canvas. Auto layout is a `layout` intent the projection answers, ranking the scene into rows through `@dxos/diagram`; `Capabilities.layout` enables the toolbar's button, which lays out the selection when two or more elements are selected and the board otherwise.
