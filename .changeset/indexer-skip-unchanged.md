...
Requests are now attributed on the pass span: `rpc-update-indexes` for the client's `DataService.updateIndexes`, `feed-query-open` for a feed-scoped query opening, and `epoch` for epoch creation, instead of one label for all three.
