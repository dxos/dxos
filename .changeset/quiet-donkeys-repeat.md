...
Add `SqlMigrations` for executing multi-statement SQL scripts, and `SqlTransaction.clientLayer` for running `@effect/sql`'s migrator on platforms whose SQL client cannot emit `BEGIN`/`COMMIT`, such as Cloudflare Durable Objects.
