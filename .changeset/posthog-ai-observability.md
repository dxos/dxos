...
The process manager now has a real tracer. Effect's default is a no-op, so every `withSpan` in the app was created and discarded; `@dxos/app-framework` installs one over the OpenTelemetry global provider — a proxy, so it no-ops until something registers a real provider and delegates from then on — and every fiber on the process-manager runtime inherits it. A plugin can still contribute its own through `RuntimeServices`, which wins. AI capture is then only what the baseline does not give — a span processor and effect's content hook — rather than a provider, sampler and tracer of its own.
