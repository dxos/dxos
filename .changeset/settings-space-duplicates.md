...
Absence is unprovable in an eventually-consistent system, so instead of guarding the create the profile now converges: the canonical settings space is resolved deterministically (designation first, then space id, so every device picks the same winner), and once it is ready each duplicate's cross-space ordering is folded into it and the duplicate is tombstoned, which replicates to the user's other devices.
