...
- plugin-projects: a project's Sessions and Artifacts branches and everything under them are addressed by the `project` key, extending the project's own id (`project/<project>+chats+<session>`), so opening one from the navtree no longer fails with "node has no URL binding".
