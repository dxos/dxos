...
- plugin-projects: a project's sessions and artifacts carry their own URL bindings, `project-session` and `project-artifact`, so opening one from the navtree no longer fails with "node has no URL binding".
