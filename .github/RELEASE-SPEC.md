...
`.changeset/config.json`, generated by the toolbox (`updateChangesets()`) from the workspace/project graph on `pnpm install`:
...
`fixed` (not `linked`): `fixed` bumps and publishes all members together every release (lockstep); `linked` only syncs packages that changed — not what the core/SDK surface wants.
