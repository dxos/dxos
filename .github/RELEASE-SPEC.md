...
**One changeset per PR is gated too.** `scripts/check-changeset-count.mjs` (the `Check changeset count` step) fails a PR that adds more than one `.changeset/*.md`. A changeset is a changelog _entry_, and a file per commit fragments one story across several entries; the genuinely-two-stories PR waives it with a `# multiple-changesets: <reason>` YAML comment in a changeset's front matter, which the parser drops. Counting is diff-based against the merge base and the step is `pull_request`-only: `.changeset/` holds every unreleased entry on `main`, and a `merge_group` batches PRs that each carry their own.
