...
**`major` is gated in CI while pre-1.0.** `scripts/check-changeset-bumps.mjs` (the `Check changeset bump levels` step) fails the build on any `major` in `.changeset/*.md`. The groups are `fixed`, so one stray `major` versions every package to `1.0.0` in a single release, and nothing surfaces it until `changeset version` runs on `main`. The script no-ops once both group anchors leave `0.x`; **delete it and its step as part of the PR that cuts `1.0.0`**, which needs the very `major` it rejects.
...
- **`preview` the environment vs. a per-PR preview.** The `preview` environment is the daily dogfood deploy on `preview.composer.space`. A per-PR preview is a Worker preview _version_ of `env.dev`, built and deployed by `pr-build.yml`/`pr-deploy.yml` — named for its trigger so the two do not read as the same thing.
...
**Pending human setup for the `nightly` → `preview` rename.** The environment name is the Worker name, the domain, and the CrabNebula channel, so none of it moves on its own. **Cloudflare:** the first `preview` deploy creates `composer-preview` (and the other apps' `<app>-preview` Workers); give each its `SIGNOZ_INGESTION_KEY` secret (`pnpm secrets remote preview`) and rename the matching 1Password sections from `<app>-nightly`; move `nightly.composer.space` onto `composer-preview` as `preview.composer.space` (deploy first, then switch — no gap), keeping the old hostname as a redirect for as long as bookmarks warrant; retire the `<app>-nightly` Workers once `preview` has deployed cleanly. **CrabNebula:** the channel is now `preview`, so existing `nightly` desktop installs stop receiving updates — their baked-in updater endpoint polls `?channel=nightly` and nothing will publish there again. Announce the one-time reinstall from the help menu's download link, then retire the `nightly` channel. **Deploy tags:** `<app>/nightly` and `composer-nightly-<base>-<n>` stop moving; the new lines are `<app>/preview` and `composer-preview-<base>-<n>`, and the build counter restarts at 0.
