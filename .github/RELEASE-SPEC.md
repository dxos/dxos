...
- **`preview` the environment vs. a per-PR preview.** The `preview` environment is the daily dogfood deploy on `preview.composer.space`. A per-PR preview is a Worker preview _version_ of `env.dev`, built by `pr-build.yml` and deployed by `pr-deploy.yml` — named for their trigger so the two do not read as the same thing.
