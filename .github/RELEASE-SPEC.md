...
- **`format: "oxfmt"`** — pinned rather than left on Changesets v3's `auto` detection, so a generated `CHANGELOG.md` matches what **Check**'s `oxfmt --check` expects.
...
**The `0.x` → `1.0.0` cascade is fixed upstream (Changesets v3).** Through v2, a `minor` on a `0.x` fixed-group member escalated the whole group to `1.0.0`: Changesets force-escalated any peer-dependent of a non-patch change to `major`, and a fixed group propagated that to every member. Holding it back took `onlyUpdatePeerDependentsWhenOutOfRange: true` plus a local `@changesets/assemble-release-plan` patch. **v3 removed the force-escalation** — a peer-dependent now takes a `patch` like any other dependent — so both were dropped and neither should be reintroduced.

One thing from that era stays, on its own merits: intra-repo **`peerDependencies` use `workspace:^`** (caret), not `workspace:*`, so a published consumer resolving the group is not pinned to one exact version. See the `peerDependencies` non-negotiable in `AGENTS.md`.
