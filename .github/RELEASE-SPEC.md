...
- **Composer keeps old chunks in R2.** Before `wrangler deploy`, `upload-assets.mjs` copies the build's hashed `assets/` into the env's `ASSET_ARCHIVE` bucket, and the Worker serves a chunk the live build no longer has from there, so a tab left open across a deploy can still load it. The buckets and their lifecycle rules live only in the Cloudflare dashboard: `composer-assets` (production, 30 days), `composer-assets-preview` (7 days), `composer-assets-staging` and `composer-assets-dev` (2 days). Expiry counts from an object's last upload. The deploy authenticates with `R2_ACCESS_KEY_ID` / `R2_SECRET_ACCESS_KEY`, an Object Read & Write token scoped to those four buckets (1Password CI vault, "Composer asset archive R2 token (CI)"); without them the deploy skips the upload with a warning.
