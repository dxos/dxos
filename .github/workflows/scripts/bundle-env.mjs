...
// rebuild — see the App deploys section of .github/RELEASE-SPEC.md).
//
...
if (apps.length === 0) {
  const valid =
    resolveApps(root, { environment })
      .map((app) => app.name)
      .join(', ') || '(none)';
  console.error(`"${only}" has no env.${environment} — nothing would be bundled. Apps with that env: ${valid}`);
  process.exit(1);
}
