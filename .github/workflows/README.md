...
| `boot-budget` | `composer-app:check-boot-budget` — the static boot-graph budget over the production bundle, on its own runner (it builds `bundle`, which nothing else in the workflow does). On a same-repo PR it also rebuilds the base commit and posts a sticky `<!-- boot-budget -->` comment when the graph grew — advisory, so only the budget itself gates. | — |
