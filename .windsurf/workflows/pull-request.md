...
6. Afterwards you will be waiting for CI to pass. Use `bun run gh-action --verify --watch --porcelain` to check the current state. If the workflow is still running, wait for it to complete. If the workflow failed -- fix errors and start this checklist again. If it suceeded -- break.
