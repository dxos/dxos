...
src/
  ...
  internal/        unchanged (private impl)
```
...
- [ ] Diff the three `index.ts` files and confirm the structure is consistent (stages barrel exported; types barrel present; testing subpath). Note the intentional divergences (rdf `Type` namespace; transcription top-level types) in a one-line comment each.
