...
Three stores, bridged by DXN refs.

### 4.1 Fact graph (`@dxos/semantic-index`, unchanged)

`Fact = { id, Assertion(subject, predicate, object, validFrom?, validTo?, quote?), Valence(factuality, polarity, confidence, nature?), Attribution(source, agent, generatedAtTime, wasDerivedFrom?, span?), Entity[] (ref? → ECHO), recordedAt, extractor, sourceHash }`. `source` = the `Message` DXN. Append-only, incremental via `sourceHash`/cursor.
...
### 4.3 New ECHO entity types (materialized projections)

Home: a new `@dxos/inbox-*` schema module (or `@dxos/types` extension — see Open Questions).
...
Refs use DXN throughout, mirroring `Entity.ref` and `extract-contact`, so the fact graph and ECHO graph reconcile on the same identities.

## 5. Capability & permission model (cross-cutting)

Two **distinct** permission kinds:

- **Action permissions** — what the agent may *do*: `summarize`, `extract`, `createTask`, `draftResponse`, `sendResponse`, `spawnResearch`, `scheduleEvent`, …
- **Access permissions** — what data the agent may *read* to perform an action: `calendar`, `contacts`, `space:<dxn>`, `thread-history`, `external:<domain>`, …
...
When an action needs data it lacks permission for, the agent emits an `AccessRequest` rather than proceeding. The user grants (or denies); a grant appends to the relevant `PermissionConfig.grantedAccess`. This is how the agent *requests* new capabilities and gradually accumulates them.
...
Every `DraftResponse` carries `groundedIn: Fact[]` so a human (or auditor) can trace *why* the agent said what it said back to message spans.
...
- New schema module for the ECHO entity types (§4.3) and `PermissionConfig`.
...
1. **Entity home** — do the new ECHO types (`Thread`, `Topic`, `ActionItem`, …) live in `@dxos/types`, a new `@dxos/inbox-schema`, or inside `@dxos/pipeline-email`? Leaning: a dedicated schema module so consumers don't depend on the pipeline.
2. **Fact vs entity duplication** — an `ActionItem` is both a `Fact` (`owes`/`dueBy`) and a materialized entity. Is the entity a *cache/projection* of facts (rebuildable) or an authoritative object? Leaning: projection, rebuildable from facts, to keep the fact graph the source of truth (validates semantic-index).
3. **Meta-thread clustering method** — embedding similarity, shared-entity graph, LLM grouping, or hybrid? Deferred to slice 3 spike.
...
6. **semantic-index gaps** — append-only model currently appends competing facts on source change (supersession deferred). Thread/commitment logic may need supersession sooner; may feed back into the semantic-index experiment.
