...
| EG-T4 coverage              | 4.1 was **already done upstream** — the live entrypoint (`entrypoint/functions-service.ts` default export) ships `instrumentEntrypoint`-wrapped on edge main; the PR's `main.ts` hunk was a no-op on a dead module and was reverted. Lazy-DO enforcement + worker triage open. |
