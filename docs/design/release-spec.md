...
- **Apps deploy, never publish** ✅ / 🔄 — every `private: true` app (Composer `composer-app` / `composer-crx` / `composer-dxos-org`, `docs`, `todomvc`, `tasks`, `testbench-app`) deploys via its own pipeline and **never** triggers an npm publish. `composer-app` / `composer-crx` keep an **independent version line** (named directly in a changeset) for the desktop/extension build; the rest deploy keyed on git branch + SHA and carry no semver.
