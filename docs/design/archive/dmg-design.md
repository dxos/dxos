...
Any of those meta can choose to redirect to another meta, for example 'refs/com/pcarrier/:meta`could say`refs/com/pcarrier/alice`is in fact`refs/org/dxos/alice`(or`refs/com/alice`) which would fork the traversal into:
