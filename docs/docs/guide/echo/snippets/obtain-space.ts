...
// create an invitation to join the space, it will be used to locate the inviter in the MESH and to create a secured peer connection
const { invitation } = space.createInvitation();

// later we will pass this second authentication code to our friend over a side-channel and they'll send it to us over the new secure connection which will verify that it's secure.
const authCode = invitation?.authenticationCode!; 

// ===== on the receiver's side:

// share this code with a friend, it will be used to establish a secure connection
