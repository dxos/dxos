...
There are a few ways a user can establish their identity (login) with DXOS on any given [device](/docs/additional-resources/glossary#device).
...
3. Recovering an identity using a [recovery code](/docs/additional-resources/glossary#recovery-code), sometimes referred to as paper key recovery.
...
- Use the [`useIdentity` hook in react](/docs/halo/react)
- Use the [`client.halo.profile` field in TypeScript](/docs/halo/typescript)
