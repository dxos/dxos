...
1. <Badge text="Peer A" variant='tip' /> listens on the peer network for peers intereseted in a specific [invite code](/docs/additional-resources/glossary#invitation-code) it generated.
2. <Badge text="Peer B" variant='note' /> obtains the [invite code](/docs/additional-resources/glossary#invitation-code) and locates the listening <Badge text="Peer A" variant='tip' /> via the [signaling network](/docs/additional-resources/glossary#signaling-service).
...
4. <Badge text="Peer A" variant='tip' /> generates an [authorization code](/docs/additional-resources/glossary#authorization-code) and sends it to <Badge text="Peer B" variant='note' /> via another safe channel (i.e.: shows a QR code).
5. Finally, <Badge text="Peer B" variant='note' /> provides the [authorization code](/docs/additional-resources/glossary#authorization-code) back to <Badge text="Peer A" variant='tip' /> over the new connection.
...
* Install the appropriate npm package [`@dxos/client`](/docs/echo/installation/typescript) or [`@dxos/react-client`](/docs/echo/installation/react)
* Create a [Client](/docs/echo/typescript/api) (or a [ClientProvider](/docs/echo/react/api) in react)
* Set up an identity with [HALO](/docs/halo/introduction)
* Create or Join a [Space](#spaces)
* [Query objects](/docs/echo/typescript/queries) (in [react](/docs/echo/react/queries))
* [Create objects](/docs/echo/typescript/mutations#creating-objects) (in [react](/docs/echo/react/mutations))
* [Mutate objects](/docs/echo/typescript/mutations) (in [react](/docs/echo/react/mutations))
...
* If using `react` see the [React guide](/docs/echo/react/api)
* Otherwise, follow the [TypeScript guide](/docs/echo/typescript/api)
