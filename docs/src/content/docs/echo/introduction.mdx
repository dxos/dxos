...
1. <Badge text='Peer A' variant='tip' /> listens on the peer network for peers
   intereseted in a specific [invite
   code](/additional-resources/glossary#invitation-code) it generated.
2. <Badge text='Peer B' variant='note' /> obtains the [invite
   code](/additional-resources/glossary#invitation-code) and locates the
   listening <Badge text='Peer A' variant='tip' /> via the [signaling
   network](/additional-resources/glossary#signaling-service).
3. <Badge text='Peer A' variant='tip' /> and
   <Badge text='Peer B' variant='note' /> establish a secure connection via
   [Diffie
   Hellmann](https://en.wikipedia.org/wiki/Diffie%E2%80%93Hellman_key_exchange)
   key exchange.
4. <Badge text='Peer A' variant='tip' /> generates an [authorization
   code](/additional-resources/glossary#authorization-code) and sends it to
   <Badge text='Peer B' variant='note' /> via another safe channel (i.e.: shows
   a QR code).
