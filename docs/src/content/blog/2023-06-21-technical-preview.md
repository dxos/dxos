...
- [**Composer.**](https://composer.space) We're building a local-first, collaborative, extensible text editor. We built a minimal markdown editor to power the Composer Extension, and now we're extending it into a plugin-friendly environment for hosting DXOS apps. It's shaping up to be something in between Notion and Airtable with a sprinkle of macOS Widgets.
