...
const groupByModule = (list: string[]) =>
  list.reduce<{ [key: string]: string[] }>((grouped, item) => {
...
export const apiSidebar = () => {
    ...
    API_SECTIONS.reduce(
      (lookup, [folder]) => ({
        ...
        [folder]: groupByModule(
          readDir(join(apiPath, folder)).filter(
            (interfaceFile) => !interfaceFile.includes("defs")
          )
        ),
      }),
      {}
  ...
  const modules = readDir(join(apiPath, "modules"))
  ...
  return [
    ...PINNED_PACKAGES.map((pinned) =>
      modules.find(({ text }) => text === pinned)
    ),
