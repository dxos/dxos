...
export const pathToReflectionId = (
  ...
  if (root.id == id) return pathSoFar;
  if (!root.children?.length) return pathSoFar;
  for (let child of root.children) {
...
export const packageOfReflectionId = (
  root: S.ContainerReflection,
  id: number
) => {
  return pathToReflectionId(root, id).find(
    (r) => r.kind == ReflectionKind.Module
  );
};
